...
public partial class IpassFeedBackFormNew : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        //}
        if (Request.QueryString["SMSNo"] != null)
        {
        }
        if (!IsPostBack)
        {
    ...
    public void GetDetails()
    {
        ...
        //return;
        dsn = FetchData(Request.QueryString["SMSNo"].ToString());
        if (dsn.Tables[0].Rows.Count > 0)
        {
            grdDetails.DataSource = dsn.Tables[0];
            grdDetails.DataBind();
            TxtnameofUnit.Text = dsn.Tables[0].Rows[0]["Unit_Name"].ToString().Trim();
            //txtpromotername.Text = dsn.Tables[0].Rows[0]["intQuessionaireid"].ToString().Trim();
            txtmobileno.Text = dsn.Tables[0].Rows[0]["MobileNumber"].ToString().Trim();

            if (dsn.Tables[0].Rows[0]["FeedBackType"].ToString().Trim() == "Y")
            {
    ...
    public DataSet FetchData(string smsno)
    {
        DataSet Dsnew = new DataSet();

        SqlParameter[] pp = new SqlParameter[] {
             new SqlParameter("@smsno",SqlDbType.VarChar)
        ...
        Dsnew = Gen.GenericFillDs("USP_GET_DATA_FEEDBACK_OTHERDEPARTMENTS",pp);
        return Dsnew;
    }
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            foreach (GridViewRow row in grdDetails.Rows)
            {
                ...
                osqlConnection.Open();
                SqlCommand oSqlCommand = new SqlCommand("SP_TsipassFeedBack_Other");
                oSqlCommand.Connection = osqlConnection;
                oSqlCommand.CommandType = CommandType.StoredProcedure;
                oSqlCommand.CommandText = "SP_TsipassFeedBack_Other";
                ...
                oSqlCommand.Parameters.AddWithValue("@UID_Number", "");
                oSqlCommand.Parameters.AddWithValue("@CreatedBy", "");
                ...
                oSqlCommand.Parameters.AddWithValue("@SMSNo", Request.QueryString["SMSNo"].ToString());
