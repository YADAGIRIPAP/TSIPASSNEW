...
public partial class UI_TSiPASS_frmstatusNew : System.Web.UI.Page
{
    int delete = 0;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                ds = Gen.RetriveStatusForViewApplicationNew(Request.QueryString[0].ToString().Trim(), ddlCategory.SelectedItem.Text, ddldistrict.SelectedValue, FromdateforDB, TodateforDB);
                ...
                HyperLink1.NavigateUrl = "frmR1ReportKMRNew.aspx?status=" + status + "&lbl=Total Applications Received&fromdate=" + txtFromDate.Text + "&todate=" + txtTodate.Text;
            }
        }
        catch (Exception ex)
        {
        ...
        try
        {
            using (StringWriter sw = new StringWriter())
            {
                using (HtmlTextWriter hw = new HtmlTextWriter(sw))
                {
                    ...
                    Response.AddHeader("content-disposition", "attachment;filename= R1.2TotalApplicationsReceived.pdf");
