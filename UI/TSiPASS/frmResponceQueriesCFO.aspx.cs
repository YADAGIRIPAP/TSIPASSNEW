...
public partial class TSTBDCReg1 : System.Web.UI.Page
{
    ...
    void FillDetails()
    {
        ...
        try
        {
            ...
            ds = Gen.GetQueryStatusByTransactionIDCFO(Request.QueryString[0].ToString());
    ...
    protected void BtnClear_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            result = Gen.InsertQueryDetailsCFO(intEnterpreniourApprovalid, intQuessionaireid, intCFEEnterpid, intDeptid, intApprovalid, QueryRaiseDate, QueryDescription, QueryStatus, ViewState["AttachmentName"].ToString(), ViewState["pathAttachment"].ToString(), "", queryresopnedesc, Created_by, "", "", "", getclientIP());
            ...
            if (result > 0)
            {
                ...
                BoilerService.TSBoilerServiceImplService BoilerQUERY = new BoilerService.TSBoilerServiceImplService();
                ...
                else if (intApprovalid == "67")//BOILERS
                {
                    ...
                    if (dsdept.Tables[0].Rows.Count > 0)
                    {
                        ...
                        for (int k = 0; k < dtraw.Rows.Count; k++)
                        {
                            ...
                            BoilerService.queryResponseAttachment boilervoQ = new BoilerService.queryResponseAttachment();
                            boilervoQ.documentName = dtraw.Rows[k]["queryRespDocName"].ToString();
                            boilervoQ.documentPath = dtraw.Rows[k]["queryRespDocPath1"].ToString();
                            //if (dtraw.Rows[k]["queryRespDocName"].ToString() != "NA")
                            //{
                            //    boilerattachmentvo.documentName = dtraw.Rows[k]["queryRespDocName"].ToString();
                            //}
                    ...
                    string boilerout = BoilerQUERY.insertIntoSPLQueryresponse(boilerattachmentvo);
                    if (boilerout == "SUCCESS")
                    {
                        Gen.UpdateDepartwebserviceflagCFO(UIDNO, intApprovalid, "Q", boilerout, "Y");
                        int k = Gen.InsertDeptDateTracingCFO(intDeptid, intQuessionaireid, UIDNO, null, null, null, System.DateTime.Now.ToString("MM/dd/yyyy"), null, "CFO", intApprovalid);
                ...
                else if (intApprovalid == "68")//BOILERS
                {
                    ...
                    if (dsdept.Tables[0].Rows.Count > 0)
                    {
                        ...
                        for (int k = 0; k < dtraw.Rows.Count; k++)
                        {
                            ...
                            BoilerService.queryResponseAttachment boilervoG = new BoilerService.queryResponseAttachment();
                            boilervoG.documentName = dtraw.Rows[k]["queryRespDocName"].ToString();
                            boilervoG.documentPath = dtraw.Rows[k]["queryRespDocPath1"].ToString();
                            //if (dtraw.Rows[k]["queryRespDocName"].ToString() != "NA")
                            //{
                            //    boilerattachmentvo.documentName = dtraw.Rows[k]["queryRespDocName"].ToString();
                            //}
