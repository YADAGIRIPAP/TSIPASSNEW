...
public partial class UI_TSiPASS_WOMENDATADRILLDOWN : System.Web.UI.Page
{
    DB.DB con = new DB.DB();
    int delete = 0;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (status == "F")
            {
                lblHeading.Text = "Total no of Micro Units for the Financial Year " + financialyear;
            }
            if (status == "G")
            {
                lblHeading.Text = "Total no of Small Units for the Financial Year " + financialyear;
            }
            if (status == "H")
            {
                lblHeading.Text = "Total no of Medium Units for the Financial Year " + financialyear;
            }
            if (status == "I")
            {
                lblHeading.Text = "Total no of Large Units for the Financial Year " + financialyear;
            }
            if (status == "J")
            {
                lblHeading.Text = "Total no of Mega Units for the Financial Year " + financialyear;
            }
    ...
    public DataSet homepagedatadrilldown(string status, string financialyaer, string District, string lineofactivity)
    {
        ...
        try
        {
            da = new SqlDataAdapter("[USP_GET_WOMENENTDASHBOARD_DRILLDOWN]", con.GetConnection);
