...
public partial class UI_TSiPASS_IpassPrintReceiptMobileTower : System.Web.UI.Page
{
                ...
                if (Session["Amount"] != null && Session["RefNo"] != null)
                {
                    ...
                    if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        if (lblMobileNumber.Text != "")
                        {
                            try
                            {
                                obcmf.SendSingleSMSnew(lblMobileNumber.Text, message, "100785677502643850");
    ...
    private DataSet BillPaymentReceipt(string TranRefNo)
    {
        try
        {
            ...
            conn.Open();
            SqlCommand cmd1 = new SqlCommand("GetBilldeskReceipt_MobileTower", conn);
    ...
    public void Webservices(string UIDNO)
    {
        ...
        try
        {
            ...
            if (!IsPostBack)
            {
                ds = GetDepartmentonuidMobileTower(UIDNO);
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    dt = ds.Tables[0];
    ...
    public DataSet GetDepartmentonuidMobileTower(string uidno)
    {
        try
        {
            SqlConnection conn = new SqlConnection(constring);
            conn.Open();
            SqlCommand cmd1 = new SqlCommand("USP_GET_DEPARTMENTS_UID_MobileTower", conn);
            cmd1.CommandType = CommandType.StoredProcedure;
