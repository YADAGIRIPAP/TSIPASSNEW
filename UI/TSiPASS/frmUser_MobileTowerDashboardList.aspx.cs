...
using System.Data;
public partial class UI_TSiPASS_frmUser_MobileTowerDashboardList : System.Web.UI.Page
{
    ...
    public void getdetails()
    {
        try
        {
            ...
            oSqlDataAdapter = new SqlDataAdapter("USP_GET_MobileTower_DASHBOARD_HISTORY", osqlConnection);
            ...
            if (Session["uid"] != null)
            {
                ...
                {
                    Response.Redirect("ApplyAdvertisement.aspx?status=B", false);
                }
    ...
    protected void grdDetails_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            anchortaglink.NavigateUrl = "frmUserMobileTowerDashboard.aspx?Qnreid=" + intApplicationID.Trim();
            anchortaglinkprint.NavigateUrl = "AdvertismentPrint.aspx?Qnreid=" + intApplicationID.Trim();
            anchortaglinkpayment.NavigateUrl = "UserMobileTowerpaymentDetails.aspx?Qnreid=" + intApplicationID.Trim();
            if (Approval_Status == "3")
            {
                anchortaglinkStatus.NavigateUrl = "RptApplicationWiseDetailedTrakerMobileTower.aspx?intqnreid=" + intApplicationID.ToString();
            }
            else if (Approval_Status == "2")
            {
                anchortaglinkStatus.NavigateUrl = "ApplyAdvertisement.aspx?status=B&id=" + intApplicationID.Trim();
    ...
    {
        Response.Redirect("ApplyAdvertisement.aspx?status=A");
    }
