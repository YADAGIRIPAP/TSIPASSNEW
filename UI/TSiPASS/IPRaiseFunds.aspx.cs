...
public partial class IPRaiseFunds : System.Web.UI.Page
{

    //designing and developed by siva as on 27-02-2016

    //Purpose : To update Raise fund Details
    //Tables used : tr_FundsRaising,tbl_WorkMaster
    //Stored procedures Used : UpdFundRaising,sp_getCounties,sp_getPayams,getBomasbyID
    ...
    protected void BtnSave_Click(object sender, EventArgs e)
    {
        ...
        if (cmf.convertDateIndia(txtRaiseDate.Text) < cmf.convertDateIndia(txtApprRejDate.Text))
        {
            lblmsg0.Text = "Bill Invoice Date should be greater than the Approved date. ";
            success.Visible = false;
        ...
        {
            lblmsg0.Text = "Bill Invoice Date should be less than the current date.";
            success.Visible = false;
            Failure.Visible = true;
            return;
        ...
        int i = Gen.UpdFundRaising(hdfID.Value, txtRaiseDate.Text, txtBillAmount.Text, txtBillRemarks.Text, Session["uid"].ToString());
        if (i > 0)
        {
            ...
            {
                SendMailAnotherApprv(hdfTSTEmail.Value, hdfWorkCode.Value, ddlstatus.SelectedValue, hdfTSTName.Value);
            }
    ...
    public void SendMailAnotherApprv(string anothermail, string workcode, string Status,string Name)
    {
        ...
        if (Status.ToString() == "Approved")
        {
            ...
            Messge = "We have Requested invoice for Work code " + workcode.ToString() + " Please verify it.";
        }
        ...
        mail.Body = "Dear " + Name.ToString() + "<br><br>  <H2>TSiPASS TSiPASS MIS - Invoice Request Notification</H2><br> <b> IP NAME: " + ddlIP.SelectedItem.Text + "</b> <br><br> Work Code: " + workcode.ToString() + "<br> <br> Remarks : " + Messge.ToString() + "<br> <br> URL:  <a href=http://203.124.107.65/publicworks target='_blank' > TSiPASS TSiPASS </a> <br> <br> Please Login by clicking the above link.<br><br>Thanks & Regards<br>"+ddlIP.SelectedItem.Text+"";
    ...
    {
        Response.Redirect("IPRaiseFunds.aspx");
