...
public partial class UI_TSiPASS_frmmsme_LTHTGST : System.Web.UI.Page
{
    int delete = 0;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["identityid"].ToString()!="")
            {
    ...
    protected void btnsubmit_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            if (Insertion >= 1)
            {
                ...
                //btnsubmit.Enabled = false;
                Response.Redirect("LTHTGSTDATAPROCESSING.aspx?DISTRICTID=" + ddldistrict.SelectedValue.ToString()  + "&UNITNAME=" + txtUnitName.Text+"&MSMENO="+ ViewState["MSMEID"].ToString()+"&IDENTITYID="+HDNIDENTITYID.Value);
    ...
    public DataSet GETLOABYSECTOR(string SECTOR)
    {
        con.OpenConnection();
        SqlDataAdapter da;
        DataSet ds = new DataSet();
        try
        {
            da = new SqlDataAdapter("GETLineofactivitybysector", con.GetConnection);
            da.SelectCommand.CommandType = CommandType.StoredProcedure;
