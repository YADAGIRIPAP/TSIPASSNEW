...
public partial class TSTBDCReg1 : System.Web.UI.Page
{
    ...
    protected void BtnSave_Click(object sender, EventArgs e)
    {
        ...
        if (BtnSave1.Text == "Submit")
        {
           ...
           // }
            ...
            FMB_File_Name = "";
            string newPath = "";
            string sFileDir = Server.MapPath("~\\TSExporters");
            if (FileUpload1.HasFile)
            {
                ...
                i = Gen.updateProductDescription(Request.QueryString[0].ToString(), FMB_File_Type, FMB_File_Path, FMB_File_Name, Session["uid"].ToString(), txtGeo_Cordinate_Latitude.Text,txtStateName.Text,txtCountries.Text);

                if (i != 999)
                {
                    lblmsg.Text = "Product Details Updated Successfully";
                    Response.Redirect("frmVATExportGM.aspx");
    ...
    void FillDetails()
    {
        ...
        try
        {
           ...
           // ds = Gen.GetdataofApprovaldataAproval(Request.QueryString[0].ToString());
            ...
            ds = Gen.GetGMExportdata(Request.QueryString[0].ToString());
            if (ds.Tables[0].Rows.Count > 0)
            {
                Label447.Text = ds.Tables[0].Rows[0]["Nameofthecompany"].ToString().Trim();
                Label448.Text = ds.Tables[0].Rows[0]["IECNumber"].ToString().Trim();
                Label449.Text = ds.Tables[0].Rows[0]["StatusofExporter"].ToString().Trim();
                Label450.Text = ds.Tables[0].Rows[0]["Commodity"].ToString().Trim();
    ...
    protected void ddlStatus_SelectedIndexChanged1(object sender, EventArgs e)
    {
        if (ddlStatus.SelectedValue.ToString() == "Yes")
        {
            ...
            stateexport.Visible = true;

        }
        else
        {
            stateexport.Visible = false;
    ...
    protected void ddlStatus0_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (ddlStatus0.SelectedValue.ToString() == "Yes")
        {
            ...
            CountryExport.Visible = true;

        }
        else
        {
            CountryExport.Visible = false;
