...
public partial class UI_TSiPASS_RptWellApplicationlistmandalwiseReport : System.Web.UI.Page
{
    Cls_OSGroundWater obj_insert = new Cls_OSGroundWater();
    string Category = ""; string Districtid = ""; string DistrictName = ""; string applicationtype = "";
    string MandalID = ""; string MandalName = "";
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (Request.QueryString.Count > 0)
            {
                ...
                MandalID = Convert.ToString(Request.QueryString["MandalID"].Trim());
                MandalName = Convert.ToString(Request.QueryString["MandalName"].Trim());

            }
            fillgrid(applicationtype, Districtid, MandalID);
            lbl_headtitle.Text = "Well Applications-"+ DistrictName+"-"+ MandalName;
        }
    }


    public void fillgrid(string Typedataofappl, string Districtid,string MandalID)
    {


        DataSet vehicleDS = new DataSet();
        vehicleDS = obj_insert.DB_RptWellApplicationlistmandalwiseReport(Typedataofappl, Districtid, MandalID);
    ...
    protected void ExportToExcel()
    {
        try
        {
            ...
            using (StringWriter sw = new StringWriter())
            {
                ...
                this.fillgrid(applicationtype, Districtid, MandalID);
