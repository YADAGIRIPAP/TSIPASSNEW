...
public partial class UI_TSiPASS_KotakPageForOtherServices : System.Web.UI.Page
{
    CCACrypto ccaCrypto = new CCACrypto();
    string workingKey = "F6A4E673735F2B05CB9A347E944F4989";//"9B383382630D206B91CCF862546AF22A";//put in the 32bit alpha numeric key in the quotes provided here 	
    string ccaRequest = "";// "http://www.localhost:5518";//"http://ipass.telangana.gov.in";
    public string strMerchantId = "185458";
    public string strEncRequest = "";
    public string strAccessCode = "AVCZ02FI27BC93ZCCB";//"AVHE79FG49CL98EHLC";// put the access key in the quotes provided here.
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (Session["UID_no"] != null && Session["onlinetransId"].ToString() != null)
            {
                ...
                dspaydtls = objgen.GetEnterprinerpaymentDtls_OtherServices(CustomerID, TSipassOrderNumber, additionalPayment);
                ...
                if (dspaydtls != null && dspaydtls.Tables.Count > 0 && dspaydtls.Tables[0].Rows.Count > 0)
                {
                    ...
                    billingname.Value = dspaydtls.Tables[0].Rows[0]["INDUSTRYNAME"].ToString();
                    billingaddress.Value = dspaydtls.Tables[0].Rows[0]["ADDRESS"].ToString();
                    billingcity.Value = dspaydtls.Tables[0].Rows[0]["CITY"].ToString();
                    billingstate.Value = "telangana";
                    billingzip.Value = dspaydtls.Tables[0].Rows[0]["PINCODE"].ToString();
                    billingcountry.Value = "India";
                    billingtel.Value = dspaydtls.Tables[0].Rows[0]["MobileNumber"].ToString();
                    billingemail.Value = dspaydtls.Tables[0].Rows[0]["Email"].ToString();
                }
                billingname.Value = dspaydtls.Tables[0].Rows[0]["INDUSTRYNAME"].ToString();
                billingaddress.Value = dspaydtls.Tables[0].Rows[0]["ADDRESS"].ToString();
                billingcity.Value = dspaydtls.Tables[0].Rows[0]["CITY"].ToString();
