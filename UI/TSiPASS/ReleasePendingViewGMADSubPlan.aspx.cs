...
public partial class UI_TSiPASS_ReleasePendingViewGMADSubPlan : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            string distid = Request.QueryString[2].ToString();
            if (distid=="ALL")
            {
               // distid = null;
                distid = "32";
            ...
            if (Session["uid"] != null)
            {
                ds = getReleaseProceedings(Cast, Investmentid, distid);
    ...
    public DataSet getReleaseProceedings(string Cast, string IncetiveID, string distid)
    {
        ...
        try
        {
            da = new SqlDataAdapter("USP_GET_UNIT_INCENTIVEWISE_GMNEW_ADSUBPLAN", con.GetConnection);
