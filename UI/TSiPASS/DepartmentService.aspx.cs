...
//REPLACE(REPLACE(REPLACE( REPLACE((a.filepath+'\'+FileNm),'D:\TS-iPassFinal','~/'),'D:\TestIncentive','https://ipass.telangana.gov.in/'),'G:\TS-iPassFinal','https://ipass.telangana.gov.in/TS-iPASSAttachments'),'\','/')
...
//Scaffold-DbContext "Server=120.138.9.236;Database=EFCORE;User Id=madhuri;Password=Devansh@26;"Microsoft.EntityFrameworkCore.SqlServer -OutputDir Models
//<add name = "TSiPASSSkils" connectionString="Data Source=120.138.9.236;database=TSIPASS;User ID=madhuri;pwd=Devansh@26;Connect Timeout=12000;" providerName="System.Data.SqlClient"/>
//lrefskmlxnoowqtc
//System.ServiceModel.Description.XmlSerializerMessageContractImporter;
//45530	ULTRASEEDTECH
//45533	MOHAMMADJAWEED
//USP_UPDATE_INACTIVEINCENTIVE_APPS
//USP_RELEASEPROC_GETDATA
//slcsplcasepages//MIC0260578107991
//t(Fw#G2x5[8"Ms+Py 2021-12-31 14:46:10.113///10000182//10005262//10003127//10002917//10001622
//Ip@ss@2018
//password@123
//INC20211030134113//USP_UPDATE_KotakSplit_PaymentDtls
//USP_Generate_Certificates//252684
//Telangana declared the No.1 state for Industry and Commerce in the Good Governance Report given by Department of Administrative Reforms and Public Grievances, Govt of India for the year 2020-21
//Configuration.GetSection("ConnectionStrings")[]

//https://localhost:44386/api/TSIPASS/ScrutinyStage?intQuessionaireid=106&intCFEEnterpid=106&UID=ROW7726120220509152110&intDeptid=61&intApprovalid=151&intStageid=5&Querytype=3&Query_Raised_Text=test&AdditionalAmount&additionaldocs&Trans_Date=11/05/2022&Created_by=123&Sysip&Status&Letter
...
// string filepathnew = ds1.Tables[0].Rows[i]["linkNew"].ToString();// LINKNEW .Tables[0].Rows[i][0].ToString();
//string encpassword = Gen.Encrypt(filepathnew, "SYSTIME");
//lab[i].Text += "<tr><td> <a href='CS.aspx?filepathnew=" + encpassword + "' target='_blank' style='color:#FF6600;'>" + ds1.Tables[0].Rows[i][1].ToString() + "</a></td></tr>";


//Username: madhuri
//Password: okyXk0CtB8RpgJdW
//MS SQL WEB 2016
//User: sa
...
public partial class UI_TSiPASS_DepartmentService : System.Web.UI.Page
{
    //WebserviceVO webvo = new WebserviceVO();tsiic
    RWSVO rwsvo = new RWSVO();
    WebserviceVO webvo = new WebserviceVO();

    General gen = new General();
    //WebClient wc = new WebClient();
    TSNPDCLService.TsipassWsService tsnpdcl = new TSNPDCLService.TsipassWsService();//https://kum.karnataka.gov.in/KUM/ShowPDFLetter.aspx
    ...
    FireNew.Service firenew = new FireNew.Service();
    ...
    LabourQueryResponseCFE.TSLabourServiceImplService labourqueryservice = new LabourQueryResponseCFE.TSLabourServiceImplService();
    //ProfessionTaxTest.TSIPASS professiontax = new ProfessionTaxTest.TSIPASS();
    ProfessionTax.TSIPASS professiontax = new ProfessionTax.TSIPASS();
    //LabourService.TSLabourServiceImplService labourservice = new LabourService.TSLabourServiceImplService();

    // LabourTest.TSLabourServiceImplService LabourServiceTest = new LabourTest.TSLabourServiceImplService();
    ...
    TGBPASSLIVE.ApplicationFormResponse objTGBPASS = new TGBPASSLIVE.ApplicationFormResponse();
    TGBPASSLIVE.tgipass objtgipass = new TGBPASSLIVE.tgipass();
    //http://odls.telangana.gov.in/tsipass_eodb.asmx
    //Dear,'Madhuri' Application no '123' Please download this agreement format https://ipass.telangana.gov.in/docs/ReleaseAttachments/T_PRIDE_2015-19_AGR_BOND.pdf and upload the filled up copy(on stamp paper). Assignment letter (on non stamp paper). Only after you submit these documents online, GM-DIC will conduct field inspection to release incentives. Thank you, Commissioner of Industries.
    //Dear,'Madhuri' Application no '123' Please visit https://ipass.telangana.gov.in and login to submit details towards further release of sanctioned incentives. Thank you, Commissioner of Industries.
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        if (txtuidno.Text.Trim() != "")
        {
            ...
            //string UIDNO = Request.QueryString["UIDNO"].ToString();Cm$@1234
            ...
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                                        ...
                                        using (WebResponse response = webRequest.GetResponse())
                                        {
                                            ...
                                            using (Stream responseStream = response.GetResponseStream())
                                            {
                                                ...
                                                if (responseJson.Contains("success"))
                                                {
                                                    ...
                                                    lblmsg.Text = "Permission from Ground Water Department to dig bore well is successful";
                                                }
                                                else
                                                {
                                                    lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " (Permission from Ground Water Department to dig bore well ) Status is: " +
                                                    Convert.ToString(responseJson) + "<br/>";
                    ...
                    if (deptid == "72") //Profession Tax Approval
                    {
                        DataSet dsdept = new DataSet();
                                ...
                                if (dsout1 != null && dsout1.Tables.Count > 0 && dsout1.Tables[0].Rows.Count > 0)
                                {
                                    if (dsout1.Tables[0].Columns.Contains("status"))
                                    {
                                        if (dsout1.Tables[0].Rows[0]["status"].ToString().TrimEnd().TrimStart() == "Success")
                                        {
                                            ...
                                            lblmsg.Text = lblmsg.Text + "For Approval ID: " + deptid + " (Profession Tax Approval) Status is: " + npdclsattachment + "<br/>";
                                            success.Visible = true;
                                        }
                                        else
                                        {
                                            ...
                                            lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " (Profession Tax Approval) Status is: " +
                                                Server.HtmlEncode(output) + "<br/>";
                                            Failure.Visible = true;
                                        }
                                    }
                                }
                    ...
                    if (deptid == "70")//CDMA Water Feasibility
                    {
                                    ...
                                    using (System.IO.Stream s = webRequest.GetResponse().GetResponseStream())
                                    {
                                        using (System.IO.StreamReader sr = new System.IO.StreamReader(s))
                                        {
                                            ...
                                            if (jsonResponse.Contains("Your Transaction  successful"))
                                            {
                                                ...
                                                lblmsg.Text = lblmsg.Text + " For Approval ID: " + deptid + " (CDMA Water Feasibility) Status is: " + jsonResponse + "<br/>";
                                                success.Visible = true;
                                            }
                                            else
                                            {
                                                ...
                                                lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " (CDMA Water Feasibility) Status is: " +
                                                Convert.ToString(jsonResponse) + "<br/>";
                                                Failure.Visible = true;
                                            }
                                        }
                                    }
                    ...
                    if (deptid == "71")
                    {
                        ...
                        if (dsdept != null && dsdept.Tables.Count > 0 && dsdept.Tables[0].Rows.Count > 0)
                        {
                            rwsvo.authorizedPersonName = dsdept.Tables[0].Rows[0]["authorizedPersonName"].ToString();
                            rwsvo.institutionName = dsdept.Tables[0].Rows[0]["institutionName"].ToString();
                            rwsvo.institutionGST = dsdept.Tables[0].Rows[0]["institutionGST"].ToString();
                            rwsvo.institutionEmail = dsdept.Tables[0].Rows[0]["InstitutionEmail"].ToString();
                            //int constituencyId = Convert.ToInt16(dsdept.Tables[0].Rows[0]["constituencyId"].ToString());
                            rwsvo.institutionType = dsdept.Tables[0].Rows[0]["InstitutionType"].ToString();
                            rwsvo.institutionSubType = dsdept.Tables[0].Rows[0]["institutionSubType"].ToString();
                            rwsvo.qualityOfWater = dsdept.Tables[0].Rows[0]["qualityOfWater"].ToString();
                            rwsvo.purposeOfRequest = dsdept.Tables[0].Rows[0]["purposeOfRequest"].ToString();
                            rwsvo.sizeOfTheConnection = dsdept.Tables[0].Rows[0]["sizeOfTheConnection"].ToString();
                            rwsvo.locationOfSupply = dsdept.Tables[0].Rows[0]["locationOfSupply"].ToString();
                            // rwsvo.RequiredQuantity = dsdept.Tables[0].Rows[0]["RequiredQuantity"].ToString();
                            rwsvo.requiredQuantity = dsdept.Tables[0].Rows[0]["RequiredQuantity"].ToString();
                            rwsvo.category = dsdept.Tables[0].Rows[0]["Category"].ToString();
                            rwsvo.district = dsdept.Tables[0].Rows[0]["District"].ToString();
                            //   string districtName = dsdept.Tables[0].Rows[0]["districtName"].ToString();
                            rwsvo.constituency = dsdept.Tables[0].Rows[0]["constituency"].ToString();
                            rwsvo.habitation = dsdept.Tables[0].Rows[0]["habitation"].ToString();
                            rwsvo.landmark = dsdept.Tables[0].Rows[0]["Landmark"].ToString();
                            rwsvo.contactPersonName = dsdept.Tables[0].Rows[0]["ContactPersonName"].ToString();
                            rwsvo.mandal = dsdept.Tables[0].Rows[0]["Mandal"].ToString();
                            // string mandalName = dsdept.Tables[0].Rows[0]["mandalName"].ToString();
                            rwsvo.panchayat = dsdept.Tables[0].Rows[0]["Panchayat"].ToString();
                            rwsvo.contactPersonEmail = dsdept.Tables[0].Rows[0]["ContactPersonEmail"].ToString();
                            rwsvo.contactPersonMobile = dsdept.Tables[0].Rows[0]["ContactPersonMobile"].ToString();
                            rwsvo.addressForcommunication = dsdept.Tables[0].Rows[0]["AddressForCommunication"].ToString();
                            rwsvo.designation = dsdept.Tables[0].Rows[0]["designation"].ToString();
                            rwsvo.connectionCharges = dsdept.Tables[0].Rows[0]["connectionCharges"].ToString();
                            rwsvo.depositCharges = dsdept.Tables[0].Rows[0]["depositCharges"].ToString();
                            rwsvo.monthlyCharges = dsdept.Tables[0].Rows[0]["monthlyCharges"].ToString();
                            rwsvo.status = dsdept.Tables[0].Rows[0]["status"].ToString();
                            rwsvo.source = dsdept.Tables[0].Rows[0]["source"].ToString();
                            rwsvo.uidno = dsdept.Tables[0].Rows[0]["UIDNO"].ToString();
                            rwsvo.intquestionaireid = dsdept.Tables[0].Rows[0]["IntQuestionaireId"].ToString();
                            rwsvo.intcfeid = dsdept.Tables[0].Rows[0]["IntCfeId"].ToString();
                            rwsvo.panUpload = dsdept.Tables[0].Rows[0]["panUpload"].ToString();
                            //rwsvo.PanUploadData = dsdept.Tables[0].Rows[0]["PanUploadData"].ToString();
                            byte[] bytesToEncodepan = Encoding.UTF8.GetBytes(dsdept.Tables[0].Rows[0]["PanUploadData"].ToString());
                            rwsvo.panUploadData = Convert.ToBase64String(bytesToEncodepan);
                            rwsvo.gstUpload = dsdept.Tables[0].Rows[0]["GstUpload"].ToString();
                            //  rwsvo.GstUploadData = dsdept.Tables[0].Rows[0]["GstUploadData"].ToString();
                            byte[] bytesToEncodeGst = Encoding.UTF8.GetBytes(dsdept.Tables[0].Rows[0]["GstUploadData"].ToString());
                            rwsvo.gstUploadData = Convert.ToBase64String(bytesToEncodeGst);
                            rwsvo.aadharUpload = dsdept.Tables[0].Rows[0]["AadharUpload"].ToString();
                            //  rwsvo.AadharUploadData = dsdept.Tables[0].Rows[0]["AadharUploadData"].ToString();
                            byte[] bytesToEncodeAadhar = Encoding.UTF8.GetBytes(dsdept.Tables[0].Rows[0]["AadharUploadData"].ToString());
                            rwsvo.aadharUploadData = Convert.ToBase64String(bytesToEncodeAadhar);
                            rwsvo.otherDocUpload = dsdept.Tables[0].Rows[0]["OtherDocUpload"].ToString();
                            rwsvo.otherDocUploadData = dsdept.Tables[0].Rows[0]["otherDocUploadData"].ToString();
                            rwsvo.ownershipDocUpload = dsdept.Tables[0].Rows[0]["OwnershipDocUpload"].ToString();
                            //  rwsvo.OwnershipDocUploadData = dsdept.Tables[0].Rows[0]["OwnershipDocUploadData"].ToString();
                            byte[] bytesToEncodeOwnership = Encoding.UTF8.GetBytes(dsdept.Tables[0].Rows[0]["OwnershipDocUploadData"].ToString());
                            rwsvo.ownershipDocUploadData = Convert.ToBase64String(bytesToEncodeOwnership);

                            rwsvo.selfDeclerationUpload = dsdept.Tables[0].Rows[0]["SelfDeclerationUpload"].ToString();
                            // rwsvo.SelfDeclarationUploadData = dsdept.Tables[0].Rows[0]["SelfDeclarationUploadData"].ToString();
                            byte[] bytesToEncodeSelfDeclaration = Encoding.UTF8.GetBytes(dsdept.Tables[0].Rows[0]["SelfDeclerationUploadData"].ToString());
                            rwsvo.selfDeclerationUploadData = Convert.ToBase64String(bytesToEncodeSelfDeclaration);

                            rwsvo.address_proofUpload = dsdept.Tables[0].Rows[0]["address_proofUpload"].ToString();
                            // rwsvo.AddressProofUploadData = dsdept.Tables[0].Rows[0]["AddressProofUploadData"].ToString();
                            byte[] bytesToEncodeAddressProof = Encoding.UTF8.GetBytes(dsdept.Tables[0].Rows[0]["address_proofUploadData"].ToString());
                            rwsvo.address_proofUploadData = Convert.ToBase64String(bytesToEncodeAddressProof);



                            var JsonFrimDetails = JsonConvert.SerializeObject(rwsvo);
                            ServicePointManager.SecurityProtocol = (SecurityProtocolType)48 | (SecurityProtocolType)192 | (SecurityProtocolType)768 | (SecurityProtocolType)3072;
                            // string WEBSERVICE_URL = "https://staging.mbmis.telangana.gov.in/mbbulk/newconnectionService/ipass_SavenewConnection";
                            string WEBSERVICE_URL = "https://staging.mbmis.telangana.gov.in/mbbulk/newconnectionService/ipass_SavenewConnection";
                            //"https://staging.missionbhagiratha.telangana.gov.in/mbbulk/newconnectionService/ipass_SavenewConnection";
                            //https://staging.mbmis.telangana.gov.in/mbbulk/newconnectionService/ipass_SavenewConnection
                            try
                            {
                                var webRequest = System.Net.WebRequest.Create(WEBSERVICE_URL);
                                if (webRequest != null)
                                        ...
                                        using (WebResponse response = webRequest.GetResponse())
                                        {
                                            ...
                                            using (Stream responseStream = response.GetResponseStream())
                                            {
                                                ...
                                                if (responseJson.Contains("success"))
                                                {
                                                    ...
                                                    lblmsg.Text = lblmsg.Text + " For Approval ID: " + deptid + "  (RWS Water Feasibility) Status is: " + Convert.ToString(responseJson) + "<br/>";
                                                    success.Visible = true;
                                                }
                                                else
                                                {
                                                    lblmsg0.Text = lblmsg0.Text + " For Approval ID: " + deptid + " (RWS Water Feasibility) Status is: " + Convert.ToString(responseJson) + "<br/>";
                                                    Failure.Visible = true;
                                                }
                                            }
                                        }
                    ...
                    if (deptid == "65") //Electrical Drawing approval from Electrical Inspectorate
                    {
                        ...
                        if (dsdept != null && dsdept.Tables.Count > 0 && dsdept.Tables[0].Rows.Count > 0)
                        {
                            ...
                            if (ceigout == "SUCCESS")
                            {
                                ...
                                lblmsg.Text = lblmsg.Text + "For Approval ID: " + deptid + " (Electrical Drawing approval from Electrical Inspectorate) Status is: " + ceigout + "<br/>";
                                success.Visible = true;
                            }
                            else
                            {
                                gen.UpdateDepartwebserviceflag(UIDNO, deptid, "C", ceigout, "N");
                                lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " (Electrical Drawing approval from Electrical Inspectorate) Status is: " + ceigout + "and XML Data is: " +
                                     Server.HtmlEncode(attcvalueshmwssb) + "<br/>";
                                Failure.Visible = true;
                            }
                        }
                    }

                    if (deptid == "20")//Consent For Establishment from Pollutuion Control Board 
                    {
                        DataSet dsdept = new DataSet();
                        ...
                        if (dsdept != null && dsdept.Tables.Count > 0 && dsdept.Tables[0].Rows.Count > 0)
                        {
                                    ...
                                    using (System.IO.Stream s = webRequest.GetResponse().GetResponseStream())
                                    {
                                        using (System.IO.StreamReader sr = new System.IO.StreamReader(s))
                                        {
                                            ...
                                            if (jsonResponse.Contains("Fee submitted successfully"))
                                            {
                                                gen.UpdateDepartwebserviceflag(UIDNO, deptid, "C", jsonResponse, "Y");
                                                int k = gen.InsertDeptDateTracing("20", dsdept.Tables[0].Rows[0]["intQuessionaireid"].ToString(), UIDNO, System.DateTime.Now.ToString("MM/dd/yyyy"), null, null, null, null, "CFE", "20");
                                                lblmsg.Text = lblmsg.Text + "For Approval ID: " + deptid + " (Consent For Establishment from Pollutuion Control Board ) Status is: " + Convert.ToString(jsonResponse) + "<br/>";
                                                success.Visible = true;
                                            }
                                            else
                                            {
                                                //string labourouterrormsg = labourout.status + ',' + labourout.uID + ',' + labourout.officerDetails;
                                                gen.UpdateDepartwebserviceflag(UIDNO, deptid, "C", jsonResponse, "N");
                                                lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " (Consent For Establishment from Pollutuion Control Board) Status is: " + Convert.ToString(jsonResponse) + "<br/>";
                                                Failure.Visible = true;
                                            }
                                        }
                                    }
                            ...
                            {
                                lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " (Consent For Establishment from Pollutuion Control Board) Status is: " + Convert.ToString(ex.ToString()) + "<br/>";
                                Failure.Visible = true;
                                System.Diagnostics.Debug.WriteLine(ex.ToString());
                            }
                    ...
                    if (deptid == "31")//Industrial Building Plan Approval from IALA/TSIIC
                    {
                        try
                        {
                            DataSet dsdept = new DataSet();
                            ...
                            if (dsdept != null && dsdept.Tables.Count > 0 && dsdept.Tables[0].Rows.Count > 0)
                            {
                                ...
                                if (tsiicapplication.Error.ToString().ToLower().Contains("file submitted"))
                                {
                                    ...
                                    if (tsiicapplication.FileNo != "" && tsiicapplication.FileNo != null
                                        && tsiicapplication.Error == "File Attached successfully")
                                    {
                                        ...
                                        lblmsg.Text = lblmsg.Text + "For Approval ID: " + deptid + " Status is: " + tsiicapplication.Error + "<br/>";
                                        success.Visible = true;
                                    }
                                    else
                                    {
                                        ...
                                        lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " (Industrial Building Plan Approval from IALA/TSIIC) Status is: " +
                                            tsiicapplication.Error + " & XML Data is:" + Server.HtmlEncode(valueshmda) + "<br/>";
                                        Failure.Visible = true;
                                    }
                        ...
                        try
                        {
                            ...
                            if (dsdept != null && dsdept.Tables.Count > 0 && dsdept.Tables[0].Rows.Count > 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " (Industrial Building Plan Approval from DT&CP) Status is: " +
                                           objTGBPASS.Error + " & XML Data is: " + Server.HtmlEncode(valueshmda) + "<br/>";
                                    Failure.Visible = true;
                                }
                            }
                        }
                    ...
                    if (deptid == "35")//Industrial Building Plan Approval from HMDA
                    {
                        try
                        {
                            DataSet dsdept = new DataSet();
                            ...
                            if (dsdept != null && dsdept.Tables.Count > 0 && dsdept.Tables[0].Rows.Count > 0)
                            {
                                ...
                                hmdapplication = hmdaservice.create(valueshmda, "$$08@213");//000844/I1/U6/HMDA/25072017
                                                                                            //Kindly contact to Administrator regarding post mapping configuration for revenue department.
                                DataSet dsdeptattachmentsHMDA = new DataSet();
                                dsdeptattachmentsHMDA = gen.getattachmentdetailsonuid(UIDNO, deptid, hmdapplication.FileNo);//000841/I1/U6/HMDA/23072017//000842/I1/U6/HMDA/23072017

                                //Kindly contact to administrator regarding add work flow.
                                string hmdaattachments = dsdeptattachmentsHMDA.GetXml();
                                hmdaattachments = hmdaattachments.Replace("&amp;", "&");
                                ...
                                if (hmdapplication.FileNo != "" && hmdapplication.FileNo != null)
                                {
                                    ...
                                    lblmsg.Text = lblmsg.Text + "For Approval ID: " + deptid + " (Industrial Building Plan Approval from HMDA) Status is: " + hmdapplication.Error + "<br/>";
                                    success.Visible = true;
                                }
                                else
                                {
                                    //string labourouterrormsg = labourout.status + ',' + labourout.uID + ',' + labourout.officerDetails;
                                    //gen.UpdateDepartwebserviceflag(UIDNO, deptid, "C", hmdapplication.ErrorMessage, "N");
                                    lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " (Industrial Building Plan Approval from HMDA) Status is: " +
                                          hmdapplication.Error + " & XML Data is:" + Server.HtmlEncode(valueshmda) + "<br/>";
                                    Failure.Visible = true;
                                }
                            }
                        }
                    ...
                    if (deptid == "10") //Permission from Hyderabad Metropolitan Water Supply & Sewerage Board(HMWS&SB) 
                    {
                        try
                        {
                            DataSet dsdept = new DataSet();
                            ...
                            if (dsdept != null && dsdept.Tables.Count > 0 && dsdept.Tables[0].Rows.Count > 0)
                            {
                                ...
                                webvo.intQuessionaireid = Convert.ToInt32(dsdept.Tables[0].Rows[0]["intQuessionaireid"].ToString());
                                webvo.intEnterpreneurid = Convert.ToInt32(dsdept.Tables[0].Rows[0]["intCFEEnterpid"].ToString());
                                webvo.NameofthePromoter = dsdept.Tables[0].Rows[0]["NameofthePromoter"].ToString();
                                webvo.Door_No = dsdept.Tables[0].Rows[0]["DOOR_NO"].ToString().Replace("&", "");
                                ...
                                else
                                {
                                    ...
                                    lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " (Permission from Hyderabad Metropolitan Water Supply & Sewerage Board(HMWS&SB) ) Status is: " +
                                          outputhmwssb + " &  Data is:" + Server.HtmlEncode(valueshmwssb) + "<br/>";
                                    Failure.Visible = true;
                                }
                            }
                        }
                    ...
                    if (deptid == "34") // NALA Conversion from revenue department
                    {
                        try
                        {
                            DataSet dsdept = new DataSet();
      ...
      webvo.ServiceCharge, webvo.UserCharge, webvo.PostalCharge, webvo.TotalAmount, webvo.GridDetails, webvo.QuestionaireId, webvo.EntrepreneurId, webvo.docapplicationform, webvo.landrelated, webvo.certificateform, webvo.idproof);
                                ...
                                if (dsout != null && dsout.Tables.Count > 0 && dsout.Tables[0].Rows.Count > 0)
                                {
                                    if (dsout.Tables[0].Columns.Contains("ResponseCode"))
                                    {
                                        if (dsout.Tables[0].Rows[0]["ResponseCode"].ToString() == "100")
                                        {
                                            ...
                                            lblmsg.Text = lblmsg.Text + "For Approval ID: " + deptid + " Status is: " + dsout.Tables[0].Rows[0]["ResponseDesc"].ToString() + "<br/>";
                                            success.Visible = true;
                                        }
                                        else
                                        {
                                            ...
                                            lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " (NALA Conversion from revenue department) Status is: " +
                                            dsout.Tables[0].Rows[0]["ResponseDesc"].ToString() + "<br/>" + "XML Data is: " + Server.HtmlEncode(valuenala);
                                            Failure.Visible = true;
                                        }
                                    }
                                }
                    ...
                    if (deptid == "47") //Registration Under Buildings & Other Construction Workers (RE & COS) Act, 1996 As Required
                    {
                        try
                        {
                            DataSet dsdept = new DataSet();
                            ...
                            if (dsdept != null && dsdept.Tables.Count > 0 && dsdept.Tables[0].Rows.Count > 0)
                            {
                                ...
                                if (actids.Contains("2"))//The Buildings & Other Construction Workers Act
                                {
                                    ...
                                    if (labourout.status == "SUCCESS")
                                    {
                                        ...
                                        lblmsg.Text = lblmsg.Text + "For Approval ID: " + deptid + " Status is: " + labourout + "<br/>";
                                        success.Visible = true;
                                    }
                                    else
                                    {
                                        ...
                                        lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " (Registration Under Buildings & Other Construction Workers (RE & COS) Act, 1996 As Required) Status is: " +
                                           labourout.status + "<br/>";
                                        Failure.Visible = true;
                                    }
                                }
                    ...
                    if (deptid == "48") //Registration Under Contract Labour(Regulation and Abolition) Act, 1970 
                    {
                        try
                        {
                            DataSet dsdept = new DataSet();
                            ...
                            string labour48xml = dsdept.GetXml();
                            LabourService.act labouract = new LabourService.act();
                            LabourService.actsResponse labourout = new LabourService.actsResponse();
                            LabourService.contractLabourPrincipalEmployer contractvo = new LabourService.contractLabourPrincipalEmployer();
                            LabourService.buildingotherconstructions labour = new LabourService.buildingotherconstructions();
                            ...
                            if (dsdept != null && dsdept.Tables.Count > 0 && dsdept.Tables[0].Rows.Count > 0)
                            {
                                ...
                                if (actids.Contains("3"))//The Contract Labour Act (Principal Employer)
                                {
                                    ...
                                    contractvo.establishment_name = dsdept.Tables[0].Rows[0]["Form1_Estb_Name"].ToString().Replace("'", "");
                                    ...
                                    contractvo.nature_work = dsdept.Tables[0].Rows[0]["Form1_Nature_of_Business"].ToString().Replace("'", "");
                                    ...
                                    if (labourout.status == "SUCCESS")
                                    {
                                        ...
                                        lblmsg.Text = lblmsg.Text + "For Approval ID: " + deptid + " (Registration Under Contract Labour(Regulation and Abolition) Act, 1970 )Status is: " + labourout + "<br/>";
                                        success.Visible = true;
                                    }
                                    else
                                    {
                                        string labourouterrormsg = labourout.status + ',' + labourout.uID + ',' + labourout.officerDetails;
                                        gen.UpdateDepartwebserviceflag(UIDNO, deptid, "C", labourouterrormsg, "N");
                                        lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " (Registration Under Contract Labour(Regulation and Abolition) Act, 1970 ) Status is: " +
                                          labourout.status + "<br/>";
                                        Failure.Visible = true;
                                    }
                                }
                    ...
                    if (deptid == "49") //Registration Under Inter State Migrant Workman Act, 1979
                    {
                        try
                        {
                            DataSet dsdept = new DataSet();
                            ...
                            if (dsdept != null && dsdept.Tables.Count > 0 && dsdept.Tables[0].Rows.Count > 0)
                            {
                                ...
                                if (actids.Contains("4"))//Principal Employer Registration Under InterState Migrant Workman Act
                                {
                                    ...
                                    else
                                    {
                                        string labourouterrormsg = labourout.status + ',' + labourout.uID + ',' + labourout.officerDetails;
                                        gen.UpdateDepartwebserviceflag(UIDNO, deptid, "C", labourouterrormsg, "N");
                                        lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " (Registration Under Inter State Migrant Workman Act, 1979) Status is: " +
                                          labourout.status + "<br/>";
                                        Failure.Visible = true;
                                    }
                                }
                    ...
                    if (deptid == "58") //Registration Under Inter State Migrant Workman Act, 1979(for Contractor)
                    {
                        try
                        {
                            DataSet dsdept = new DataSet();
                            ...
                            if (dsdept != null && dsdept.Tables.Count > 0 && dsdept.Tables[0].Rows.Count > 0)
                            {
                                ...
                                if (actids.Contains("5"))//Principal Employer Registration Under InterState Migrant Workman Act
                                {
                                    ...
                                    else
                                    {
                                        string labourouterrormsg = labourout.status + ',' + labourout.uID + ',' + labourout.officerDetails;
                                        gen.UpdateDepartwebserviceflag(UIDNO, deptid, "C", labourouterrormsg, "N");
                                        lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " (Registration Under Inter State Migrant Workman Act, 1979(for Contractor)) Status is: " +
                                          labourout.status + "<br/>";
                                        Failure.Visible = true;
                                    }
                                }
                    ...
                    if (deptid == "8")// Provisional NOC from Fire Services Department
                    {
                        try
                        {
                            ServicePointManager.Expect100Continue = true;
                            ...
                            //if(split[0]=="Success" && outputfirepayment == "Success")
                            {
                                ...
                                lblmsg.Text = lblmsg.Text + "For Approval ID: " + deptid + " (Provisional NOC from Fire Services Department) Status is: " + outapplicant + "<br/>";
                                success.Visible = true;
                            ...
                            else
                            {
                                ...
                                lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " (Provisional NOC from Fire Services Department) Status is: " +
                                          outapplicant + "<br/>";
                                Failure.Visible = true;
                            }
                        }
                    ...
                    if (deptid == "4" || deptid == "41")//Service Connection Certificate from TSNPDCL
                    {
                        try
                        {
                            string valueNPDCL = "";
                            ...
                            if (dsout != null && dsout.Tables.Count > 0 && dsout.Tables[0].Rows.Count > 0)
                            {
                                if (dsout.Tables[0].Columns.Contains("status"))
                                {
                                    if (dsout.Tables[0].Rows[0]["status"].ToString() == "S")
                                    {
                                        ...
                                        lblmsg.Text = lblmsg.Text + "For Approval ID: " + deptid + " Status is: " +
                                         dsout.Tables[0].Rows[0]["status"].ToString() + "<br/>";
                                        success.Visible = true;
                                    }
                                    else
                                    {
                                        ...
                                        lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " Status is: " +
                                         dsout.Tables[0].Rows[0]["status"].ToString() + "<br/>";
                                        Failure.Visible = true;
                                    }
                                }
                            ...
                            if (dsout1 != null && dsout1.Tables.Count > 0 && dsout1.Tables[0].Rows.Count > 0)
                            {
                                if (dsout1.Tables[0].Columns.Contains("status"))
                                {
                                    if (dsout1.Tables[0].Rows[0]["status"].ToString() == "S")
                                    {
                                        ...
                                        lblmsg.Text = lblmsg.Text + "For Approval ID: " + deptid + " Status is: " +
                                            dsout1.Tables[0].Rows[0]["status"].ToString() + "<br/>";
                                        success.Visible = true;
                                    }
                                    else
                                    {
                                        ...
                                        lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " ( Service Connection Certificate from TSNPDCL) Status is: " +
                                        dsout1.Tables[0].Rows[0]["status"].ToString() + "<br/>";
                                        Failure.Visible = true;
                                    }
                                }
                    ...
                    if (deptid == "25") //Service Connection Certificate from TSSPDCL
                    {
                        try
                        {
                            DataSet dsdept = new DataSet();
                            ...
                            if (dsout != null && dsout.Tables.Count > 0 && dsout.Tables[0].Rows.Count > 0)
                            {
                                if (dsout.Tables[0].Columns.Contains("RESULT"))
                                {
                                    if (dsout.Tables[0].Rows[0]["RESULT"].ToString() == "Success")
                                    {
                                        ...
                                        lblmsg.Text = lblmsg.Text + "For Approval ID: " + deptid + " Status is: " + dsout + "<br/>";
                                        success.Visible = true;
                                    }
                                    else
                                    {
                                        ...
                                        lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " (Service Connection Certificate from TSSPDCL) Status is: " +
                                        dsout.Tables[0].Rows[0]["RESULT"].ToString() + "<br/>";
                                        Failure.Visible = true;
                                    }
                                }
                            }
                            ...
                            {
                                lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " (Service Connection Certificate from TSSPDCL)"+"Output Error is:"
                                    + outputspdcl+"and XML Data is: " +
                                           valuesPDCL + "<br/>";
                                Failure.Visible = true;
                            }
                        }
                        ...
                        try
                        {
                            ...
                            if (dsdept != null && dsdept.Tables.Count > 0 && dsdept.Tables[0].Rows.Count > 0)
                            {
                                ...
                                if (OUTPUT == "SUCCESS")
                                {
                                    ...
                                    int k = gen.InsertDeptDateTracing(dsdept.Tables[0].Rows[0]["DEPTID"].ToString(), dsdept.Tables[0].Rows[0]["intQuessionaireid"].ToString(), UIDNO, System.DateTime.Now.ToString("MM/dd/yyyy"), null, null, null, null, "CFE", deptid);
                                    lblmsg.Text = lblmsg.Text + "For Approval ID: " + deptid + " Status is: " + OUTPUT + "<br/>";
                                    success.Visible = true;
                                }
                                else
                                {
                                    gen.UpdateDepartwebserviceflag(UIDNO, deptid, "C", OUTPUT, "N");
                                    lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " (Factory plan approval) Status is: " +
                                       OUTPUT + "<br/>";
                                    Failure.Visible = true;
                                }
                            }
                    ...
                    if (deptid == "66")// License Under Contract Labour Act(Contractor)
                    {
                        //try
                        //{
                        //    DataSet dsdept = new DataSet();
                        ...
                        //    LabourService.act labouracttest = new LabourService.act();
                        //    LabourService.actsResponse labourouttest = new LabourService.actsResponse();
                        //    LabourService.contractLabourContractorRegistration Contractorvo = new LabourService.contractLabourContractorRegistration();


                        //    //LabourServiceTest.contractLabourContractorActData  Contractorvo = new LabourServiceTest.contractLabourContractorActData();
                        //    string actids = "";
                        //    string actid = "";
                        //    if (dsdept != null && dsdept.Tables.Count > 0 && dsdept.Tables[0].Rows.Count > 0)
                        //    {
                        ...
                        //        if (actids.Contains("4"))//Principal Employer Registration Under InterState Migrant Workman Act
                        //        {
                        //            Contractorvo.actID = "";//dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.bankName = dsdept.Tables[0].Rows[0]["BankName"].ToString();
                        //            Contractorvo.bank_code = dsdept.Tables[0].Rows[0]["bank_code"].ToString();
                        //            Contractorvo.bank_ref_number = dsdept.Tables[0].Rows[0]["OnlineOrderNo"].ToString();
                        //            Contractorvo.cin = dsdept.Tables[0].Rows[0]["cin"].ToString();
                        //            Contractorvo.compound_fee = 0;// Convert.ToInt32(dsdept.Tables[0].Rows[0][""].ToString());
                        //            Contractorvo.contractor_commit_offence = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.contractor_email = dsdept.Tables[0].Rows[0]["Form_1_5_contr_Email"].ToString();
                        //            Contractorvo.contractor_fathername = dsdept.Tables[0].Rows[0]["Form_1_5_contr_Fname"].ToString();
                        //            Contractorvo.contractor_licence_revoke = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.contractor_licence_revoke_dt = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.contractor_mobile = dsdept.Tables[0].Rows[0]["Form_1_5_contr_Mobile"].ToString();
                        //            Contractorvo.contractor_offence_details = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.contractor_other_work = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.contractor_other_work_details = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.date_commencement = dsdept.Tables[0].Rows[0]["Form1_1_DateofCommencement"].ToString();
                        //            Contractorvo.date_ending = dsdept.Tables[0].Rows[0]["Form1_2_Est_Compltd_Dt"].ToString();
                        //            Contractorvo.entrepreneur_id = dsdept.Tables[0].Rows[0]["intCFEEnterpid"].ToString();
                        //            Contractorvo.establishment_category = dsdept.Tables[0].Rows[0]["Form1_1_Estb_Category"].ToString();
                        //            Contractorvo.establishment_category_others = dsdept.Tables[0].Rows[0]["Form1_1_Estb_Classification"].ToString();
                        //            Contractorvo.establishment_name = dsdept.Tables[0].Rows[0]["Form1_Estb_Name"].ToString();
                        //            Contractorvo.establishment_otherstate_address = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.establishment_postal_district = dsdept.Tables[0].Rows[0]["Form1_Estd_District"].ToString();
                        //            Contractorvo.establishment_postal_door = dsdept.Tables[0].Rows[0]["Form1_Estd_DoorNo"].ToString();
                        //            Contractorvo.establishment_postal_locality = dsdept.Tables[0].Rows[0]["Form1_Estd_Locality"].ToString();
                        //            Contractorvo.establishment_postal_mandal = dsdept.Tables[0].Rows[0]["Form1_Estd_Mandal"].ToString();
                        //            Contractorvo.establishment_postal_pincode = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.establishment_postal_village = dsdept.Tables[0].Rows[0]["Form1_Estd_Village"].ToString();
                        //            Contractorvo.individual_age = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.individual_dob = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.manager_agent_address = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.manager_agent_district = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.manager_agent_door = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.manager_agent_locality = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.manager_agent_mandal = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.manager_agent_name = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.manager_agent_pincode = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.manager_agent_village = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.max_employees_aday = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.nature_work = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.other_attachments_1 = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.other_attachments_2 = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.paymentFlag = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.payment_mode = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.penality_percentage = Convert.ToInt32(dsdept.Tables[0].Rows[0][""].ToString());
                        //            Contractorvo.penality_years = Convert.ToInt32(dsdept.Tables[0].Rows[0][""].ToString());
                        //            Contractorvo.previous_registered_act = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.previous_registration_certificate = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.previous_registration_no = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.principal_employees = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.principal_employer_certificate = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.principal_employer_certificate_dt = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.principal_employer_certificate_no = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.principal_employer_district = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.principal_employer_door = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.principal_employer_mandal = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.principal_employer_name = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.principal_employer_otherstate = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.principal_employer_pincode = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.principal_employer_village = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.projectSubmitDate = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.questionnaire_id = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.registration_fee = Convert.ToInt32(dsdept.Tables[0].Rows[0][""].ToString());
                        //            Contractorvo.registration_years = Convert.ToInt32(dsdept.Tables[0].Rows[0][""].ToString());
                        //            Contractorvo.security_deposit_amount = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.security_deposit_challan = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.security_deposit_challan_date = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.security_deposit_challan_no = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.stageID = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.totalAmount = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.total_amount_payable = Convert.ToInt32(dsdept.Tables[0].Rows[0][""].ToString());
                        //            Contractorvo.total_penality_amount = Convert.ToInt32(dsdept.Tables[0].Rows[0][""].ToString());
                        //            Contractorvo.total_registration_fee = Convert.ToInt32(dsdept.Tables[0].Rows[0][""].ToString());
                        //            Contractorvo.transactionDate = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.transactionNumber = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.transaction_for = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.transaction_id = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.transaction_status = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.type_of_application = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.uID = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.unpaid_balance_welfare = Convert.ToInt32(dsdept.Tables[0].Rows[0][""].ToString());
                        //            Contractorvo.valid_from_date = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.valid_to_date = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.worksite_district = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.worksite_door = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.worksite_locality = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.worksite_mandal = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.worksite_name = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.worksite_pincode = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.worksite_village = dsdept.Tables[0].Rows[0][""].ToString();
                        //            Contractorvo.zones_selected = dsdept.Tables[0].Rows[0][""].ToString();


                        //            labouracttest.contractLabourContractorActData = Contractorvo;
                        //            //labourout = LabourServicetest.actSelected(labouract);
                        //            labourouttest = LabourServiceTest.actSelected(labouracttest);

                        //            if (labourouttest.status == "SUCCESS")
                        //            {
                        //                string labouroutmsg = labourouttest.status + ',' + labourouttest.uID + ',' + labourouttest.officerDetails;
                        //                gen.UpdateDepartwebserviceflag(UIDNO, deptid, "C", labouroutmsg, "Y");
                        //                int k = gen.InsertDeptDateTracing(dsdept.Tables[0].Rows[0]["DEPTID"].ToString(), dsdept.Tables[0].Rows[0]["intQuessionaireid"].ToString(), Contractorvo.uID, System.DateTime.Now.ToString("MM/dd/yyyy"), null, null, null, null, "CFE", deptid);
                        //            }
                        //            else
                        //            {
                        //                string labourouterrormsg = labourouttest.status + ',' + labourouttest.uID + ',' + labourouttest.officerDetails;
                        //                gen.UpdateDepartwebserviceflag(UIDNO, deptid, "C", labourouterrormsg, "N");
                        //            }
                        //        }
                        //    }
                ...
                for (int i = 0; i < dtadd.Rows.Count; i++)
                {
                        ...
                        try
                        {
                            ...
                            if (dsdept != null && dsdept.Tables.Count > 0 && dsdept.Tables[0].Rows.Count > 0)
                            {
                                ...
                                if (outceigpayment == "SUCCESS")
                                {
                                    ...
                                    lblmsg.Text = lblmsg.Text + "For Approval ID: " + deptid + " Status is: " + outceigpayment + "<br/>";
                                    success.Visible = true;
                                }
                                else
                                {
                                    gen.UpdateDepartwebserviceflag(UIDNO, deptid, "AP", outceigpayment, "N");
                                    lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " (Electrical Drawing approval from Electrical Inspectorate) Status is: " + outceigpayment + "<br/>";
                                    Failure.Visible = true;
                                }
                            }
                        }
                    ...
                    if (deptid == "35")
                    {
                        try
                        {
                            ...
                            if (hmdapplication.ResponseStatus != 0 && hmdapplication.ResponseStatus != 0)
                            {
                                ...
                                lblmsg.Text = lblmsg.Text + "For Approval ID: " + deptid + " Status is: " +
                                    hmdapplication.Error + hmdapplication.ErrorMessage + "<br/>";
                                success.Visible = true;
                            ...
                            else
                            {
                                ...
                                lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " (Industrial Building Plan Approval from HMDA) Status is: "
                                    + hmdapplication.Error + hmdapplication.ErrorMessage + "<br/>";
                                lblmsg0.Text = lblmsg0.Text + "XML Data is: " + Server.HtmlEncode(hmdapayment);
                                Failure.Visible = true;
                            }
                    ...
                    if (deptid == "7")
                    {
                        try
                        {
                            ...
                            ServicePointManager.SecurityProtocol = (SecurityProtocolType)48 | (SecurityProtocolType)192 | (SecurityProtocolType)768 | (SecurityProtocolType)3072;
                            objTGBPASS = objtgipass.Mortgage(hmdapayment, "$$08@213");
                            if (objTGBPASS.ResponseStatus != 0 && objTGBPASS.ResponseStatus != 0)
                            {
                                //string labouroutmsg = labourout.status + ',' + labourout.uID + ',' + labourout.officerDetails;
                                gen.UpdateDepartwebserviceflag(UIDNO, deptid, "M", objTGBPASS.FileNo, "Y");
                                //int k = gen.InsertDeptDateTracing(deptid, dsdept.Tables[0].Rows[0]["intQuessionaireid"].ToString(), UIDNO, System.DateTime.Now.ToString("MM/dd/yyyy"), null, null, null, null, "CFE");
                            }
                            else
                            {
                                //string labourouterrormsg = labourout.status + ',' + labourout.uID + ',' + labourout.officerDetails;
                                gen.UpdateDepartwebserviceflag(UIDNO, deptid, "M", objTGBPASS.ErrorMessage, "N");
                        ...
                        try
                        {
                            ...
                            string hmdapayment = dsdept.GetXml();
                            ServicePointManager.SecurityProtocol = (SecurityProtocolType)48 | (SecurityProtocolType)192 | (SecurityProtocolType)768 | (SecurityProtocolType)3072;
                            objTGBPASS = objtgipass.DCPayment(hmdapayment, "$$08@213");
                            if (objTGBPASS.ResponseStatus != 0 && objTGBPASS.ResponseStatus != 0)
                            {
                                //string labouroutmsg = labourout.status + ',' + labourout.uID + ',' + labourout.officerDetails;
                                gen.UpdateDepartwebserviceflag(UIDNO, deptid, "AP", objTGBPASS.FileNo, "Y");
                                //int k = gen.InsertDeptDateTracing(deptid, dsdept.Tables[0].Rows[0]["intQuessionaireid"].ToString(), UIDNO, System.DateTime.Now.ToString("MM/dd/yyyy"), null, null, null, null, "CFE");
                            }
                            else
                            {
                    ...
                    if (deptid == "31")
                    {
                        try
                        {
                            ...
                            else
                            {
                                ...
                                lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " (Industrial Building Plan Approval from IALA/TSIIC) Status is: " +
                                 hmdapplication.Error + hmdapplication.ErrorMessage + "<br/>";
                                lblmsg0.Text = lblmsg0.Text + "XML Data is: " + Server.HtmlEncode(dtcppayment);
                                Failure.Visible = true;
                            }
                        ...
                        try
                        {
                            ...
                            if (tsiicapplication.ResponseStatus != 0 && tsiicapplication.ResponseStatus != 0)
                            {
                                ...
                                //int k = gen.InsertDeptDateTracing(deptid, dsdept.Tables[0].Rows[0]["intQuessionaireid"].ToString(), UIDNO, System.DateTime.Now.ToString("MM/dd/yyyy"), null, null, null, null, "CFE");
                                lblmsg.Text = lblmsg.Text + "For Approval ID: " + deptid + " Status is: " +
                                                             tsiicapplication.Error + tsiicapplication.ErrorMessage + "<br/>";
                                success.Visible = true;
                            }
                            else
                            {
                                ...
                                lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " Status is: " +
                                     tsiicapplication.Error + tsiicapplication.ErrorMessage + "<br/>";
                                lblmsg0.Text = lblmsg0.Text + "XML Data is: " + Server.HtmlEncode(tsiicdata);
                                Failure.Visible = true;
                            }
                                ...
                                if (webRequest != null)
                                {
                                    ...
                                    using (System.IO.Stream s = webRequest.GetResponse().GetResponseStream())
                                    {
                                        using (System.IO.StreamReader sr = new System.IO.StreamReader(s))
                                        {
                                            ...
                                            {
                                                lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " (Consent For Establishment from Pollutuion Control Board ) Status is: " +
                                                            jsonResponse + "<br/>";
                                                Failure.Visible = true;
                                                //string labourouterrormsg = labourout.status + ',' + labourout.uID + ',' + labourout.officerDetails;
                            ...
                            if (dsoutSPDCL != null && dsoutSPDCL.Tables.Count > 0 && dsoutSPDCL.Tables[0].Rows.Count > 0)
                            {
                                if (dsoutSPDCL.Tables[0].Columns.Contains("RESULT"))
                                {
                                    if (dsoutSPDCL.Tables[0].Rows[0]["RESULT"].ToString() == "Success")
                                    {
                                        ...
                                        gen.UpdateDepartwebserviceflag(UIDNO, deptid, "C", spdcloutmsg, "Y");
                                        lblmsg.Text = lblmsg.Text + "For Approval ID: " + deptid + " Status is: " +
                                                      Convert.ToString(spdcloutmsg) + "<br/>";
                                        success.Visible = true;
                                    }
                                    else
                                    {
                                        ...
                                        gen.UpdateDepartwebserviceflag(UIDNO, deptid, "C", spdcloutmsgerror, "N");
                                        lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " Status is: " +
                                                     Convert.ToString(dsoutSPDCL.Tables[0].Rows[0]["RESULT"]) + "<br/>";
                                        Failure.Visible = true;
                                    }
                                }
                            }
                    ...
                    for (int l = 0; l < dt.Rows.Count; l++)
                    {
                                ...
                                if (dsout != null && dsout.Tables.Count > 0 && dsout.Tables[0].Rows.Count > 0)
                                {
                                    if (dsout.Tables[0].Columns.Contains("RESULT"))
                                    {
                                        if (dsout.Tables[0].Rows[0]["RESULT"].ToString() == "Success")
                                        {
                                            ...
                                            int k = gen.InsertDeptDateTracing(dsdept.Tables[0].Rows[0]["DEPTID"].ToString(), dsdept.Tables[0].Rows[0]["intQuessionaireid"].ToString(), UIDNO, System.DateTime.Now.ToString("MM/dd/yyyy"), null, null, null, null, "CFE", deptid);
                                            lblmsg.Text = lblmsg.Text + "For Approval ID: " + deptid + " Status is: " +
                                                          Convert.ToString(dsout.Tables[0].Rows[0]["RESULT"]) + "<br/>";
                                            success.Visible = true;
                                        }
                                        else
                                        {
                        ...
                        if (deptid == "21" || deptid == "5" || deptid == "44")//FACTORIES
                        {
                            try
                            {
                                ...
                                if (dsdept != null && dsdept.Tables.Count > 0 && dsdept.Tables[0].Rows.Count > 0)
                                {
                                    ...
                                    factoryvo.factoryDistrictID = dsdept.Tables[0].Rows[0]["Land_intDistrictid"].ToString();
                                    factoryvo.factoryLocality = dsdept.Tables[0].Rows[0]["Name_Gramapachayat"].ToString();
                                    factoryvo.factoryDoorNumber = dsdept.Tables[0].Rows[0]["SurveyNo"].ToString().Replace("'", "");
                                    factoryvo.factoryMandalID = dsdept.Tables[0].Rows[0]["Land_intMandalid"].ToString();
                                    ...
                                    if (dsdept.Tables[1].Rows.Count > 0)
                                    {
                                        ...
                                        for (int m = 0; m < dtproduct.Rows.Count; m++)
                                        {
                                            FactoryService.productsOutput productvo = new FactoryService.productsOutput();
                                            productvo.product = dtproduct.Rows[m]["Manf_ItemName"].ToString().Replace("'", ""); ;
                                            productvo.output = dtproduct.Rows[m]["OUTPUT"].ToString().Replace("'", ""); ;
                                            product[m] = productvo;
                                        }
                                    }
                        ...
                        if (deptid == "31")//TSIIC
                        {
                            ...
                            if (dsdept != null && dsdept.Tables.Count > 0 && dsdept.Tables[0].Rows.Count > 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " (Industrial Building Plan Approval from IALA/TSIIC) Status is: " +
                                                              tsiicapplication.Error + "<br/>";
                                    lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " XML Data is: " + Server.HtmlEncode(valueshmda);
                                    Failure.Visible = true;
                                }
                            }
                        }
                        ...
                        if (deptid == "65")
                        {
                            ...
                            else
                            {
                                ...
                                lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " (Electrical Drawing approval from Electrical Inspectorate) Status is: " +
                                                                     Ceigappeal + "<br/>";
                                Failure.Visible = true;
                            }
                        ...
                        if (deptid == "8")// FIRE
                        {
                            try
                            {
                                ...
                                //DataSet dsout1 = new DataSet();
                                //dsout1.ReadXml(str1);
                                if (outputfire == "Success" && outputfirepayment == "Success")
                                {
                                    gen.UpdateDepartwebserviceflag(UIDNO, deptid, "R", outputfire, "Y");
                                    //Gen.UpdateDepartwebserviceflag(UIDNO, deptid, "A", OUTPUT, "Y");
                                    int k = gen.InsertDeptDateTracing(dsdept.Tables[0].Rows[0]["DEPTID"].ToString(), dsdept.Tables[0].Rows[0]["intQuessionaireid"].ToString(), UIDNO, System.DateTime.Now.ToString("MM/dd/yyyy"), null, null, null, null, "CFE", deptid);
                                    lblmsg.Text = lblmsg.Text + "For Approval ID: " + deptid + " Status is: " +
                                                                         outputfire + "<br/>";
                                ...
                                {
                                    gen.UpdateDepartwebserviceflag(UIDNO, deptid, "R", outputfire, "N");
                                    lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + deptid + " (Provisional NOC from Fire Services Department)  Status is: " +
                                                                         outputfire + "<br/>";
                                    Failure.Visible = true;
                                }
                ...
                for (int m = 0; m < dt.Rows.Count; m++)
                {
                                        ...
                                        using (System.IO.Stream s = webRequest.GetResponse().GetResponseStream())
                                        {
                                            using (System.IO.StreamReader sr = new System.IO.StreamReader(s))
                                            {
                                                ...
                                                else
                                                {
                                                    ...
                                                    lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + intApprovalid + " (Consent For Establishment from Pollutuion Control Board ) Status is: " +
                                                                        Convert.ToString(jsonResponse) + "<br/>";
                                                    Failure.Visible = true;
                                                }
                                            }
                    ...
                    if (intApprovalid == "65")
                    {
                        ...
                        if (output == "SUCCESS")
                        {
                            ...
                            lblmsg.Text = lblmsg.Text + "For Approval ID: " + intApprovalid + " Status is: " +
                                                                      output + "<br/>";
                            success.Visible = true;
                        }
                        else
                        {
                            ...
                            lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + intApprovalid + " (Electrical Drawing approval from Electrical Inspectorate) Status is: " +
                                                                    output + "<br/>";
                            Failure.Visible = true;
                        }
                        //SUCCESS
                            ...
                            if (dsout1 != null && dsout1.Tables.Count > 0 && dsout1.Tables[0].Rows.Count > 0)
                            {
                                if (dsout1.Tables[0].Columns.Contains("status"))
                                {
                                    if (dsout1.Tables[0].Rows[0]["status"].ToString() == "S")
                                    {
                                        ...
                                        lblmsg.Text = lblmsg.Text + "For Approval ID: " + intApprovalid + " Status is: " +
                                                                          Convert.ToString(dsout1.Tables[0].Columns.Contains("status")) + "<br/>";
                                        success.Visible = true;
                                    }
                                    else
                                    {
                                        ...
                                        lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + intApprovalid + " Status is: " +
                                                                         Convert.ToString(dsout1.Tables[0].Columns.Contains("status")) + "<br/>";
                                        Failure.Visible = true;
                                    }
                                }
                    ...
                    else if (intApprovalid == "8") //FIRE
                    {
                        try
                        {
                            ...
                            remarks = dsdept.Tables[0].Rows[0]["queryRespText"].ToString();
                            string OUTPUT1 = fireservice.StoreQueryResponse(UIDNO, filepath, remarks);
                            DataTable dtfire2 = new DataTable();
                            dtfire2 = dsdept.Tables[0];
                            dsdept.Tables.Remove(dtfire2);

                            fireattachments = dsdept.GetXml();
                            string outputfire = fireservice.StoreUploadDocuments(fireattachments);
                            if (outputfire == "Success")
                            {
                                gen.UpdateDepartwebserviceflag(UIDNO, intApprovalid, "Q", outputfire, "Y");
                            ...
                            {
                                gen.UpdateDepartwebserviceflag(UIDNO, intApprovalid, "Q", outputfire, "N");
                                lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + intApprovalid + " (Provisional NOC from Fire Services Department0 Status is: " +
                                                                          outputfire + "<br/>";
                                Failure.Visible = true;
                            }
                        }
                            ...
                            if (dsout1 != null && dsout1.Tables.Count > 0 && dsout1.Tables[0].Rows.Count > 0)
                            {
                                if (dsout1.Tables[0].Columns.Contains("RESULT"))
                                {
                                    if (dsout1.Tables[0].Rows[0]["RESULT"].ToString() == "Success")
                                    {
                                        ...
                                        int k = gen.InsertDeptDateTracing(intDeptid, intQuessionaireid, ds.Tables[0].Rows[0]["UID_No"].ToString().Trim(), null, null, null, System.DateTime.Now.ToString("MM/dd/yyyy"), null, "CFE", intApprovalid);
                                        lblmsg.Text = lblmsg.Text + "For Approval ID: " + intApprovalid + " Status is: " +
                                                                                Convert.ToString(dsout1.Tables[0].Rows[0]["RESULT"].ToString()) + "<br/>";
                                        success.Visible = true;
                                    }
                                    else
                                    {
                    ...
                    else if (intApprovalid == "21" || intApprovalid == "5" || intApprovalid == "44")//FACTORIES
                    {
                        try
                        {
                            ...
                            queryvo.entrepreneurRemarks = dsdept.Tables[0].Rows[0]["queryRespText"].ToString().Replace("'", "").Replace("/", "");
                    ...
                    else if (intApprovalid == "35")
                    {
                        ...
                        try
                        {
                            ...
                            if (hmdapplication.ResponseStatus.ToString() == "1")
                            {
                                ...
                                int k = gen.InsertDeptDateTracing(intDeptid, intQuessionaireid, UIDNO, null, null, null, System.DateTime.Now.ToString("MM/dd/yyyy"), null, "CFE", intApprovalid);
                                lblmsg.Text = lblmsg.Text + "For Approval ID: " + intApprovalid + " Status is: " +
                                                                                          hmdapplication.ErrorMessage + "<br/>";
                                success.Visible = true;
                            }
                            else
                            {
                                gen.UpdateDepartwebserviceflag(UIDNO, intApprovalid, "Q", hmdapplication.Error, "N");
                                lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + intApprovalid + " (Industrial Building Plan Approval from HMDA) Status is: " +
                                                                                          hmdapplication.Error + "<br/>";
                                lblmsg0.Text = lblmsg0.Text + "XML Data is: " + Server.HtmlEncode(hmda);
                                Failure.Visible = true;
                            }
                        }
                        catch (Exception ex)
                    ...
                    else if (intApprovalid == "7")
                    {
                       ...
                       SecurityProtocolType.Tls | SecurityProtocolType.Tls11 | SecurityProtocolType.Tls12 | SecurityProtocolType.Ssl3;
                        ...
                        if (objTGBPASS.ResponseStatus.ToString() == "1")
                        {
                            ...
                            int k = gen.InsertDeptDateTracing(intDeptid, intQuessionaireid, ds.Tables[0].Rows[0]["UID_No"].ToString().Trim(), null, null, null, System.DateTime.Now.ToString("MM/dd/yyyy"), null, "CFE", intApprovalid);
                            lblmsg.Text = lblmsg.Text + "For Approval ID: " + intApprovalid + " Status is: " +
                                                                                                 objTGBPASS.ResponseStatus + "<br/>";
                            success.Visible = true;
                        }
                        else
                        {
                            lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + intApprovalid + " (Industrial Building Plan Approval from HMDA) Status is: " +
                                                                                             objTGBPASS.ErrorMessage + "<br/>";
                            lblmsg0.Text = lblmsg0.Text + "XML Data is: " + Server.HtmlEncode(hmda);
                            Failure.Visible = true;
                    ...
                    else if (intApprovalid == "31")
                    {
                        ...
                        if (tsiicapplication.ResponseStatus.ToString() == "1")
                        {
                            ...
                            int k = gen.InsertDeptDateTracing(intDeptid, intQuessionaireid, UIDNO, null, null, null, System.DateTime.Now.ToString("MM/dd/yyyy"), null, "CFE", intApprovalid);
                            lblmsg.Text = lblmsg.Text + "For Approval ID: " + intApprovalid + " Status is: " +
                                                                                              tsiicapplication.ErrorMessage + "<br/>";
                            success.Visible = true;
                        }
                        else
                        {
                            gen.UpdateDepartwebserviceflag(UIDNO, intApprovalid, "Q", tsiicapplication.Error, "N");
                            lblmsg0.Text = lblmsg0.Text + "For Approval ID: " + intApprovalid + " (Industrial Building Plan Approval from DT&CP) Status is: " +
                                                                                             tsiicapplication.ErrorMessage + "<br/>";
                            lblmsg0.Text = lblmsg0.Text + "XML Data is: " + Server.HtmlEncode(tsiic);
                            Failure.Visible = true;
                        }
                    }
                    else if (intApprovalid == "72")
                    {
                        //DataSet dsdept = new DataSet();
                        //dsdept = gen.GetQueryStatusByTransactionIDResponse("1065", "35");
                        //string UIDNO = dsdept.Tables[0].Rows[0]["tsipassUid"].ToString();
                        ...
                        dsprofessiontax = gen.GetQueryStatusByTransactionIDResponse(intQuessionaireid, intApprovalid);
                        string quesid, entrpid, uid, documentpath, documentid, queryremarks, createdby, createdt;
                        dsprofessiontax.DataSetName = "Reg_Data";
                        dsprofessiontax.Tables[0].TableName = "regdtls";
                        dsprofessiontax.Tables[1].TableName = "bkacdtls";
                        dsprofessiontax.Tables[2].TableName = "docchklistdtls";
                        dsprofessiontax.Tables[3].TableName = "ownerdtls";
                        dsprofessiontax.Tables[4].TableName = "othpofbizdtls";
                        dsprofessiontax.Tables[5].TableName = "partnerdtls";
                        string ptxml = dsprofessiontax.GetXml();
                        //quesid = dsdept.Tables[0].Rows[0]["questionniareId"].ToString();
                        entrpid = dsdept.Tables[0].Rows[0]["entrepreneurId"].ToString();


                        string ptquery = professiontax.Registration_Query_Update(ptxml, entrpid, UIDNO);
                        StringReader str1 = new StringReader(ptquery);
                        DataSet dsout1 = new DataSet();
                        dsout1.ReadXml(str1);
                        if (dsout1 != null && dsout1.Tables.Count > 0 && dsout1.Tables[0].Rows.Count > 0)
                        {
                            if (dsout1.Tables[0].Columns.Contains("status"))
                            {
                                if (dsout1.Tables[0].Rows[0]["status"].ToString().TrimEnd().TrimStart() == "Success")
                                {
                                    gen.UpdateDepartwebserviceflag(UIDNO, intApprovalid, "Q", dsout1.Tables[0].Rows[0]["status"].ToString().TrimEnd().TrimStart(), "Y");
                                    int k = gen.InsertDeptDateTracing(intDeptid, intQuessionaireid, UIDNO, null, null, null, System.DateTime.Now.ToString("MM/dd/yyyy"), null, "CFE", intApprovalid);
                                    lblmsg.Text = lblmsg.Text + "For Approval ID: " + intApprovalid + " Status is: " +
                                                  Convert.ToString(dsout1.Tables[0].Rows[0]["status"]) + "<br/>";
                                    success.Visible = true;
                                }
                                else
                                {
                        ...
                        //string profession = dsprofessiontax.GetXml();
                        //tsiicapplication = tsiicservice.reSubmission(tsiic, "$$08@213");
                        //if (tsiicapplication.ResponseStatus.ToString() == "1")
                        //{
                        //    gen.UpdateDepartwebserviceflag(UIDNO, intApprovalid, "Q", tsiicapplication.ErrorMessage, "Y");
                    ...
                    else if (intApprovalid == "48")
                    {
                        DataSet dsdeptCPF = new DataSet();
                        DataSet dsdeptattachments = new DataSet();
                        dsdeptCPF = gen.getdepartmentdetailsonuid(UIDNO, intApprovalid);
                        ...
                        string actids = "";
                        string actid = "";
                        actids = dsdeptCPF.Tables[0].Rows[0]["LabourAct_Id"].ToString();
                        string[] split = actids.Split(',');
                        if (actids.Contains("3"))//The Contract Labour Act (Principal Employer)
                        {
                            ...
                            contractvo.employees_attachement = "";//dsdept.Tables[0].Rows[0][""].ToString();
                                                                  //contractvo.entrepreneur_id = dsdept.Tables[0].Rows[0]["intCFEEnterpid"].ToString();
                            contractvo.establishment_category = dsdeptCPF.Tables[0].Rows[0]["Form1_1_Estb_Category"].ToString();
                            contractvo.establishment_category_others = dsdeptCPF.Tables[0].Rows[0]["Form1_1_Estb_Classification"].ToString();
                            contractvo.establishment_name = dsdeptCPF.Tables[0].Rows[0]["Form1_Estb_Name"].ToString();
                            contractvo.establishment_postal_district = dsdeptCPF.Tables[0].Rows[0]["Form1_Estd_District"].ToString();
                            contractvo.establishment_postal_door = dsdeptCPF.Tables[0].Rows[0]["Form1_Estd_DoorNo"].ToString();
                            contractvo.establishment_postal_locality = dsdeptCPF.Tables[0].Rows[0]["Form1_Estd_Locality"].ToString();
                            contractvo.establishment_postal_mandal = dsdeptCPF.Tables[0].Rows[0]["Form1_Estd_Mandal"].ToString();
                            contractvo.establishment_postal_village = dsdeptCPF.Tables[0].Rows[0]["Form1_Estd_Village"].ToString();
                            contractvo.manager_agent_district = dsdeptCPF.Tables[0].Rows[0]["Form1_1_Manager_District"].ToString();
                            contractvo.manager_agent_door = dsdeptCPF.Tables[0].Rows[0]["Form1_1_Manager_DoorNo"].ToString();
                            contractvo.manager_agent_mandal = dsdeptCPF.Tables[0].Rows[0]["Form1_1_Manager_Mandal"].ToString();
                            contractvo.manager_agent_name = dsdeptCPF.Tables[0].Rows[0]["Form1_1_Manager_Name"].ToString();
                            contractvo.manager_agent_village = dsdeptCPF.Tables[0].Rows[0]["Form1_1_Manager_Village"].ToString();
                            // contractvo.max_employees_aday = dsdept.Tables[0].Rows[0]["Form1_2_MaxWorkers"].ToString();
                            contractvo.nature_work = dsdeptCPF.Tables[0].Rows[0]["Form1_Nature_of_Business"].ToString();
                            contractvo.other_attachments_1 = "";
                            contractvo.other_attachments_2 = "";
                            //contractvo.payment_mode = dsdept.Tables[0].Rows[0]["payment_mode"].ToString();
                            //contractvo.paymentFlag = dsdept.Tables[0].Rows[0]["paymentFlag"].ToString();
                            ...
                            //contractvo.previous_registration_certificate = dsdept.Tables[0].Rows[0][""].ToString();
                            //contractvo.previous_registration_no = dsdept.Tables[0].Rows[0][""].ToString();
                            contractvo.principal_employer_district = dsdeptCPF.Tables[0].Rows[0]["Form1_Employer_District"].ToString();
                            contractvo.principal_employer_door = dsdeptCPF.Tables[0].Rows[0]["Form1_Employer_DoorNo"].ToString();
                            contractvo.principal_employer_email = dsdeptCPF.Tables[0].Rows[0]["Form1_Employer_EmailID"].ToString();
                            contractvo.principal_employer_fathername = dsdeptCPF.Tables[0].Rows[0]["Form1_Empolyer_FatherName"].ToString();
                            contractvo.principal_employer_mandal = dsdeptCPF.Tables[0].Rows[0]["Form1_Employer_Mandal"].ToString();
                            contractvo.principal_employer_mobile = dsdeptCPF.Tables[0].Rows[0]["Form1_Employer_MobileNo"].ToString();
                            contractvo.principal_employer_name = dsdeptCPF.Tables[0].Rows[0]["Form1_Employer_Name"].ToString();
                            contractvo.principal_employer_village = dsdeptCPF.Tables[0].Rows[0]["Form1_Employer_Village"].ToString();
                            contractvo.projectSubmitDate = dsdeptCPF.Tables[0].Rows[0]["projectSubmitDate"].ToString();
                            //contractvo.questionnaire_id = dsdept.Tables[0].Rows[0]["intQuessionaireid"].ToString();
                            //contractvo.registration_fee = Convert.ToInt32(dsdept.Tables[0].Rows[0]["registration_fee"].ToString());
                            //contractvo.registration_years = 0;
                            contractvo.stageID = dsdeptCPF.Tables[0].Rows[0]["intStageid"].ToString();
                            //contractvo.total_amount_payable = Convert.ToInt32(dsdept.Tables[0].Rows[0]["total_amount_payable"].ToString());
                            //contractvo.total_penality_amount = 0;
                            //contractvo.total_registration_fee = Convert.ToInt32(dsdept.Tables[0].Rows[0]["registration_fee"].ToString());
                            //contractvo.totalAmount = dsdept.Tables[0].Rows[0]["total_amount_payable"].ToString();
                            ...
                            //contractvo.tsipassApplicationID = dsdept.Tables[0].Rows[0][""].ToString();
                            //contractvo.type_of_application = dsdept.Tables[0].Rows[0]["type_of_application"].ToString();
                            contractvo.uID = dsdeptCPF.Tables[0].Rows[0]["UID_NO"].ToString();
                            //contractvo.unpaid_balance_welfare = Convert.ToInt32(dsdept.Tables[0].Rows[0]["unpaid_balance_welfare"].ToString());
                            //contractvo.valid_from_date = dsdept.Tables[0].Rows[0]["Form1_1_DateofCommencement"].ToString();
                            //contractvo.valid_to_date = dsdept.Tables[0].Rows[0]["Form1_2_Est_Compltd_Dt"].ToString();
                            ...
                            ContractorDetails co = new ContractorDetails();
                            //FactoryService.rawMaterial[] lst = null;
                            if (dsdeptCPF.Tables[1].Rows.Count > 0)
                            {
                                DataTable dtraw = new DataTable();
                                dtraw = dsdeptCPF.Tables[1];
                            ...
                            labouract.contractLabourPrincipalEmplyerActData = contractvo;
                            labourout = labourqueryservice.actSelected(labouract);
                            if (labourout.status == "SUCCESS")
                            {
                                string labouroutmsg = labourout.status + ',' + labourout.uID + ',' + labourout.officerDetails;
                                // Gen.UpdateDepartwebserviceflag(UIDNO, deptid, "C", labouroutmsg, "Y");
                                ...
                                lblmsg.Text = lblmsg.Text + "For Approval ID: " + intApprovalid + " Status is: " +
                                                 Convert.ToString(labourout.status) + "<br/>";
                                success.Visible = true;
                            }
                            else
                            {
        ...
        {
            lblmsg0.Text = "Please enter UID no.";
            Failure.Visible = true;
        }
    }
...
public class RWSVO
{
    ...
    public string institutionGST { get; set; }
    ...
    public string sizeOfTheConnection { get; set; }
    public string locationOfSupply { get; set; }
    ...
    public string connectionCharges { get; set; }
    public string depositCharges { get; set; }
    public string monthlyCharges { get; set; }
    public string status { get; set; }
    public string source { get; set; }
    public string uidno { get; set; }
    public string intquestionaireid { get; set; }
    public string intcfeid { get; set; }
    DataSet AttachmentDetails { get; set; }
    public string panUpload { get; set; }
    public string panUploadData { get; set; }
    public string gstUpload { get; set; }
    public string gstUploadData { get; set; }
    public string aadharUpload { get; set; }
    public string aadharUploadData { get; set; }
    public string otherDocUpload { get; set; }
    public string otherDocUploadData { get; set; }
    public string ownershipDocUpload { get; set; }
    public string ownershipDocUploadData { get; set; }
    public string selfDeclerationUpload { get; set; }
    public string selfDeclerationUploadData { get; set; }
    public string address_proofUpload { get; set; }
    public string address_proofUploadData { get; set; }
