...
public partial class TSTBDCReg1 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            dsp = Gen.GetShowAttachment(Session["uid"].ToString().Trim());
            if (dsp.Tables[0].Rows.Count > 0)
            {
                Session["intQuessionaireid"] = dsp.Tables[0].Rows[0]["intQuessionaireid"].ToString().Trim();
            }
            else
            {
                Session["intQuessionaireid"] = "0";
            }

            if (Session["intQuessionaireid"].ToString().Trim() == "0")
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Please fill the Common Application Form');window.location ='frmDepartmentApprovalDetails.aspx';",true);
                //ClientScript.RegisterStartupScript(this.GetType(), "myalert", "alert('Please fill the Common Application Form');", true);
                //Response.Redirect("frmDepartmentApprovalDetails.aspx");
            }
    ...
    protected void BtnClear0_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            dsland = Gen.Verifylandordetails(Session["Applid"].ToString());
            ...
            if (dsland.Tables[1].Rows.Count == 0)
            {
                ...
                success.Visible = false;
                Failure.Visible = true;
                lblmsg0.Text = "Please Enter land Details in  Common Application Form";
            ...
            dsland = Gen.Verifylandordetails(Session["Applid"].ToString());
            if (dsland.Tables[3].Rows.Count == 0)
            {

                success.Visible = false;
                Failure.Visible = true;
                lblmsg0.Text = "Please Enter Line of Raw material in Common Application Form";
