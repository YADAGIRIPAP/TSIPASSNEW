...
public partial class UI_TSiPASS_frmApproveDetailsbyqueryBattery : System.Web.UI.Page
{
    int delete = 0;
        ...
        try
        {
            ...
            if (BtnSave1.Text == "Submit")
            {
                if (ddlStatus.SelectedItem.Text.Trim() == "Approved")
                {
                    ...
                    {
                        i = insertApprovalDataBattery(Request.QueryString[0].ToString(), txtGeo_Cordinate_Latitude.Text, ddlStatus.SelectedValue.ToString(), Session["uid"].ToString(), hdfFlagID1.Value, hdfFlagID2.Value, txtremarks.Text, getclientIP());
    ...
    void FillDetails()
    {
        ...
        try
        {
            ...
            ds = GetdataofApprovaldataAprovalBATTERYbyID(Request.QueryString[0].ToString(), Session["User_Code"].ToString().Trim());
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                hdfFlagID3.Value = ds.Tables[0].Rows[0]["BatdealerID"].ToString();
                Session["Applid"] = ds.Tables[0].Rows[0]["BatdealerID"].ToString();
                ...
                if ((FileUpload1.PostedFile != null) && (FileUpload1.PostedFile.ContentLength > 0))
                {
                    ...
                    try
                    {
                        ...
                        if (fileType[i - 1].ToUpper().Trim() == "PDF" || fileType[i - 1].ToUpper().Trim() == "DOC" || fileType[i - 1].ToUpper().Trim() == "JPG" || fileType[i - 1].ToUpper().Trim() == "XLS" || fileType[i - 1].ToUpper().Trim() == "XLSX" || fileType[i - 1].ToUpper().Trim() == "DOCX" || fileType[i - 1].ToUpper().Trim() == "ZIP" || fileType[i - 1].ToUpper().Trim() == "RAR")
                        {
                            ...
                            result = InsertBatteryAttachementApproval(Session["Applid"].ToString(), Request.QueryString[0].ToString(), "1", "1", fileType[i - 1].ToUpper(), sFileName, newPath, "A", Session["uid"].ToString(), "ApprovalDocument", hdfFlagID2.Value, hdfFlagID1.Value);
    ...
    public int insertApprovalDataBattery(string Enterprenuer, string RefNo, string Status, string Modified_by, string intApprovalid, string intDeptid, string Remarks, string ipaddress)
    {

        SqlCommand com = new SqlCommand();
        com.CommandType = CommandType.StoredProcedure;
        com.CommandText = "updateApprovaldataBATTERY";
    ...
    public int InsertBatteryAttachementApproval(
            ...
            string FileDescription, string intDeptid, string intApprovalid)
    {
        try
        {
            ...
            myDataAdapter = new SqlDataAdapter("sp_BATTERYAttachmentDetDeptApproval", con.GetConnection);
    ...
    public DataSet GetdataofApprovaldataAprovalBATTERYbyID(string enterprenuer, string intDeptid)
    {
        ...
        try
        {
            ...
            myDataAdapter = new SqlDataAdapter("getenterprenuerdatbyidAprovalsBatteryNew", con.GetConnection);
            myDataAdapter.SelectCommand.CommandType = CommandType.StoredProcedure;
