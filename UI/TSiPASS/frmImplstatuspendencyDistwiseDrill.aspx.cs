...
public partial class UI_TSIPASS_frmImplstatuspendencyDistwiseDrill : System.Web.UI.Page
{
    int delete = 0;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (status == "A")
            {
                lblHeading.Text = "YET TO START CONSTRUCTION";
            }
            //---------------------------------------------------------------------------------------------------
            else if (status == "B")
            {
                lblHeading.Text = "INITIAL STAGE";
            }
            // -----------------------------------------------------------------------------------------------------
            else if (status == "C")
            {
                lblHeading.Text = "ADVANCED STAGE";
            }
            // -----------------------------------------------------------------------------------------------------
            else if (status == "W")
            {
    ...
    public DataSet GetAppealApplications(string ONLINEFLAG, string District, string TSiPASSType, string status, string Year)
    {
        ...
        pp[3].Value = status;
        pp[4].Value = Year;
        Dsnew = Gen.GenericFillDs("USP_GET_IMPLIMENTAION_STATUS_ABSTRACT_DRILL_DISTWISE", pp);
        return Dsnew;
    }
