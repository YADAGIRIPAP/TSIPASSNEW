...
public partial class TSTBDCReg1 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                ds = Gen.RetriveStatusByTypedept(status, type, Request.QueryString[0].ToString().Trim(), Session["DistrictID"].ToString().Trim());
            }
            else
            {
                ds = Gen.RetriveStatusByTypedept(status, type, Request.QueryString[0].ToString().Trim(), "%");
            }
