...
public partial class UI_TSiPASS_frmBillDeskNotUpdatedSingle : System.Web.UI.Page
{
    #region Declaration
    ...
    protected void btnupdate_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string onlineorderno = txtuidno.Text.Trim();
            string Appltype = ddlappltype.SelectedItem.Text.Trim();
            var st = "data";
                ...
                if (webRequest != null)
                {
                    ...
                    using (System.IO.Stream s = webRequest.GetResponse().GetResponseStream())
                    {
                        using (System.IO.StreamReader sr = new System.IO.StreamReader(s))
                        {
                            ...
                            string BilldeskResponse = jsonResponse;//Request.Form["jsonResponse"];
                            ...
                            if (paymentresponseVOsobj.AdditionalInfo4_19 == "NODEPT")
                            {
                                ...
                                if (paymentresponseVOsobj.AdditionalInfo5_20 == "Y")
                                {
                                    ...
                                    if (Appltype == "REN")
                                    {
                                        dspaydtls = Gen.GetEnterprinerpaymentDtlsRenewal(paymentresponseVOsobj.CustomerID_1, paymentresponseVOsobj.AdditionalInfo2_17, paymentresponseVOsobj.AdditionalInfo5_20);
                                    }
                            ...
                            if (paymentresponseVOsobj.AuthStatus_14 == "0300")
                            {
                                ...
                                if (Appltype == "REN")
                                {
                                    int valid = InsertUpdatepaymentdtlsRenewal(paymentresponseVOsobj, paymendepartwisetresponseVOsobj);
                                    if (valid == 1)
                                    {
                                        //  LogErrorToText1("Payment done successfully,Reference number: " + paymentresponseVOsobj.TxnReferenceNo_2);
    ...
    public int InsertUpdatepaymentdtlsRenewal(paymentresponseVOs paymentresponseVOsobj, List<paymendepartwisetresponseVOs> paymendepartwisetresponseVOsobj)
    {
        string str = ConfigurationManager.ConnectionStrings["TSiPASSSkils"].ConnectionString;
        int valid = 0;
        int itemidout = 0;
