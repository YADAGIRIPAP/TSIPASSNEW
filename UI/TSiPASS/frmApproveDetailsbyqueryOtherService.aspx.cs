...
public partial class UI_TSiPASS_frmApproveDetailsbyqueryOtherService : System.Web.UI.Page
{
    int delete = 0;
    ...
    protected void BtnSave_Click(object sender, EventArgs e)
    {
        ...
        if (BtnSave1.Text == "Submit")
        {
            ...
            if (ddlStatus.SelectedItem.Text.Trim() == "Approved")
            {
                ...
                {
                    i = Gen.insertApprovalDataOtherServices(Request.QueryString[0].ToString(), txtGeo_Cordinate_Latitude.Text, ddlStatus.SelectedValue.ToString(), Session["uid"].ToString(), hdfFlagID1.Value, hdfFlagID2.Value, txtremarks.Text, getclientIP());

                    try
                    {
                        int k = Gen.InsertDeptDateTracingOtherServices(hdfFlagID2.Value.Trim(), hdfFlagID3.Value.Trim(), Label447.Text.Trim(), null, null, null, null, System.DateTime.Now.ToString("MM/dd/yyyy"), "OTHERSERVICE", hdfFlagID1.Value);
                    }
                    catch (Exception ex)
                    {
                    ...
                    //{
                    //    int result = 0;
                    //    result = Gen.UpdCommissionerApprovalNew(Request.QueryString[0].ToString(), hdfFlagID2.Value, hdfFlagID1.Value, "15", dscer.Tables[0].Rows[0]["LastDateofPreScrutiy"].ToString(), Session["uid"].ToString().Trim(), hdfFlagID3.Value);
                    //}
                    //else
                    //{
            ...
            else
            {
                ...
                {
                    i = Gen.insertApprovalDataOtherServices(Request.QueryString[0].ToString(), txtGeo_Cordinate_Latitude.Text, ddlStatus.SelectedValue.ToString(), Session["uid"].ToString(), hdfFlagID1.Value, hdfFlagID2.Value, txtremarks.Text, getclientIP());
                    int k = Gen.InsertDeptDateTracingOtherServices(hdfFlagID2.Value, hdfFlagID3.Value, Label447.Text, null, null, null, null, System.DateTime.Now.ToString("MM/dd/yyyy"), "OtherService", hdfFlagID1.Value);
                }
                else
                {
            ...
            if (i != 999)
            {
                ...
                dsMail1 = Gen.GetShowEmailidandMobileNumberOtherServices(hdfFlagID3.Value.ToString());
                if (ddlStatus.SelectedItem.Text.Trim() == "Approved")
                {
    ...
    void FillDetails()
    {
        ...
        try
        {
            ...
            ds = Gen.GetdataofApprovaldataAprovalbyIDOtherServices(Request.QueryString[0].ToString(), Session["User_Code"].ToString().Trim());
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    hdfFlagID1.Value = ds.Tables[0].Rows[0]["approvalid"].ToString().Trim();
                    hdfFlagID2.Value = ds.Tables[0].Rows[0]["intDeptid"].ToString().Trim();
                    hdfFlagID3.Value = ds.Tables[0].Rows[0]["IncentReformId"].ToString();
                }
                else
                {
            ...
            if ((FileUpload1.PostedFile != null) && (FileUpload1.PostedFile.ContentLength > 0))
            {
                ...
                try
                {
                    ...
                    if (fileType[i - 1].ToUpper().Trim() == "PDF" || fileType[i - 1].ToUpper().Trim() == "DOC" || fileType[i - 1].ToUpper().Trim() == "JPG" || fileType[i - 1].ToUpper().Trim() == "XLS" || fileType[i - 1].ToUpper().Trim() == "XLSX" || fileType[i - 1].ToUpper().Trim() == "DOCX" || fileType[i - 1].ToUpper().Trim() == "ZIP" || fileType[i - 1].ToUpper().Trim() == "RAR")
                    {
                        ...
                        int result = 0;
                        result = t1.InsertImagedataApprovalOtherService(hdfFlagID3.Value, Request.QueryString[0].ToString(), fileType[i - 1].ToUpper(), newPath, sFileName, "ApprovalDocument", "", Session["uid"].ToString(), DateTime.Now.ToString(), "1000", DateTime.Now.ToString(), hdfFlagID2.Value, hdfFlagID1.Value);
