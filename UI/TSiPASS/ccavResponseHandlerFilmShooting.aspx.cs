...
public partial class UI_TSiPASS_ccavResponseHandlerFilmShooting : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (msg != null)
        {
            ...
            if (paymentresponseVOsobj.AdditionalInfo4_19 == "NODEPT")
            {
                ...
                {
                    dspaydtls = GetEnterprinerpaymentDtlsFilmshooting(paymentresponseVOsobj.CustomerID_1, paymentresponseVOsobj.AdditionalInfo2_17, paymentresponseVOsobj.AdditionalInfo5_20);
                ...
                {
                    dspaydtls = GetEnterprinerpaymentDtlsFilmshooting(paymentresponseVOsobj.CustomerID_1, paymentresponseVOsobj.AdditionalInfo2_17, "");
                    //Departmentdetails (Questionnaire id#Entrepreneur id1#Department id1#TSIPASSUID#amount1#ApprovalId)
                    paymentresponseVOsobj.Createdby = dspaydtls.Tables[1].Rows[0]["CREATEDBY"].ToString();
                    if (dspaydtls != null && dspaydtls.Tables.Count > 1 && dspaydtls.Tables[1].Rows.Count > 0)
                    {
            ...
            {
                int valid = InsertUpdatepaymentdtlsFilmshooting(paymentresponseVOsobj, paymendepartwisetresponseVOsobj);
                try
                {
                    GetTESTVALUESFilmshooting(BilldeskResponse);
                }
                catch (Exception ex)
                {
                ...
                if (valid == 1)
                {
                    ...
                    Page.ClientScript.RegisterStartupScript(this.GetType(), "alertMsg", "alert('Payment Done Successfully');" + "window.location='IpassPrintReceiptFilmShooting.aspx';", true);
                }
            }
            else
            {
                // LogErrorToText1("Payment failed at bank values: " + respActualValues);
                Page.ClientScript.RegisterStartupScript(this.GetType(), "alertMsg", "alert('Payment Failed at Bank due to " + paymentresponseVOsobj.ErrorDescription_24 + "');" + "window.location='FilmShootingUserDashboardList.aspx';", true);
    ...
    public void GetTESTVALUESFilmshooting(string Responce)
    {
        DB.DB con = new DB.DB();
        con.OpenConnection();
        SqlDataAdapter da;
        ...
        try
        {
            da = new SqlDataAdapter("USP_TEST_WEBSERVISE_filmshooting", con.GetConnection);
            da.SelectCommand.Parameters.Add("@responce", SqlDbType.VarChar).Value = Responce.ToString();
            da.SelectCommand.Parameters.Add("@Online", SqlDbType.VarChar).Value = "Online";
    ...
    public int InsertUpdatepaymentdtlsFilmshooting(paymentresponseVOs paymentresponseVOsobj, List<paymendepartwisetresponseVOs> paymendepartwisetresponseVOsobj)
    {
        ...
        SqlCommand command = new SqlCommand("USP_UPDATE_Builldesc_PaymentDtls_filmshooting", connection);
        ...
        try
        {
            ...
            if (valid == 1)
            {
                ...
                {
                    command1 = new SqlCommand("USP_UPDATE_DEPTWISE_Builldesc_PaymentDtls_filmshooting", connection);
    ...
    public DataSet GetEnterprinerpaymentDtlsFilmshooting(string UID, string Orderno, string AdditionalPaymentflg)
    {
        DB.DB con = new DB.DB();
        con.OpenConnection();
        SqlDataAdapter da;
        ...
        try
        {
            da = new SqlDataAdapter("USP_GET_Builldesc_PaymentDtls_DESE_FilmShooting", con.GetConnection);
            da.SelectCommand.CommandType = CommandType.StoredProcedure;
