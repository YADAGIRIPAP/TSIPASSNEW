             ...
             else
             {
                 ...
                 foreach (GridViewRow row in grdDetails.Rows)
                 {
                     ...
                     else
                     {
                         ...
                         string HdfApprovalid = ((HiddenField)row.FindControl("HdfApprovalid")).Value.ToString();
                         string HdfAmount = ((HiddenField)row.FindControl("HdfAmount")).Value.ToString();
                         int j = Gen.UpdateAdditionalpaymentsUID(HdfQueid, "", "Completed", Session["uid"].ToString(), "2", HdfDeptid, HdfApprovalid,""); //fix by CMS by passing string IPaddress
                 ...
                 if (rdbOnlineBankType.SelectedIndex == 0)// SBI
                 {
                     ...
                     foreach (GridViewRow row in grdDetails.Rows)
                     {
                         if (((CheckBox)row.FindControl("ChkApproval")).Checked)
                         {
                             ...
                             {
                                 Hdfeid.Value = dsch.Tables[0].Rows[0]["intCFEEnterpid"].ToString();
                                 int s = Gen.InsertPaymentDisbusmentSBI(dsch.Tables[0].Rows[0]["intCFEEnterpid"].ToString(), Session["onlinetransId"].ToString().Trim(), HdfDeptid, HdfAmount, "Y", Session["uid"].ToString(), HdfApprovalid);
                        ...
                        // Session["Amount"]=Convert.ToDecimal(Session["Amount"].ToString()).ToString("f2");
                         ...
                         foreach (GridViewRow row in grdDetails.Rows)
                         {
                             if (((CheckBox)row.FindControl("ChkApproval")).Checked)
                             {
                                 ...
                                 {
                                     Hdfeid.Value = dsch.Tables[0].Rows[0]["intCFEEnterpid"].ToString();
                                     int s = Gen.InsertPaymentDisbusment(dsch.Tables[0].Rows[0]["intCFEEnterpid"].ToString(), sonlinetrnsNo, HdfDeptid, HdfAmount, "Y", Session["uid"].ToString(), HdfApprovalid,"CFE");
