...
public partial class TSTBDCReg1 : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (dsver != null && dsver.Tables.Count > 0 && dsver.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (dsver.Tables[0].Rows[0]["TourismFlag"].ToString() != null && dsver.Tables[0].Rows[0]["TourismFlag"].ToString() == "Y")
                    {
                    ...
                    {
                        DateTime testng = Convert.ToDateTime(dsver.Tables[0].Rows[0]["Created_dt"].ToString());
                        string testngDate;
                        testngDate = testng.ToString("dd-MM-yyyy");
                        if (Convert.ToDateTime(testngDate) >= Convert.ToDateTime("05-09-2020"))
                        {
                    ...
                    else
                    {
                        ...
                        string testDate_string = testDate.ToString();

                        if (Convert.ToDateTime(testDate_string) >= Convert.ToDateTime("04/09/2020"))
                        {
                            Response.Redirect("frmQuesstionniareReg1.aspx");
                        }
                    }
    ...
    public void CalcFees()
    {
        try
        {
            ...
            // END OF ADDED CODE ON 11/10/2022
            ...
            if (RdbExecptedactShop.SelectedValue == "Y")
            {
                ...
                dsLabour2 = Gen.GetShowApprovalandFeesLabouract1("66", Enterpriseid, ddlProp_intDistrictid.SelectedValue, "7", txtNoofworkers.Text.Trim().TrimStart(), date1, Session["uid"].ToString().Trim());
                dsv.Tables[0].Merge(dsLabour2.Tables[0]);
            }
            ...
            DataSet dsLabour9 = new DataSet();
            if (Rdb20d.SelectedValue == "Y")
            {
                dsLabour9 = Gen.GetShowApprovalandFeesLabour("58", Enterpriseid, ddlProp_intDistrictid.SelectedValue, "5", txtNoofworkers20d.Text.Trim().TrimStart());
                dsv.Tables[0].Merge(dsLabour9.Tables[0]);
            }
    ...
    public void ProjectCostFinal()
    {
        ...
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Your project cost mentioned below 1 Lakhs. please correct the same..!');", true);
            return;
        }
    ...
    protected void btntab2next_Click(object sender, EventArgs e)
    {
        ...
        if (dscheck != null && dscheck.Tables.Count > 0 && dscheck.Tables[0].Rows.Count > 0)
        {
            if (dscheck.Tables[0].Rows[0]["UERNAME"].ToString() != "chandraquarry")
            {
                string msg = "Unit already exists with user id -" + dscheck.Tables[0].Rows[0]["UERNAME"].ToString().Trim();
                lblmsg0.Text = msg;
                Failure.Visible = true;
