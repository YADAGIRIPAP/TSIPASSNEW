...
public partial class UI_TSiPASS_ReleasePendingView : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
...

            {
                ...
                ds = gen.GenericFillDs("USP_GET_UNIT_INCENTIVEWISE", pp);

                //ds = gen.getReleaseProceedings(Cast, Investmentid);
            ...
            else if (Convert.ToString(Request.QueryString["LEVEL"]) == "COI")
            {
                SqlParameter[] pp = new SqlParameter[]
                {
                new SqlParameter("@IncentiveTypID",SqlDbType.VarChar),
                ...
                pp[4].Value = APPSTATUS;
                pp[5].Value = Distid;
                pp[6].Value = Convert.ToString(Session["uid"]);

                ds = gen.GenericFillDs("USP_GET_UNIT_INCENTIVEWISE_COI", pp);
    ...
    protected void grdDetails_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            else
            {
                ...
                {
                    (e.Row.FindControl("anchortagGMCertificate") as HyperLink).NavigateUrl = "ProformaApplicantCoalReimbursement.aspx?incentiveid=" + enterid.Text.Trim() + "&Scheme=" + lblScheme.Text.Trim() + "&DIPCNumer=" + lblDIPCNumer.Text.Trim();
                }
                if (MstIncentiveId.Text == "19")
                {
                    (e.Row.FindControl("anchortagGMCertificate") as HyperLink).NavigateUrl = "ProformaApplicantWoodlReimbursement.aspx?incentiveid=" + enterid.Text.Trim() + "&Scheme=" + lblScheme.Text.Trim() + "&DIPCNumer=" + lblDIPCNumer.Text.Trim();
                }
