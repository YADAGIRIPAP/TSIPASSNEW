...
public partial class UI_TSiPASS_frmProcessPattalandappl : System.Web.UI.Page
{
    DB.DB con = new DB.DB();
    ...
    {
        string PATTADARID = Request.QueryString[0].ToString();
        string ID = Request.QueryString[1].ToString();

        DataSet dsn = new DataSet();

        dsn = GetPattalandApplDetails(ID, PATTADARID);
    ...
    public DataSet GetPattalandApplDetails(string ID, string PATTADARID)
    {
        con.OpenConnection();
        SqlDataAdapter da;
        DataSet ds = new DataSet();

        da = new SqlDataAdapter("GetPattalandApplDetails_ToProcess", con.GetConnection);
        ...
        if (PATTADARID.Trim() == "" || PATTADARID.Trim() == null)
            da.SelectCommand.Parameters.Add("@PATTADARID", SqlDbType.VarChar).Value = "%";
        else
            da.SelectCommand.Parameters.Add("@PATTADARID", SqlDbType.VarChar).Value = PATTADARID.ToString();
        ...
        try
        {
            ...
            if ((FUPSignedDGPS.PostedFile != null) && (FUPSignedDGPS.PostedFile.ContentLength > 0))
            {
                ...
                try
                {
                    ...
                    {
                        //Create a new subfolder under the current active folder
                        newPath = System.IO.Path.Combine(sFileDir, Request.QueryString[1].ToString() + "\\SignedDGPSMap");
                        ...
                        int result = 0;
                        result = InsertImagedata(Request.QueryString[1].ToString(), Request.QueryString[0].ToString(), fileType[i - 1].ToUpper(),
                            newPath, sFileName, "SignedDGPS", "Y", Session["uid"].ToString(), DateTime.Now.ToString());
                        if (result > 0)
                        {
                            lblmsg.Text = "<font color='green'>Attachment Successfully Added..!</font>";
    ...
    public int InsertImagedata(string ID, string PATTADARID, string FileType, string FilePath, string FileName, string FileDescription,
      string ApprovedDGPS, string Modified_by, string Modified_dt)
    {
        try
        {
            con.OpenConnection();
            ...
            myDataAdapter = new SqlDataAdapter("sp_InsertNOC_Pattadar", con.GetConnection);
            ...
            {
                myDataAdapter.SelectCommand.Parameters.Add("@PATTADARID", SqlDbType.VarChar).Value = DBNull.Value;
            }
            else
            {
                myDataAdapter.SelectCommand.Parameters.Add("@PATTADARID", SqlDbType.VarChar).Value = PATTADARID.Trim();
            }
            ...
            if (ApprovedDGPS.Trim() == "" || ApprovedDGPS.Trim() == null || ApprovedDGPS.Trim() == "--Select--")
            {
                myDataAdapter.SelectCommand.Parameters.Add("@ApprovedDGPS", SqlDbType.VarChar).Value = DBNull.Value;
            }
            else
            {
                myDataAdapter.SelectCommand.Parameters.Add("@ApprovedDGPS", SqlDbType.VarChar).Value = ApprovedDGPS.Trim();
            }
    ...
    protected void btnsave_Click(object sender, EventArgs e)
    {
        try
        {
            if (ddlStatus.SelectedValue.ToString() == "16")
            {
                ...
                if (RjctReason != "")
                {
                    int valid = UpdatePattalandGRejectStatus(Session["uid"].ToString(), Request.QueryString[0].ToString().Trim(), 
                        Request.QueryString[1].ToString().Trim(), RjctReason, getclientIP());
                    if (valid == 1)
                    {
            ...
            if (ddlStatus.SelectedValue.ToString() == "13")
            {
                ...
                {
                    lblmsg0.Text = "<font color='red'>Please Upload SignedDGPS in PDF format only..!</font>";
                    success.Visible = false;
                    Failure.Visible = true;
                }
                ...
                {
                    int valid = UpdatePattalandApprovalStatus(Session["uid"].ToString(), Request.QueryString[0].ToString().Trim(),
                        Request.QueryString[1].ToString().Trim(), getclientIP(), DateTime.Now.ToString());
                    if (valid >= 1)
                    {
    ...
    public int UpdatePattalandGRejectStatus(string createdby, string PATTADARID, string ID, String RjctReason, string IPaddress)
    {
        ...
        SqlCommand command = new SqlCommand("USP_UPDATE_Pattadar_RejectSTATUS", connection);
        command.CommandType = CommandType.StoredProcedure;

        try
        {
            command.Parameters.AddWithValue("@Createdby", createdby);
            command.Parameters.AddWithValue("@PATTADARID", PATTADARID);
    ...
    public int UpdatePattalandApprovalStatus(string createdby, string PATTADARID,string ID, string IPaddress, string ApprovalDate)
    {
        try
        {
            con.OpenConnection();
            ...
            myDataAdapter = new SqlDataAdapter("USP_UPDATE_Pattadar_approvalSTATUS", con.GetConnection);
            myDataAdapter.SelectCommand.CommandType = CommandType.StoredProcedure;
