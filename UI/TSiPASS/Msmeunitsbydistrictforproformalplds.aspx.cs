...
public partial class UI_TSiPASS_Msmeunitsbydistrictforproformalplds : System.Web.UI.Page
{
    ...
    public void fillgrid()
    {
        ...
        if (valid == 0)
        {
            ...
            DataSet ds = obj_insert.getmsmeunitsbydistrictforproformalplds(ddldistrict.SelectedValue, ddlCategory.SelectedValue, FromdateforDB, TodateforDB);
            if (ds != null)
            {
    ...
    protected void grdDetails_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        {
            string MSME = Convert.ToString(DataBinder.Eval(e.Row.DataItem, "MSME_NO"));
            string Countmsmepdls = Convert.ToString(DataBinder.Eval(e.Row.DataItem, "Countmsmepdls"));
            HyperLink hyp_addeditdatapdls = (e.Row.FindControl("hyp_addeditdatapdls") as HyperLink);
            HyperLink hyp_viewdatapdls = (e.Row.FindControl("hyp_viewdatapdls") as HyperLink);
            hyp_viewdatapdls.Visible = false;
            if (!string.IsNullOrEmpty(Countmsmepdls))
            {
                if (Convert.ToInt16(Countmsmepdls) > 0)
                {
                    hyp_viewdatapdls.Visible = true;
                    hyp_addeditdatapdls.Text = "Edit";
                }

            }
            hyp_addeditdatapdls.NavigateUrl = "Applyproformalifesciences.aspx?MSMENO=" + Convert.ToString(DataBinder.Eval(e.Row.DataItem, "MSME_NO")).Trim();
            hyp_viewdatapdls.NavigateUrl = "PrintmsmeProformaofdirectorlifesciences.aspx?MSMENO=" + Convert.ToString(DataBinder.Eval(e.Row.DataItem, "MSME_NO")).Trim();
        }
    }
