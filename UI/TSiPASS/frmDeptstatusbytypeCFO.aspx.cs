    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
           ...
           // getdistricts();
            ...
            string Fromdt = Request.QueryString[3].ToString().Trim();
            string Todt = Request.QueryString[4].ToString().Trim();
            string FromdateforDB = "", TodateforDB = "";
            if (Fromdt != "" && Todt != "")
            {
                FromdateforDB = Convert.ToString(DateTime.ParseExact(Fromdt, "dd-MM-yyyy", null).ToString("MM-dd-yyyy"));
                TodateforDB = Convert.ToString(DateTime.ParseExact(Todt, "dd-MM-yyyy", null).ToString("MM-dd-yyyy"));
            }
            if (Convert.ToString(Session["DistrictID"]) != "")
            {
                ds = Gen.RetriveStatusByTypeDistrictCFO(status, type, "%", Request.QueryString[0].ToString().Trim(),Session["DistrictID"].ToString().Trim(), FromdateforDB,  TodateforDB);
            }
            else
            {
                ds = Gen.RetriveStatusByTypeDistrictCFO(status, type, "%", Request.QueryString[0].ToString().Trim(),"%" , FromdateforDB, TodateforDB);
            }
