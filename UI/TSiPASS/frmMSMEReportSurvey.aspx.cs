...
public partial class UI_TSiPASS_frmMSMEReportSurvey : System.Web.UI.Page
{
    ...
    void FillDetails()
    {
        try
        {
            ...
            {
                FromdateforDB = Convert.ToString(DateTime.ParseExact(txtFromDate.Text, "MM-dd-yyyy", null).ToString("MM-dd-yyyy"));
                TodateforDB = Convert.ToString(DateTime.ParseExact(txtTodate.Text, "MM-dd-yyyy", null).ToString("MM-dd-yyyy"));
            }


            string district = Request.QueryString["DistrictID"].ToString();
            DataSet ds = GetMSMEApplicationsDrill(FromdateforDB, TodateforDB, district);
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            //if ((Session["Role_Code"] != null && Session["Role_Code"].ToString() == "GM")) //provision given to all logins to update msme report details on 20.07.2021
                  ...
                  //  string totalemployee = Convert.ToString(DataBinder.Eval(e.Row.DataItem, "totalEmployement")).Trim();
                    ...
                    LinkEDIT.Text = "Edit/Map to TGIPASS";
                    //LinkEDIT.OnClientClick = "javascript:return PopupONE('" + intedit + "')";
                    LinkEDIT.NavigateUrl = "frmMSMEedit_MSMESURVEY.aspx?UID=" + intedit;
    ...
    #endregion
    public DataSet GetMSMEApplicationsDrill(string fromdate, string todate, string district)
    {
        ...
        pp[1].Value = todate;
        pp[2].Value = district;
        Dsnew = Gen.GenericFillDs("USP_GET_MSME_REPORT_MSMESURVEY", pp);
        return Dsnew;
    }
