...
public partial class TSTBDCReg1 : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (BtnSave1.Text == "Submit")
            {
                if (ddlStatus.SelectedItem.Text.Trim() == "Approved")
                {
                    ...
                    {
                        i = Gen.insertApprovalDataCFO(Request.QueryString[0].ToString(), txtGeo_Cordinate_Latitude.Text, ddlStatus.SelectedValue.ToString(), Session["uid"].ToString(), hdfFlagID1.Value, hdfFlagID2.Value, txtremarks.Text, getclientIP());
                        ...
                        if (dscer.Tables[0].Rows.Count > 0)
                        {
                            ...
                            try
                            {
                                // int k = Gen.InsertDeptDateTracingCFO(intDeptid, intQuessionaireid, Label447.Text, null, null, null, null, System.DateTime.Now.ToString("MM/dd/yyyy"), "CFO", hdfFlagID1.Value);
                                int k = Gen.InsertDeptDateTracingCFO(hdfFlagID2.Value.Trim(), hdfFlagID3.Value.Trim(), Label447.Text.Trim(), null, null, null, null, System.DateTime.Now.ToString("MM/dd/yyyy"), "CFO", hdfFlagID1.Value);
                            }
                            catch (Exception ex)
                            {
                ...
                else
                {
                    if (txtremarks.Text != "")
                    {
                        ...
                        {
                            // int k = Gen.InsertDeptDateTracingCFO(intDeptid, intQuessionaireid, Label447.Text, null, null, null, null, System.DateTime.Now.ToString("MM/dd/yyyy"), "CFO", hdfFlagID1.Value);
                            int k = Gen.InsertDeptDateTracingCFO(hdfFlagID2.Value, hdfFlagID3.Value, Label447.Text, null, null, null, null, System.DateTime.Now.ToString("MM/dd/yyyy"), "CFO", hdfFlagID1.Value);
                        }
                        catch (Exception ex)
                        {
                ...
                if (i != 999)
                {
                    ...
                    dsMail1 = Gen.GetShowEmailidandMobileNumbernewCFO(hdfFlagID3.Value.ToString(), hdfFlagID2.Value.ToString());
    ...
    void FillDetails()
    {
        ...
        try
        {
            ...
            ds = Gen.GetdataofApprovaldataAprovalCFObyID(Request.QueryString[0].ToString(), Session["User_Code"].ToString().Trim());


            if (ds.Tables[0].Rows.Count > 0)
            {
                Label447.Text = ds.Tables[0].Rows[0]["UID_No"].ToString().Trim();
                Label448.Text = ds.Tables[0].Rows[0]["NameofIndustrialUnder"].ToString().Trim();
                ...
                hdfFlagID3.Value = ds.Tables[0].Rows[0]["intQuessionaireCFOid"].ToString();
                Session["Applid"] = ds.Tables[0].Rows[0]["intQuessionaireCFOid"].ToString();
