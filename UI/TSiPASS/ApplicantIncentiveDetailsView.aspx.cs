...
public partial class UI_TSiPASS_ApplicantIncentiveDetailsView : System.Web.UI.Page
{
                ...
                try
                {
                    if (!IsPostBack)
                    {
                        ...
                        try
                        {
                            ...
                            if (Gds != null && Gds.Tables.Count > 0 && Gds.Tables[5].Rows.Count > 0)
                            {
                                trUpdateInspectionReport1.Visible = true;
                                trUpdateInspectionReport.Visible = true;
                                gvinspectionReport.DataSource = Gds.Tables[5];
                                gvinspectionReport.DataBind();

                            }

                            if (Gds != null && Gds.Tables.Count > 0 && Gds.Tables[3].Rows.Count > 0)
                            {
                                tr5.Visible = true;
                                gvquery.DataSource = Gds.Tables[3];
                                gvquery.DataBind();

                            }


                            if (Gds != null && Gds.Tables.Count > 0 && Gds.Tables[6].Rows.Count > 0)
                            {
                                tr2.Visible = true;
                                //gvinspectionOfficer.DataSource = Gds.Tables[6];
                                //gvinspectionOfficer.DataBind();
                                BindingGmRecommendedIncentives(Gds.Tables[6]);
                            }

                            if (Gds != null && Gds.Tables.Count > 0 && Gds.Tables[4].Rows.Count > 0)
                            {
                                tr6.Visible = true;
                                gvresponcegmgv.DataSource = Gds.Tables[4];
                                gvresponcegmgv.DataBind();

                            }


                            if (Gds != null && Gds.Tables.Count > 0 && Gds.Tables[7].Rows.Count > 0)
                            {
                                tr7.Visible = true;
                                gvProcessedApplications.DataSource = Gds.Tables[7];
                                gvProcessedApplications.DataBind();

                            }

                            if (Gds != null && Gds.Tables.Count > 0 && Gds.Tables[8].Rows.Count > 0)
                            {
                                tr8.Visible = true;
                                GridView1.DataSource = Gds.Tables[8];
                                GridView1.DataBind();

                            }

                            if (Gds != null && Gds.Tables.Count > 0 && Gds.Tables[9].Rows.Count > 0)
                            {
                                tr9.Visible = true;
                                gvGmReject.DataSource = Gds.Tables[9];
                                gvGmReject.DataBind();

                            }


                            if (Gds != null && Gds.Tables.Count > 0 && Gds.Tables[10].Rows.Count > 0)
                            {
                                tr10.Visible = true;
                                gvJdReject.DataSource = Gds.Tables[10];
                                gvJdReject.DataBind();

                            }

                            if (Gds != null && Gds.Tables.Count > 0 && Gds.Tables[11].Rows.Count > 0)
                            {
                                tr11.Visible = true;
                                gvJdAbeyance.DataSource = Gds.Tables[11];
                                gvJdAbeyance.DataBind();
