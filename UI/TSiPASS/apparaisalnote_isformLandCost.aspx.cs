...
public partial class UI_TSiPASS_apparaisalnote_isformLandCost : System.Web.UI.Page
{
    ...
    public bool save()
    {
        ...
        if (returnval == "1")
        {
            ...
            fromvo.MstIncentiveId = "17"; //Request.QueryString["MstIncentiveId"].ToString();
    ...
    public string InsertCommonData(IncentiveVosAppraisalStamp oIncentiveVosA)
    {
        ...
        try
        {
            SqlCommand com = new SqlCommand();
            com.CommandType = CommandType.StoredProcedure;
            com.CommandText = "[USP_INSERT_INCENTIVES_DATA_COMMON_appraisal_LANDCOST]";
    ...
    public void AssignValuestoVosFromcontrols()
    {
        ...
        oIncentiveVosA.MstIncentiveId = "17";//Request.QueryString["MstIncentiveId"].ToString();
        oIncentiveVosA.LANDPURCHASE = Convert.ToDecimal(txtlandpurchase.Text);
        oIncentiveVosA.LANDVALUE = Convert.ToDecimal(txtlandvalue.Text);
        ...
        if(txtlandcosttotal.Text==""|| txtlandcosttotal.Text==null)
        {
            oIncentiveVosA.LANDCOST_Total = 0;
        }
        else
        {
            oIncentiveVosA.LANDCOST_Total = Convert.ToDecimal(txtlandcosttotal.Text);
        }
        oIncentiveVosA.TotalBuildingPlinth_CALCULATED = Convert.ToDecimal(txttotalplinth.Text);
    ...
    public string ValidateControls()
    {
        ...
        if (txtlandpurchase.Text.TrimStart().TrimEnd().Trim() == "")
        {
            ErrorMsg = ErrorMsg + slno + ". Please enter Land Purchase in Sq, Mts.\\n";
            slno = slno + 1;
        }
        if (txtlandvalue.Text.TrimStart().TrimEnd().Trim() == "")
        {
            ErrorMsg = ErrorMsg + slno + ". Please Enter Land Value in Rs.  \\n";
            slno = slno + 1;
        }
    ...
    protected void btnPrint_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            //    //if (retval == "deleted")
            //    //    ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Line of activity deleted');", true);
            Response.Redirect("appraisalNote_is.aspx?incid=" + txtINCNoEntry.Text.ToString() + "&mstid=17");
        }
    ...
    protected void rdeligibility_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            if (rdeligibility.SelectedValue != "")
            {
                ...
                if (rdeligibility.SelectedValue == "Regular")
                {
                    txteligiblelandcost.Text = txtproportionate.Text;
                }
                else if (rdeligibility.SelectedValue == "Belated")
                {
                    txteligiblelandcost.Text =(Convert.ToDecimal( txtproportionate.Text)/2).ToString();
                }
            ...
            if (txteligiblelandcost.Text != null || txteligiblelandcost.Text != "")
            {
                ...
                { 
                    ...
                    {
                        TXTFINALELIGIBLELANDCOST.Text = "1000000";
                    }
                ...
                if(Convert.ToDecimal(txtgmRecommend.Text) > Convert.ToDecimal(txteligiblelandcost.Text))
                {
                    if(Convert.ToDecimal(txteligiblelandcost.Text) <= 1000000)
                    {
    ...
    protected void txtbuildingplinth_TextChanged(object sender, EventArgs e)
    {
        ...
        {
            txtproportionate.Text = Convert.ToString((Convert.ToDecimal(txttotalplinth.Text) * Convert.ToDecimal(txtlandvalue.Text)) / Convert.ToDecimal(txtlandpurchase.Text));
        }
