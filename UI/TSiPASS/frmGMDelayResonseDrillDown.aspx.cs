...
public partial class UI_TSiPASS_frmGMDelayResonseDrillDown : System.Web.UI.Page
{
    ...
    protected void fetchIncentivedet()
    {
        ...
        string fromdate = "";
        string todate = "";
        string districtid, designation, addlflag;
        ds = GetGMDelayApplicationsdrilldown(fromdate, todate, Request.QueryString[0].ToString(), "", "", "", Request.QueryString["StatusFlag"].ToString());
        ...
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
        {
            ...
            Session["DS4Grid"] = ds;
            if (Request.QueryString[1].ToString() == "7")
            {
            ...
            if (Request.QueryString[1].ToString() == "5")
            {
            ...
            {
                if (Request.QueryString[1].ToString() == "6")
                {
    ...
    protected void gvdetailsnew_RowDataBound(object sender, System.Web.UI.WebControls.GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            HyperLink anchortagIPOCertificate = (e.Row.FindControl("anchortagGMCertificate") as HyperLink);//)gvinspectionReport.Rows[indexing].FindControl("anchortagGMCertificate");
            anchortagIPOCertificate.NavigateUrl = "industriesprintpage.aspx?EntrpId=" + enterid.Text.Trim();
        }
        //if (e.Row.RowType == DataControlRowType.DataRow)
        //{
        //    Label enterid = (e.Row.FindControl("lblIncentiveID") as Label);
    ...
    protected void anchortaglink_Click(object sender, EventArgs e)
    {
        ...
        fromvo.EnterperIncentiveID = enterid.Text; //((Label)gvrow.FindControl("lblIncentiveID")).Text.ToString();
        ...
        string message = "alert('Reason Accepted Successfully')";
    ...
    public DataSet GetGMDelayApplicationsdrilldown(string fromdate, string todate, string districtid, string designation, string addlflag, string scflag, string statusflag)
    {
        DataSet Dsnew = new DataSet();
        ...
        SqlParameter[] pp = new SqlParameter[] {
             new SqlParameter("@FromDate",SqlDbType.VarChar),
               ...
               new SqlParameter("@DISTRICTID",SqlDbType.VarChar),
                ...
                new SqlParameter("@ADDLFLAG",SqlDbType.VarChar),
                new SqlParameter("@SCFLAG",SqlDbType.VarChar),
                 new SqlParameter("@STATUSFLAG",SqlDbType.VarChar)
        ...
        pp[4].Value = addlflag;
        pp[5].Value = scflag;
        pp[6].Value = statusflag;

        Dsnew = Gen.GenericFillDs("USP_GET_GMEXPLAINATION_DETAILS_DRILLDOWN", pp);
        return Dsnew;
    }
    ...
    protected void btnnotaccept_Click(object sender, EventArgs e)
    {
        ...
        GridViewRow row = (GridViewRow)ddlDeptnameFnl2.NamingContainer;
        Label enterid = (Label)row.FindControl("lblIncentiveID");
        HyperLink scletter = (HyperLink)row.FindControl("anchortagGMCertificate"); //anchortagGMCertificate.NavigateUrl;
        ...
        (row.FindControl("anchortagGMCertificate") as HyperLink).Text = "Showcause Letter";
        (row.FindControl("anchortagGMCertificate") as HyperLink).NavigateUrl = "industriesprintpage.aspx?EntrpId=" + enterid.Text.Trim();

        //scletter.NavigateUrl = "industriesprintpage.aspx?EntrpId=" + enterid.Text.Trim();
        fetchIncentivedet();
