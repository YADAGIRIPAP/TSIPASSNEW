...
public partial class UI_TSiPASS_IncentivesAnnexures_AnnexQualityCertification : System.Web.UI.Page
{
    ...
    private void FillDetailsQualityCertification(DataSet ds)
    {
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
        {
            ...
            ddlstate.Text = ds.Tables[0].Rows[0]["State_Name"].ToString();

            if (ds.Tables[0].Rows[0]["intStateid"].ToString() == "31")
            {
                ddlDistrict.Text = ds.Tables[0].Rows[0]["District_Name"].ToString();
                ddlmandal.Text = ds.Tables[0].Rows[0]["Manda_lName"].ToString();
                ddlvillage.Text = ds.Tables[0].Rows[0]["Village_Name"].ToString();
            }
            else
            {
