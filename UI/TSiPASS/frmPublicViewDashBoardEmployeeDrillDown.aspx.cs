...
public partial class UI_TSiPASS_frmPublicViewDashBoardEmployeeDrillDown : System.Web.UI.Page
{
    DB.DB con = new DB.DB();
    int delete = 0;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            //{
            //    ds = Gen.homepagedatadrilldown(status, financialyear, ddldistrict.SelectedItem.Text.Trim().ToString(),"%");
            //}
            //else
            //{
    ...
    public DataSet homepagedatadrilldown(string status, string financialyaer, string District, string lineofactivity)
    {
        ...
        try
        {
            da = new SqlDataAdapter("[USP_GET_PublicViewDashBoardAll_DrillDown]", con.GetConnection);
            da.SelectCommand.CommandType = CommandType.StoredProcedure;
