...
using System.Data;
public partial class UI_TSiPASS_frmPaymentMobileTower : System.Web.UI.Page
{
    Cls_UserMobileTower obj_adv = new Cls_UserMobileTower();
    int delete = 0;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            DataSet ds = obj_adv.GetMobileTowerAppIDbyuserid(CreatedBy);
            if (ds.Tables.Count > 0)
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (App_Status == "2")
                    {
                        ...
                        dsv = null;
                        dsv = obj_adv.GetShowApprovalandFees_MobileTower(Session["Applid"].ToString());
                    ...
                    else
                    {
                        ...
                        {
                            DataSet ds_ap = obj_adv.GetMobileTowerPayDetailsAddtionalPayment(Session["Applid"].ToString().Trim());
    ...
    protected void BtnSave_Click(object sender, EventArgs e)
    {
        ...
        int UIDUpdate = obj_adv.UpdateUIDMobileTower(Session["UID_no"].ToString(), hdnTransactionNumber.Value);
        ...
        foreach (GridViewRow row in grdDetails.Rows)
        {
            ...
            {
                s = obj_adv.insertDepartmentAprroval_MobileTower(hdnTransactionNumber.Value, HdfQueid, HdfApprovalid, HdfAmount, "N", Session["uid"].ToString().Trim(), "N");
            }

            DataSet dsch = new DataSet();
            dsch = obj_adv.getMobileTowerdatabyQues(hdnTransactionNumber.Value);
            if (dsch.Tables[0].Rows.Count > 0)
            {
                string Hdfeid = dsch.Tables[0].Rows[0]["CreatedBy"].ToString();

                int A = obj_adv.InsertPaymentBillDesk_MobileTower(Session["UID_no"].ToString(), Convert.ToString(Session["Applid"]), sonlinetrnsNo, HdfQueid, HdfAmount, Session["uid"].ToString(), HdfApprovalid, "TE", Session["AdditionalPayment"].ToString());
        ...
        if (Result.ToUpper().ToString() == "SUCCESS")
        {
            try
            {
                Response.Redirect("KotakMobileTower.aspx");
            }
            catch (Exception ex)
            {
