...
public partial class UI_TSiPASS_frmDepartmentGroundwaterPaymentDetails : System.Web.UI.Page
{

    Cls_OSGroundWater obj_payment = new Cls_OSGroundWater();
    decimal TotalFee;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["intApplicationId"] != null)
            {
                dscheck = obj_payment.GetUidnumber_groundwater(Request.QueryString["intApplicationId"].ToString());
            ...
            else
            {
                ...
                DataSet dspay = obj_payment.GetEnterPreniourPayDetailsPaidDetgroundwater(Session["Applid"].ToString().Trim());
                ...
                {
                    // should pay
                    DataSet ds = obj_payment.GetEnterPreniourPayDetailsGroundwater(Session["Applid"].ToString().Trim());
            ...
            if (chkBuilldesc.Checked && rdbOnlineBankType.SelectedValue == "Kotak")
                {
                    ...
                    foreach (GridViewRow row in grdDetails.Rows)
                    {
                            ...
                            dsch = obj_payment.GetEnterPreneurdatabyQue_Groundwater(HdfQueid);
                       ...
                       int dept = obj_payment.insertDepartmentAprroval_Groundwater(HdfQueid, HdfDeptid, HdfApprovalid, HdfAmount, "N", Session["uid"].ToString().Trim(), "N"); 

                        int s = obj_payment.InsertPaymentBillDesk_groundwater(UIDNOPresent, Convert.ToString(dsch.Tables[0].Rows[0]["intQuessionaireid"]),sonlinetrnsNo, HdfDeptid, HdfAmount,Convert.ToString(Session["uid"]), HdfApprovalid, "GW",Convert.ToString(Session["AdditionalPayment"]));
                            }
                      
                    }
                    Response.Redirect("BilldeskPaymentPage_Groundwater.aspx");
                }
