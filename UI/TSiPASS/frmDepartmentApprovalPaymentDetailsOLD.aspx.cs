                 ...
                 if (rdbOnlineBankType.SelectedIndex == 0)// SBI
                 {
                     ...
                     foreach (GridViewRow row in grdDetails.Rows)
                     {
                         if (((CheckBox)row.FindControl("ChkApproval")).Checked)
                         {
                             ...
                             {
                                 Hdfeid.Value = dsch.Tables[0].Rows[0]["intCFEEnterpid"].ToString();
                                 int s = Gen.InsertPaymentDisbusmentSBI( dsch.Tables[0].Rows[0]["intCFEEnterpid"].ToString(),sonlinetrnsNo, HdfDeptid, TxtAmountOnline.Text,"Y"  , Session["uid"].ToString());
                     ...
                     if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                     {
                         ...
                         foreach (GridViewRow row in grdDetails.Rows)
                         {
                             if (((CheckBox)row.FindControl("ChkApproval")).Checked)
                             {
                                 ...
                                 {
                                     Hdfeid.Value = dsch.Tables[0].Rows[0]["intCFEEnterpid"].ToString();
                                     int s = Gen.InsertPaymentDisbusment(dsch.Tables[0].Rows[0]["intCFEEnterpid"].ToString(), sonlinetrnsNo, HdfDeptid, TxtAmountOnline.Text, "Y", Session["uid"].ToString());
