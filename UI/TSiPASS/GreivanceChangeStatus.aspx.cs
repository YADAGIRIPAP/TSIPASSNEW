...
public partial class TSTBDCReg1 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["intGrievanceid"] != "")
        {
            ...
            if (!IsPostBack)
            {
                ...
                DataSet ds = Gen.fetchgrievancedet("%", "%", "%", "%", intGrievanceid, "%", "%");
                if (ds.Tables[0].Rows.Count > 0)
                {
                    LblInd.Text = ds.Tables[0].Rows[0]["IndustryName"].ToString();
                    lblUIDText.Text = ds.Tables[0].Rows[0]["UIDNo"].ToString();
                    ...
                    lblDeptOfficeMailId.Text = ds.Tables[0].Rows[0]["OfficerEmailId"].ToString();
                    lblOfficerName.Text = ds.Tables[0].Rows[0]["OfficerName"].ToString();
                    ...
                    else
                    {
                        ...
                        trForward.Visible = false;

                        Labelcolun3.Visible = false;
                        //ddlforward.Visible = false;
                        ReqfvStatus.Visible = true;
                        reqfvRemarks.Visible = true;
                        if (ds.Tables[0].Rows[0]["Status"].ToString() == "Redressed" || ds.Tables[0].Rows[0]["Status"].ToString() == "Reject")
                        {
                    ...
                    {
                        lblFilePath.Text = ds.Tables[0].Rows[0]["Grivance_File_Path"].ToString();
                        lblFileName.Text = ds.Tables[0].Rows[0]["Grievnace_FileName"].ToString();
    ...
    protected void BtnSave_Click(object sender, EventArgs e)
    {
        ...
        string newPath = "";
        string sFileDir = Server.MapPath("~\\GreivanceReply");
        ...
        if (ddlstatus.SelectedItem.Text == "Reject")
        {
            ...
            {
                lblMsg.Text = "Please Upload Rejected Reason File";
                FileUpload.Focus();
                //BtnSave.Attributes.Add("onclick", "document.getElementById('" + FileUpload.ClientID + "').click()");
                FileUpload.Attributes["onchange"] = "UploadFile(this)";
                //btnBrowse.Attributes.Add("onclick", "document.getElementById('" + fileUp1.ClientID + "').click()");
                return;
            }
        }
        if (Request.QueryString.Count > 1)
        {
            //Grivance_File_Path, Grivance_File_Type, Grievnace_FileName
        ...
        //string FileNameofrMail = "";// = lblFilePath.Text + "\\" + lblFileName.Text;

        if (Request.QueryString["forward"] == null)
        {
            if (ddlstatus.SelectedItem.Text == "Redressed" || ddlstatus.SelectedItem.Text == "Reject")
            {
                string body = @"<div>
                            Dear " + LblInd.Text + "," + @"
                            <br/><br />
                            Grievance " + ddlstatus.SelectedItem.Text + ". <br/> Remarks : " + TxtRemarks.Text + "<br/><br/>Regards<br/>" + "TS-iPASS Cell,<br/>" + "Commissinerate of Industries," + " <br/> " + "Chirag-ali-lane, Abids, Hyderabad." + "<br/>" + "Phone:040-23441636" + "<br/>" + "Email id : asstdir.sw.inds@telangana.gov.in" + "<br/></div>";

                // SendEmail1(txtEmail.Text.Trim(), "tsipass.telangana@gmail.com", body, lblHead1.Text);

                string msgs = "Dear " + LblInd.Text + "\r\n" + "Grievance " + ddlstatus.SelectedItem.Text + ", with remarks " + TxtRemarks.Text;
                try
                {
                    //SendMailAnother(lblEmail.Text.Trim(), body, FileNameofrMail); --uncomment if required
                }
                catch (Exception ex)
                {
                ...
                try
                {
                    //cmf.SendSingleSMS(LblMob.Text, msgs);--uncomment if required
                }
                catch (Exception ex)
                {
        ...
        if (Request.QueryString["forward"] != null && Request.QueryString["forward"].ToString() == "Y")
        {
    ...
    public void SendMailAnother(string anothermail, string body, string FilePath)
    {
        ...
        string from = "tsipass.telangana@gmail.com"; //lblDeptOfficeMailId.Text; //Replace this with your own correct Gmail Address
    ...
    protected void btnforward_Click(object sender, EventArgs e)
    {
        ...
        int j = Gen.InsGrievStaus(intGrievanceid, "Forward", TxtRemarks.Text, Session["uid"].ToString(), ddlforward.SelectedValue, Grivance_File_Path, Grivance_File_Type, Grievnace_FileName, txtEmail.Text.Trim());

        lblMsg.Text = "Forwarded Successfully..!";
        Page.ClientScript.RegisterStartupScript(GetType(), "msgbox", "alert('Forwarded Successfully..!');", true);
        FileNameofrMail = lblFilePath.Text + "\\" + lblFileName.Text;
        string body = @"<div>
                            Sir/ Madam, " + @"
                            <br/><br />
                            Grievance Details.<br/><br/> <table><tr><td>Industry Name 	:</td><td>" + LblInd.Text + "</td></tr>";//</table> </div>"; //; +ddlstatus.SelectedItem.Text + " successfully, with remarks " + TxtRemarks.Text + "<br/><br/>Regards<br>TSiPASS</div>";
        body = body + "<tr><td>Email 	:</td><td>" + lblEmail.Text + "</td></tr>";
        body = body + "<tr><td>District Name 	:</td><td>" + LblDist.Text + "</td></tr>";
        body = body + "<tr><td>Mobile Number 	:</td><td>" + LblMob.Text + "</td></tr>";
        body = body + "<tr><td>Subject 	:</td><td>" + lblSub.Text + "</td></tr>";
        body = body + "<tr><td>Description 	:</td><td>" + LblDesc.Text + "</td></tr></table><br/><br/>Regards<br/>" + lblOfficerName.Text + "<br/>" + lblDeptOfficeMailId.Text + " </div>";
        ...
        try
        {
            //SendMailAnother(txtEmail.Text.Trim(), body, FileNameofrMail);  uncomment if required
        }
        catch (Exception ex)
        {
