...
public partial class UI_TSiPASS_IncentiveWiseDIPClist : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                pp[1].Value = Request.QueryString[1].ToString();
            }
            else pp[1].Value = null;

            if (Session["uid"].ToString() == "32951")
            {
                pp[2].Value = "32951";
            }
            else pp[2].Value = null;

            ds = gen.GenericFillDs("USP_GET_LIST_INCENTIVEWISE_ABSTRACT_NEW_DIPC", pp);
