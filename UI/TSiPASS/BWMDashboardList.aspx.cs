...
public partial class UI_TSiPASS_BWMDashboardList : System.Web.UI.Page
{
    ...
    public void getdetails()
    {
        try
        {
            ...
            oSqlDataAdapter = new SqlDataAdapter("USP_GET_BMWVALUES_DASHB0ARD_HISTORY", osqlConnection);
            ...
            if (Session["uid"] != null)
            {
                ...
                if (oDataSet.Tables.Count >= 1)
                {
                    if (oDataSet.Tables[0].Rows.Count == 0)
                    {
                        ...
                        Response.Redirect("Healthcareestablishment.aspx?status=B", false);
                    }
                    else
                    {
    ...
    protected void grdDetails_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            lblanchortaglinkPendingQueriesAtLevel.NavigateUrl = "frmBWMRedirectDashboard.aspx?id=" + lblRENINTQNREID.Text.Trim() +
                "&status=" + "Queries -Yet to Respond" + "&check=" + "A";

            anchortaglink.NavigateUrl = "BWMDashboard.aspx?Qnreid=" + lblRENINTQNREID.Text.Trim();
            ...
            if (lblanchortaghplRejectionAtLevel.Text != "0")
                ...
                lblanchortaghplRejectionAtLevel.NavigateUrl = "frmBWMRedirectDashboard.aspx?id=" + lblRENINTQNREID.Text.Trim() +
            ...
            if (lblApprovalStatus.Text == "3")
            {
                ...
                anchortaglinkStatus.NavigateUrl = "RptApplicationWiseDetailedTrakerBMW.aspx?intqnreid=" + intqnreid.ToString();
            }
            else if (lblApprovalStatus.Text == "2")
            {
                //Label lblRENINTQNREID = (e.Row.FindControl("lblRENINTQNREID") as Label);
                string intqnreid = lblRENINTQNREID.Text.ToString();
                anchortaglinkStatus.NavigateUrl = "Healthcareestablishment.aspx?status=B" + "&intqnreid=" + intqnreid.ToString();
    ...
    {
        Response.Redirect("Healthcareestablishment.aspx?status=A");
    }
}