...
public partial class UI_TSiPASS_frmstatusbytype1New : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                ds = Gen.RetriveStatusByType1New(status, type, ddlCategory.SelectedItem.Text, ddldistrict.SelectedValue, FromdateforDB, TodateforDB);
                ...
                HyperLink1.NavigateUrl = "frmR1ReportKMRNew.aspx?status=" + "" + "&lbl=Total Applications Received&fromdate=" + txtFromDate.Text + "&todate=" + txtTodate.Text;
    ...
    protected void BtnSave1_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            //else
            //    Label1.Text = "Report from 1st April 2016 to " + System.DateTime.Now.ToString("dd-MMM-yyyy");
            ////ds = Gen.RetriveStatusByType1(status, type, ddlCategory.SelectedItem.Text, ddldistrict.SelectedValue);
            //string fromdate = "", Todate = "";
            if (Request.QueryString["fromdate"] != null && Request.QueryString["fromdate"] != "" && Request.QueryString["todate"] != null && Request.QueryString["todate"] != "")
            {
                // Label1.Text = "Report from " + Request.QueryString["fromdate"].ToString().Trim() + " To " + Request.QueryString["todate"].ToString().Trim();
    ...
    //        string FileName = Request.QueryString[2].ToString().Trim() + Request.QueryString[1].ToString().Trim();
    ...
    protected void ExportToExcel()
    {
        try
        {
            ...
            using (StringWriter sw = new StringWriter())
            {
                ...
                string status = Request.QueryString[0].ToString().Trim();
                string type = Request.QueryString[1].ToString().Trim();
                //string fromdate = Request.QueryString[3].ToString().Trim();
                //string todate = Request.QueryString[4].ToString().Trim();
        ...
        try
        {
            using (StringWriter sw = new StringWriter())
            {
                using (HtmlTextWriter hw = new HtmlTextWriter(sw))
                {
                    ...
                    Response.AddHeader("content-disposition", "attachment;filename= R1.2TotalApplicationsReceivedApprovalStage.pdf");
