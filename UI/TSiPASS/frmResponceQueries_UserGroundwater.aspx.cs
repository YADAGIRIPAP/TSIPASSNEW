...
public partial class UI_TSiPASS_frmResponceQueries_UserGroundwater : System.Web.UI.Page
{
    comFunctions cmf = new comFunctions();
    Cls_OSGroundWater obj_gwdata = new Cls_OSGroundWater();
    ...
    void FillDetails()
    {
        ...
        try
        {
            ds = obj_gwdata.GetQueryStatusByTransactionID_Groundwater(Request.QueryString[0].ToString(), Convert.ToString(Session["uid"]),Convert.ToString(Request.QueryString[1]));
            if (Request.QueryString[0].ToString() != null)
            {
            ...
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                ...
                lbl_placeofrtoregistration.Text = ds.Tables[0].Rows[0]["Districtmandal"].ToString().Trim();
    ...
    protected void BtnClear_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            if (txtdiscription.Text == "" || hyp_resattach.Text == "") 
                //txtdiscription.Text == string.Empty) || (Label440.Text == "" || Label440.Text == string.Empty))
            {
            ...
            result = obj_gwdata.InsertQueryDetails_groundwater(intQuessionaireid, intCFEEnterpid, intDeptid, intApprovalid, ViewState["AttachmentName"].ToString(),
            ...
            int k = obj_gwdata.InsertDeptDateTracing_mrogroundwater(intDeptid, intQuessionaireid, UIDNO, null, null, null, System.DateTime.Now.ToString("MM/dd/yyyy"), null, "GW", intApprovalid);
            ...
            dsMail = obj_gwdata.GetShowEmailidandMobileNumber_MROGroundwater(intQuessionaireid);
            ...
            try
            {
                ...
                if (fileType[i - 1].ToUpper().Trim() == "PDF")
                {
                    ...
                    if (count == 0)
                    {
                        ...
                        result = obj_gwdata.InsertImagedataApproval_groundwater(hf_intcfeentrpid.Value.ToString(), fileType[i - 1].ToUpper(), newPath, sFileName, "Response Attachment", Session["uid"].ToString(), DateTime.Now.ToString(), Session["uid"].ToString(), DateTime.Now.ToString(), hf_intdeptid.Value.ToString(), hf_intapprovalid.Value.ToString());
                    ...
                    else
                    {
                        if (count > 0)
                        {
                            ...
                            result = obj_gwdata.InsertImagedataApproval_groundwater(hf_intcfeentrpid.Value.ToString(), fileType[i - 1].ToUpper(), newPath, FinalFileName, "Response Attachment", Session["uid"].ToString(), DateTime.Now.ToString(), Session["uid"].ToString(), DateTime.Now.ToString(), hf_intdeptid.Value.ToString(), hf_intapprovalid.Value.ToString());
                            hyp_resattach.Visible = true;
