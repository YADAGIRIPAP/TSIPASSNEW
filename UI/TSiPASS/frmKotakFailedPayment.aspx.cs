...
public partial class UI_TSiPASS_frmKotakFailedPayment : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            //ccavo.order_name = ""; ccavo.order_payment_type = ""; ccavo.order_type = ""; ccavo.order_currency = "";
            ...
            // ccavo.order_payment_type = "NEFT-RTGS";
            var JsonFrimDetails = JsonConvert.SerializeObject(ccavo);
            string orderStatusQueryJson = JsonFrimDetails;
            LogErrorFile.LogData(Convert.ToString("Request" + JsonFrimDetails.ToString()));
            //"{ \"reference_no\":\"103001198924\", \"order_no\":\"77141516\" }"; //Ex. { "reference_no":"CCAvenue_Reference_No" , "order_no":"123456"} 
            string encJson = "";

            string queryUrl = "https://login.ccavenue.com/apis/servlet/DoWebTrans";// "https://api.ccavenue.com/apis/servlet/DoWebTrans";

            CCACrypto ccaCrypto = new CCACrypto();
            encJson = ccaCrypto.Encrypt(orderStatusQueryJson, workingKey);
            //LogErrorFile.LogData(Convert.ToString("Request enc" + encJson.ToString()));
            // make query for the status of the order to ccAvenues change the command param as per your need
            string authQueryUrlParam = "enc_request=" + encJson + "&access_code=" + strAccessCode + "&command=orderLookup&request_type=JSON&response_type=JSON";
