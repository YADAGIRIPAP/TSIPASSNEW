...
public partial class UI_TSIPASS_StampDutyTransferdutyAnnex4 : System.Web.UI.Page
{
        ...
        if (!IsPostBack)
        {
            if (Session["uid"] != null)
            {
                if (Session["incentivedata"] != null)
                {
                    ...
                    DataRow[] drs = ds.Tables[0].Select("IncentiveID = " + 17);  // Reimbursement of stamp Duty/Transfer Duty/Mortgage/Land Conversion/Land Cost
                    if (drs.Length > 0)
                    {
                        DataSet dsnew = new DataSet();
                        dsnew = Gen.GetIncentivesStampDutydata4(IncentveID);
                        Filldata(dsnew);
                    }
                    else
                    {
                        ...
                        {
                            Response.Redirect("StampDutyTransferdutyAnnex3.aspx?Previous=" + "P");
                        }
                    }
                    ...
                    else
                    {
                        ...
                        lblheadTIDEA.Text = "<center>" + "ANNEXURE: VI" + "</center>" + "<center>APPLICATION CUM VERIFICATION FOR</center>" + "<span style='font-size: 14pt;'><b><u><center>REIMBURSEMENT OF LAND COST PURCHASED IN IE/IDA/IP’s UNDER  " + lblscheme.Text + "</center></u></b></span>" + "<center>(TELANGANA STATE INDUSTRIAL DEVELOPMENT AND ENTREPRENEUR ADVANCEMENT) INCENTIVE SCHEME 2014 </center>" + "<center>(G.O.Ms.No.28 Industries and Commerce (IP & INF) Department. dated.29/11/2014)</center>";
    ...
    protected void BtnSave_Click(object sender, EventArgs e)
    {
        if (lblAttachedFileName1.Text != "" && lblAttachedFileName2.Text != "")
        {
            if (save())
            {
                ...
                UpdateAnnexureflag(IncentiveId, "17", "Y", "");
                string message = "alert('Land Cost Applied Successfully')";
    ...
    public bool save()
    {
        try
        {
            ...
            string valid = Gen.InsertIncentStampDuty4(objvoA);
            ...
            if ((fuDocuments1.PostedFile != null) && (fuDocuments1.PostedFile.ContentLength > 0))
            {
                ...
                try
                {
                    ...
                    if (fileType[i - 1].ToUpper().Trim() == "PDF" || fileType[i - 1].ToUpper().Trim() == "DOC" || fileType[i - 1].ToUpper().Trim() == "JPG" || fileType[i - 1].ToUpper().Trim() == "XLS" || fileType[i - 1].ToUpper().Trim() == "XLSX" || fileType[i - 1].ToUpper().Trim() == "DOCX" || fileType[i - 1].ToUpper().Trim() == "ZIP" || fileType[i - 1].ToUpper().Trim() == "RAR" || fileType[i - 1].ToUpper().Trim() == "JPEG" || fileType[i - 1].ToUpper().Trim() == "PNG")
                    {
                        ...
                        newPath = System.IO.Path.Combine(sFileDir, Session["EntprIncentive"].ToString() + "\\17\\100016");
        ...
        if (fuDocuments2.HasFile)
        {
            if ((fuDocuments2.PostedFile != null) && (fuDocuments2.PostedFile.ContentLength > 0))
            {
                ...
                try
                {
                    ...
                    {
                        //Create a new subfolder under the current active folder
                        newPath = System.IO.Path.Combine(sFileDir, Session["EntprIncentive"].ToString() + "\\17\\100017");
    ...
    {
        Response.Redirect("StampDutyTransferdutyAnnex4.aspx");
    }
