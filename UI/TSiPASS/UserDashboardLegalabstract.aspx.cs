...
public partial class UI_TSiPASS_UserDashboardLegalabstract : System.Web.UI.Page
{
    ...
    public void getdetails()
    {
        try
        {
            ...
            oSqlDataAdapter = new SqlDataAdapter("USP_GET_LEGAL_DASHBOARD_HISTORY", osqlConnection);
            ...
            if (Session["uid"] != null)
            {
                ...
                if (oDataSet.Tables.Count >= 1)
                {
                    if (oDataSet.Tables[0].Rows.Count == 0)
                    {
                        ...
                        Response.Redirect("Frmverificationlegalmetrology.aspx?status=B", false);
                    }
                    else
                    {
    ...
    protected void grdDetails_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            //lblanchortaglinkPendingQueriesAtLevel.NavigateUrl = "frmDashboardRedirectRenewal.aspx?id=" + lblRENINTQNREID.Text.Trim() +
            //    "&status=" + "Queries -Yet to Respond" + "&check=" + "A";

            anchortaglink.NavigateUrl = "UserDashboardLegalMain.aspx?Qnreid=" + lblRENINTQNREID.Text.Trim();
            ...
            if (lblanchortaghplRejectionAtLevel.Text != "0")
                ...
                lblanchortaghplRejectionAtLevel.NavigateUrl = "LegalApprovalRejectpayment.aspx?id=" + lblRENINTQNREID.Text.Trim() +
            ...
            if (lblApprovalStatus.Text == "3")
            {
                ...
                anchortaglinkStatus.NavigateUrl = "RptApplicationwiseDetailedTrackerLM.aspx?intqnreid=" + intqnreid.ToString();////RptApplicationWiseDetailedTrakerLegal
            }
            else if (lblApprovalStatus.Text == "2")
            {
                //Label lblRENINTQNREID = (e.Row.FindControl("lblRENINTQNREID") as Label);
                string intqnreid = lblRENINTQNREID.Text.ToString();
                anchortaglinkStatus.NavigateUrl = "Frmverificationlegalmetrology.aspx?status=B" + "&intqnreid=" + intqnreid.ToString();
    ...
    {
        Response.Redirect("Frmverificationlegalmetrology.aspx?status=A");
    }
}