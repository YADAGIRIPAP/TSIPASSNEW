...
public partial class UI_TSiPASS_KotakMobileTowerTSROW : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (Session["UID_no"] != null && Session["onlinetransId"].ToString() != null)
            {
                ...
                dspaydtls = GetMobiletowerTSROWpaymentDtls(CustomerID, TSipassOrderNumber, additionalPayment);
    ...
    public DataSet GetMobiletowerTSROWpaymentDtls(string UID, string Orderno, string AdditionalPaymentflg)
    {
        DB.DB con = new DB.DB();
        con.OpenConnection();
        SqlDataAdapter da;
        ...
        try
        {
            da = new SqlDataAdapter("USP_GET_Builldesc_PaymentDtls_MobiletowerTSROW_DESE", con.GetConnection);
