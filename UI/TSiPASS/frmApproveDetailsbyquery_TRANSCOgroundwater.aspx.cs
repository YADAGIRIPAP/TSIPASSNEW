...
public partial class UI_TSiPASS_frmApproveDetailsbyquery_TRANSCOgroundwater : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (ds != null && ds.Tables.Count > 0)
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    string GWTRASCOApproved = Convert.ToString(ds.Tables[0].Rows[0]["GWTRASCOApproved"]);
                    string GWTRASCORejected = Convert.ToString(ds.Tables[0].Rows[0]["GWTRASCORejected"]);
                    string GWTRASCOQueryRaised = Convert.ToString(ds.Tables[0].Rows[0]["GWTRASCOQueryRaised"]);
                    string GWTRASCOQueryResponded = Convert.ToString(ds.Tables[0].Rows[0]["GWTRASCOQueryResponded"]);
                    if (!string.IsNullOrEmpty(stageid))
                    {
                        if (stageid == "12")
                        {
                            if (GWTRASCOQueryRaised == "Y" && GWTRASCOQueryResponded == "Y")
                            {
                    ...
                    if ((file_uploadcert.PostedFile != null) && (file_uploadcert.PostedFile.ContentLength > 0))
                    {
                        ...
                        try
                        {
                            ...
                            if (fileType[i - 1].ToUpper().Trim() == "PDF")
                            {
                                ...
                                {
                                    newPath = System.IO.Path.Combine(sFileDir, Request.QueryString[0].ToString() + "\\DistrictTRANSCORejectedDocument\\" + hf_intDeptid.Value);// + System.DateTime.Now.ToString("ddMMyyyyHHmmtt")
                                    LogErrorFile.LogData(newPath);
                                }
                                else
                                {
                                    newPath = System.IO.Path.Combine(sFileDir, Request.QueryString[0].ToString() + "\\DistrictTRANSCOApprovalDocument\\" + hf_intDeptid.Value + System.DateTime.Now.ToString("ddMMyyyyHHmmtt"));
                                }
                                ...
                                if (ddlStatus.SelectedValue == "16")
                                {
                                    result = obj_dashboard.InsertImagedataApproval_groundwater(hf_intQuessionaireid.Value, fileType[i - 1].ToUpper(), newPath, sFileName, "DistrictTRANSCORejectedDocument", Session["uid"].ToString(), DateTime.Now.ToString(), "1000", DateTime.Now.ToString(), hf_intDeptid.Value, hf_intApprovalid.Value);
                                    LogErrorFile.LogData(Convert.ToString(result));
                                }
                                else
                                {
                                    result = obj_dashboard.InsertImagedataApproval_groundwater(hf_intQuessionaireid.Value, fileType[i - 1].ToUpper(), newPath, sFileName, "DistrictTRANSCOApprovalDocument", Session["uid"].ToString(), DateTime.Now.ToString(), "1000", DateTime.Now.ToString(), hf_intDeptid.Value, hf_intApprovalid.Value);
                                }
            ...
            if (BtnSave1.Text == "Submit")
            {
                ...
                if (i != 999)
                {
                    ...
                    if (ddlStatus.SelectedValue == "13")
                    {
                        ...
                        {
                            cmf.SendMailTSiPASS(dsMail1.Tables[0].Rows[0]["Email"].ToString().Trim(), "Dear " + lbl_nameofapplicant.Text + " - (" + lbl_UIDNumber.Text + ") :<br/><br/> <b>" + "District TRANSCO Ground Water Officer" + " has issued an approval to your application.Please login to TS-iPASS to download your approval. Please click on this link " + '\n' + dsMail1.Tables[0].Rows[0]["LINK"].ToString().Trim() + '\n' + " to give feedback. Thank You.");
                        }
                        if (dsMail1.Tables[0].Rows[0]["MobileNo"].ToString().Trim() != "")
                        {
                            SendSingleSMS(dsMail1.Tables[0].Rows[0]["MobileNo"].ToString().Trim(), "Dear " + lbl_nameofapplicant.Text + " - (" + lbl_UIDNumber.Text + ") ," + "District  TRANSCO Ground Water Officer" + " has issued an approval to your application.Please login to TS-iPASS to download your approval. Please click on this link " + '\n' + dsMail1.Tables[0].Rows[0]["LINK"].ToString().Trim() + '\n' + " to give feedback. Thank You.");
                        }
                    ...
                    else if (ddlStatus.SelectedValue == "16")
                    {
                        if (dsMail1.Tables[0].Rows.Count > 0)
                        {
                            cmf.SendMailTSiPASS(dsMail1.Tables[0].Rows[0]["Email"].ToString().Trim(), "Dear " + lbl_nameofapplicant.Text + " - (" + lbl_UIDNumber.Text + ") :<br/><br/> <b>" + "District TRANSCO Ground Water Officer" + " has " + ddlStatus.SelectedItem.Text.ToString() + " your application.Please login to TS-iPASS Appeal for Rejection. Thank You.</b>");
                        }
                        if (dsMail1.Tables[0].Rows[0]["MobileNo"].ToString().Trim() != "")
                        {
                            cmf.SendSingleSMS(dsMail1.Tables[0].Rows[0]["MobileNo"].ToString().Trim(), "Dear " + lbl_nameofapplicant.Text + " - (" + lbl_UIDNumber.Text + ") ," + "District TRANSCO Ground Water Officer" + " has " + ddlStatus.SelectedItem.Text.ToString() + " your application.Please login to TS-iPASS Appeal for Rejection. Thank You.");
                        }
                        txt_FileReferenceNo.Text = "";
                        ddlStatus.SelectedIndex = 0;
                        txtremarks.Text = "";
                    ...
                    else if (ddlStatus.SelectedValue == "5")
                    {
                        if (dsMail1.Tables[0].Rows.Count > 0)
                        {
                            cmf.SendMailTSiPASS(dsMail1.Tables[0].Rows[0]["Email"].ToString().Trim(), "Dear " + lbl_nameofapplicant.Text + " - (" + lbl_UIDNumber.Text + ") :<br/><br/> <b>" + "District TRANSCO Ground Water Officer" + " has " + ddlStatus.SelectedItem.Text.ToString() + " has raised a query on your application. </b><br/><br/>Please respond to the query in your login in https://ipass.telangana.gov.in/. Thank You..</b>");
                        }
                        if (dsMail1.Tables[0].Rows[0]["MobileNo"].ToString().Trim() != "")
                        {
                            cmf.SendSingleSMS(dsMail1.Tables[0].Rows[0]["MobileNo"].ToString().Trim(), "Dear " + lbl_nameofapplicant.Text + " - (" + lbl_UIDNumber.Text + ") ," + "District TRANSCO Ground Water Officer" + " has " + ddlStatus.SelectedItem.Text.ToString() + " has raised a query on your application. </b><br/><br/>Please respond to the query in your login in https://ipass.telangana.gov.in/. Thank You..");
                        }
                        txt_FileReferenceNo.Text = "";
                        ddlStatus.SelectedIndex = 0;
                        txtremarks.Text = "";
