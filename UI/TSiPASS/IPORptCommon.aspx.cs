...
public partial class UI_TSiPASS_IPORptCommon : System.Web.UI.Page
{
                ...
                if (Session["uid"] != null)
                {
                    ...
                    if (dsmst != null && dsmst.Tables.Count > 0 && dsmst.Tables[0].Rows.Count > 0)
                    {
                        if (dsmst.Tables[0].Rows[0]["IncentiveName"].ToString() != null)
                        {
                            ...
                            txtAmountclaimed.Text = dsmst.Tables[0].Rows[0]["UnitClaimedAmount"].ToString();
    ...
    protected void BtnSave1_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            if (txtAmountclaimed.Text == "" && txtAmountclaimed.Text == string.Empty)
            {
                lblmsg0.Text = "<font color=Red>Please Enter Amount Claimed</font>";
                success.Visible = false;
                Failure.Visible = true;
               // BtnNext.Enabled = false;
                i = 1;
            }
            if (txtAmountrecommended.Text == "" && txtAmountrecommended.Text == string.Empty)
            {
                lblmsg0.Text = "<font color=Red>Please Enter Amount Recommended</font>";
                success.Visible = false;
                Failure.Visible = true;
               // BtnNext.Enabled = false;
                i = 1;
            ...
            if (i == 0)
            {
                ...
                {
                    valid = Gen.InsertIPORpt_Common(IncentiveId, MstIncentiveId, txtAmountclaimed.Text, txtAmountrecommended.Text, createdby, txtremarks.Text, rblDLCSLC.SelectedValue.ToString(), rdbCaste.SelectedValue.ToString(), rdbGender.SelectedValue.ToString(), rdbCategory.SelectedValue.ToString(), rdbEnterprise.SelectedValue.ToString(), rdbSector.SelectedValue.ToString(), rdbServiceType.SelectedValue.ToString(), rdbTransportNonTrans.SelectedValue.ToString());  //test 
                }
                else if (rdbSector.SelectedIndex == 1)
                {
                    valid = Gen.InsertIPORpt_Common(IncentiveId, MstIncentiveId, txtAmountclaimed.Text, txtAmountrecommended.Text, createdby, txtremarks.Text, rblDLCSLC.SelectedValue.ToString(), rdbCaste.SelectedValue.ToString(), rdbGender.SelectedValue.ToString(), rdbCategory.SelectedValue.ToString(), rdbEnterprise.SelectedValue.ToString(), rdbSector.SelectedValue.ToString(), "", "");  //test 
                }
                ...
                if (valid == "Y")
                {
                   ...
                   // lblmsg.Text = "<font color=Black>Inspection Report Submitted Sucessfully</font>";
