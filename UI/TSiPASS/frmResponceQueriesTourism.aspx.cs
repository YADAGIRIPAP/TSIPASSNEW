...
public partial class UI_TSiPASS_frmResponceQueriesTourism : System.Web.UI.Page
{
    ...
    public DataSet GetQueryStatusByTransactionIDTourism(string User_id)
    {
        ...
        try
        {
            da = new SqlDataAdapter("sp_GetQueryStatusByTransactionIDTourism", con);
    ...
    protected void BtnClear_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (result > 0)
            {
                ...
                //        dsdeptquery = GetQueryStatusByTransactionIDTourism(intQuessionaireid, "55");
                ...
                //    {
                //    }
                //} //commented end lavanya 03.11.2020
                ...
                Response.Redirect("TourismDashBoard.aspx?Qnreid=" + Session["Applid"].ToString());
                //this.Clear();
    ...
    public int InsertQueryDetailsTourism(string intEnterpreniourApprovalid,
...
string Modified_by, string Modified_dt, string IPAddress)
    {
        try
        {
            ...
            myDataAdapter = new SqlDataAdapter("sp_InsertQueryDetailsTourismEvent", con );
    ...
    protected void Button6_Click(object sender, EventArgs e)
    {
        ...
        string sFileDir = Server.MapPath("~\\TOURISMEVENT");
        ...
        if ((FileUpload1.PostedFile != null) && (FileUpload1.PostedFile.ContentLength > 0))
        {
            ...
            try
            {
                ...
                if (fileType[i - 1].ToUpper().Trim() == "PDF" || fileType[i - 1].ToUpper().Trim() == "DOC" || fileType[i - 1].ToUpper().Trim() == "JPG" || fileType[i - 1].ToUpper().Trim() == "XLS" || fileType[i - 1].ToUpper().Trim() == "XLSX" || fileType[i - 1].ToUpper().Trim() == "DOCX" || fileType[i - 1].ToUpper().Trim() == "ZIP" || fileType[i - 1].ToUpper().Trim() == "RAR" || fileType[i - 1].ToUpper().Trim() == "DWG")
                {
                    ...
                    result = SaveAttachments(Session["Applid"].ToString(), fileType[i - 1].ToUpper(), newPath, sFileName, "ResponseAttachment", Session["uid"].ToString(), hdfFlagID1.Value.ToString(), hdfFlagID2.Value.ToString());
    ...
    public int SaveAttachments(string intTourismId, string FileType, string FilePath, string FileName, string FileDescription, string Created_by, string deptid, string approvalid)
    {
        ...
        try
        {
            ...
            connew.OpenConnection();

            SqlCommand cmdFiles = new SqlCommand("usp_Insert_TourismEvent_attachments", connew.GetConnection);
