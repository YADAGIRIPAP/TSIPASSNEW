...
public partial class TSTBDCReg1 : System.Web.UI.Page
{
    ...
    protected void BtnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (BtnSave1.Text == "Submit")
            {
                ...
                int i = Gen.insertApprovalDataCFO(Request.QueryString[0].ToString(), txtGeo_Cordinate_Latitude.Text, ddlStatus.SelectedValue.ToString(), Session["uid"].ToString(), hdfFlagID1.Value, hdfFlagID2.Value, txtremarks.Text, ""); //fix by CMS by passing string IPaddress
    ...
    void FillDetails()
    {
        ...
        try
        {
            ...
            ds = Gen.GetdataofApprovaldataAprovalCFOIndus(Request.QueryString[0].ToString());
                ...
                if ((FileUpload1.PostedFile != null) && (FileUpload1.PostedFile.ContentLength > 0))
                {
                    ...
                    try
                    {
                        ...
                        if (fileType[i - 1].ToUpper().Trim() == "PDF" || fileType[i - 1].ToUpper().Trim() == "DOC" || fileType[i - 1].ToUpper().Trim() == "JPG" || fileType[i - 1].ToUpper().Trim() == "XLS" || fileType[i - 1].ToUpper().Trim() == "XLSX" || fileType[i - 1].ToUpper().Trim() == "DOCX" || fileType[i - 1].ToUpper().Trim() == "ZIP" || fileType[i - 1].ToUpper().Trim() == "RAR" || fileType[i - 1].ToUpper().Trim() == "DWG")
                        {
                            ...
                            //result = t1.InsertCFOAttachement(Session["Applid"].ToString(), Request.QueryString[0].ToString(), "1", "1", fileType[i - 1].ToUpper(), sFileName, newPath, "A", Session["uid"].ToString(), "ApprovalDocument");// Session["uid"].ToString()


                            result = t1.InsertCFOAttachementApproval(Session["Applid"].ToString(), Session["uid"].ToString(), "1", "1", fileType[i - 1].ToUpper(), sFileName, newPath, "A", Session["uid"].ToString(), "ApprovalDocument", hdfFlagID2.Value, hdfFlagID1.Value);
