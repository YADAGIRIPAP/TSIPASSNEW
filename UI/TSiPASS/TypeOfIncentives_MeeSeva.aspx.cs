...
public partial class TSTBDCReg1 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                btnPrevious.PostBackUrl = "~/UI/TSiPASS/Incentives_Meeseva.aspx?enc=" + (Session["enc"] != null ? Session["enc"].ToString() : "");
                ...
                if (Session["Incentive_isVehicle"] != null && Convert.ToBoolean(Session["Incentive_isVehicle"])) rblVehicleIncetive.SelectedValue = "0";                
                BindIncentives();
            }
        }
    ...
    //        if (Session["Incentive_DateOfCommencement"] != null)
    //        {
    //            DateTime dcp = Convert.ToDateTime(Session["Incentive_DateOfCommencement"].ToString());
    //            //Response.Write(dcp.ToString());
    //            //if (dcp.AddYears(5).Year>DateTime.Now.Year)
    //            //{
    //            //    int year = dcp.Year;
    //            //    int month = dcp.Month;
    ...
    //            //        if (month >= 10)
    //            //        {
    //            //            dt.Rows.Add("October-" + year.ToString());
    //            //        }
    //            //        else
    //            //        {
    //            //            dt.Rows.Add("October-" + year.ToString());
    //            //            dt.Rows.Add("April-" + (year - 1).ToString());
    //            //        }
    //            //    }
    //            //    else
    //            //    {
    //            //        if (month >= 10)
    //            //        {
    //            //            dt.Rows.Add("October-" + (year + 1).ToString());
    //            //        }
    //            //        else
    //            //        {
    //            //            dt.Rows.Add("April-" + year.ToString());
    //            //            dt.Rows.Add("October-" + (year + 1).ToString());
    //            //        }
    //            //    }

    //                for (int yr = dcp.Year; yr < DateTime.Now.Year; yr++)
    //                {
    //                    int year = dcp.Year;
    //                    int month = dcp.Month;
    //                    if (month <= 3)
    ...
    //                if (month >= 10)
    //                {
    //                    dt.Rows.Add("September-" + year.ToString());
    //                }
    //                else
    //                {
    //                    dt.Rows.Add("September-" + year.ToString());
    //                    dt.Rows.Add("March-" + (year - 1).ToString());
    //                }
    //            }
    //            else
    //            {
    //                if (month >= 10)
    //                {
    //                    dt.Rows.Add("September-" + (year + 1).ToString());
    //                }
    //                else
    //                {
    //                    dt.Rows.Add("September-" + year.ToString());
    ...
    //            //{
    //            //    dt.Rows.Add("September-" + (year - 1).ToString());
    //            //    dt.Rows.Add("March-" + year.ToString());
    //            //}
    //            //else
    //            //{
    //            //    dt.Rows.Add("September-" + year.ToString());
    ...
    protected void gvRepetitive_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                //Convert.ToString(DataBinder.Eval(e.Row.DataItem, "intHousingID")).Trim()
        ...
        try
        {
            ...
            foreach (GridViewRow gr in gvRepetitive.Rows)
            {
                if ((gr.FindControl("cbIncentiveCheck") as CheckBox).Checked == true)
                {
                    ...
                    if (dtto > dcp.AddYears(6))
                    {
                        Failure.Visible = true;
                        txtTo.Focus();
                        lblmsg0.Text = "From date cannot be Greater than (" + dcp.AddYears(6).ToString("dd-MMM-yyyy") + ")";
                        break;
                    }
            ...
            if (dt != null && dt.Rows.Count > 0 && rblSelection.SelectedIndex > -1)
            {
                ...
                btnNext.Visible = true;
                btnNext.PostBackUrl = "~/UI/TSiPASS/IncentiveSingleUploads_MeeSeva.aspx?q=" + dtIncentiveTypes.Rows[0]["IncentiveType"].ToString();
    ...
    protected void txtFrom_TextChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            try
            {
                ...
                if (dtFrom < dt || dtFrom > dt.AddYears(6))
                {
                    Failure.Visible = true;
                    lblmsg0.Text = "Please select Date between " + dt.ToString("dd-MMM-yyyy") + " and " + dt.AddYears(6).ToString("dd-MMM-yyyy");
                    System.Web.UI.ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "AlertBox", "alert('Please select Date between '" + dt.ToString("dd-MMM-yyyy") + " and " + dt.AddYears(6).ToString("dd-MMM-yyyy") + "');", true);
                }
            }
            catch (Exception)
            {
    ...
    protected void txtTo_TextChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            try
            {
                ...
                else if (dtTo > dt.AddYears(6))
                {
                    Failure.Visible = true;
                    lblmsg0.Text = "To Date cannot be greater than " + dtfrom.AddYears(6).ToString("dd-MMM-yyyy");
                }
            }
            catch (Exception)
            {
