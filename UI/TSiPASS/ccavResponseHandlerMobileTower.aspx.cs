...
public partial class UI_TSiPASS_ccavResponseHandlerMobileTower : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (msg != null)
        {
            ...
            if (paymentresponseVOsobj.AdditionalInfo4_19 == "NODEPT")
            {
                ...
                {
                    dspaydtls = GetEnterprinerpaymentDtlsMobileTower(paymentresponseVOsobj.CustomerID_1, paymentresponseVOsobj.AdditionalInfo2_17, paymentresponseVOsobj.AdditionalInfo5_20);
                ...
                else
                {
                       ...
                       dspaydtls = GetEnterprinerpaymentDtlsMobileTower(paymentresponseVOsobj.CustomerID_1, paymentresponseVOsobj.AdditionalInfo2_17, "");
                    //Departmentdetails (Questionnaire id#Entrepreneur id1#Department id1#TSIPASSUID#amount1#ApprovalId)
                    paymentresponseVOsobj.Createdby = dspaydtls.Tables[1].Rows[0]["CREATEDBY"].ToString();
                    if (dspaydtls != null && dspaydtls.Tables.Count > 1 && dspaydtls.Tables[1].Rows.Count > 0)
                    {
            ...
            {
                int valid = InsertUpdatepaymentdtls_MobileTower(paymentresponseVOsobj, paymendepartwisetresponseVOsobj);
                try
                {
                    GetTESTVALUESMobileTower(BilldeskResponse);
                }
                catch (Exception ex)
                {
                ...
                if (valid == 1)
                {
                    ...
                    Page.ClientScript.RegisterStartupScript(this.GetType(), "alertMsg", "alert('Payment Done Successfully');" + "window.location='IpassPrintReceiptMobileTower.aspx';", true);
                }
            }
            else
            {
    ...
    public void GetTESTVALUESMobileTower(string Responce)
    {
        DB.DB con = new DB.DB();
        con.OpenConnection();
        SqlDataAdapter da;
        ...
        try
        {
            da = new SqlDataAdapter("USP_TEST_WEBSERVISE_MobileTower", con.GetConnection);
            da.SelectCommand.Parameters.Add("@responce", SqlDbType.VarChar).Value = Responce.ToString();
            da.SelectCommand.Parameters.Add("@Online", SqlDbType.VarChar).Value = "Online";
    ...
    public int InsertUpdatepaymentdtls_MobileTower(paymentresponseVOs paymentresponseVOsobj, List<paymendepartwisetresponseVOs> paymendepartwisetresponseVOsobj)
    {
        ...
        SqlCommand command = new SqlCommand("USP_UPDATE_Builldesc_PaymentDtls_MobileTower", connection);
        ...
        try
        {
            ...
            if (valid == 1)
            {
                ...
                {
                    command1 = new SqlCommand("USP_UPDATE_DEPTWISE_Builldesc_PaymentDtls_MobileTower", connection);
    ...
    public DataSet GetEnterprinerpaymentDtlsMobileTower(string UID, string Orderno, string AdditionalPaymentflg)
    {
        DB.DB con = new DB.DB();
        con.OpenConnection();
        SqlDataAdapter da;
        ...
        try
        {
            da = new SqlDataAdapter("USP_GET_Builldesc_PaymentDtls_DESE_MobileTower", con.GetConnection);
            da.SelectCommand.CommandType = CommandType.StoredProcedure;
