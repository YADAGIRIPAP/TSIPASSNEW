...
public partial class UI_TSiPASS_QueryChangeStatus : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["intGrievanceid"] != "")
        {
            ...
            if (!IsPostBack)
            {
                ...
                DataSet ds = Gen.fetch_FeedbackQuerydet("%", "%", "%", "%", intGrievanceid, "%", "%","Q");
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (ds.Tables[0].Rows[0]["hd"].ToString() != "NA")
                        Label5.Text = "id:" + ds.Tables[0].Rows[0]["hdid"].ToString() + "Desc:" + ds.Tables[0].Rows[0]["hddesc"].ToString()
                            + "Rem:" + ds.Tables[0].Rows[0]["hd"].ToString();
                    else if (ds.Tables[0].Rows[0]["hd"].ToString() == "NA")

                        Label5.Text = "NA".ToString();
                    ...
                    else
                    {
                        ...
                        if (ds.Tables[0].Rows[0]["Status"].ToString() == "Attended" || ds.Tables[0].Rows[0]["Status"].ToString() == "Rejected")
                        {
                    ...
                    if (ds.Tables[0].Rows[0]["QueryReply_File_Path"].ToString() != null && ds.Tables[0].Rows[0]["QueryReply_File_Path"].ToString() != "")
                    {
                        if (!Directory.Exists(ds.Tables[0].Rows[0]["QueryReply_File_Path"].ToString()))
                        {
                            lblFileName1.Text = "No Attachment";
                        }
                        else
                        {
                            string sen, sen1, sen2;
                            sen2 = ds.Tables[0].Rows[0]["QueryReply_File_Path"].ToString();
                            sen1 = sen2.Replace(@"\", @"/");
                            sen1 = sen1 + "/" + ds.Tables[0].Rows[0]["QueryReply_FileName"].ToString();
                            sen = sen1.Replace(@"D:/TS-iPASSFinal/", "~/");
                            lblFileName1.NavigateUrl = sen;
                            lblFileName1.Text = ds.Tables[0].Rows[0]["QueryReply_FileName"].ToString();
                        }
                    }
                }
            }
    ...
    protected void BtnSave_Click(object sender, EventArgs e)
    {
        ...
        string newPath = "";
        string sFileDir = Server.MapPath("~\\GeneralQuery");
        ...
        Page.ClientScript.RegisterStartupScript(GetType(), "msgbox", "alert('Replied Successfully..!');", true);
        if (ddlstatus.SelectedItem.Text == "Attended" || ddlstatus.SelectedItem.Text == "Rejected")
        {
            string body = @"<div>
                            Dear " + LblInd.Text + "," + @"
                            <br/><br />
                            Query " + ddlstatus.SelectedItem.Text + " . <br/>Remarks " + TxtRemarks.Text + "<br/><br/>" + "TS-iPASS Cell,<br/>" + "Commissinerate of Industries," + " <br/> " + "Chirag-ali-lane, Abids, Hyderabad." + "<br/>" + "Phone:040-23441636" + "<br/>" + "Email id : asstdir.sw.inds@telangana.gov.in" + "<br/></div>";

            // SendEmail1(txtEmail.Text.Trim(), "tsipass.telangana@gmail.com", body, lblHead1.Text);

            string msgs = "Dear " + LblInd.Text + "\r\n" + "Query " + ddlstatus.SelectedItem.Text + ". Remarks " + TxtRemarks.Text;
            try
            {
