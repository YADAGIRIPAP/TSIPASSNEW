...
public partial class UI_TSiPASS_frmSanctionedReleasedDrilldown : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                string FinYear = Request.QueryString[0].ToString().Trim();
                string CASTE = Request.QueryString[1].ToString().Trim();
                // string Type = Request.QueryString[2].ToString().Trim();
                
                SqlConnection con = new SqlConnection(conString);
                con.Open();
                SqlDataAdapter da = new SqlDataAdapter("USP_GET_INCENTIVES_SANCTIONED_RELEASEPENDING_YEARWISE_Drilldown", con);
                da.SelectCommand.CommandType = CommandType.StoredProcedure;
                da.SelectCommand.Parameters.AddWithValue("@fyear", SqlDbType.VarChar).Value = FinYear;
                da.SelectCommand.Parameters.AddWithValue("@CASTE", SqlDbType.VarChar).Value = CASTE;
