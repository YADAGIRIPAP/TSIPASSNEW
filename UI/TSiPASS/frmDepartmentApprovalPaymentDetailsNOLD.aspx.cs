    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (dscheck.Tables[0].Rows.Count > 0)
            {
               ...
               // Session["UIDNO"] = dscheck.Tables[0].Rows[2]["UID_No"].ToString().Trim();
            }
            else
            {
                Session["Applid"] = "0";
        ...
        else
        {
            ...
            //generating 
             ...
             if (rblPaymentMode.SelectedIndex== 0)
             {
                 ...
                 foreach (GridViewRow row in grdDetails.Rows)
                 {
                     ...
                     else
                     {
                         ...
                         string HdfApprovalid = ((HiddenField)row.FindControl("HdfApprovalid")).Value.ToString();
                         string HdfAmount = ((HiddenField)row.FindControl("HdfAmount")).Value.ToString();
                         int j = Gen.UpdateAdditionalpayments(HdfQueid, "", "Completed", Session["uid"].ToString(), "2", HdfDeptid, HdfApprovalid, ""); //fix by CMS by passing string IPaddress
             ...
             else
             {
                 ...
                 Session["Amount"] = Convert.ToDecimal( TxtAmountOnline.Text).ToString("f2");
                 ...
                 if (rdbOnlineBankType.SelectedIndex == 0)// SBI
                 {
                     ...
                     foreach (GridViewRow row in grdDetails.Rows)
                     {
                         if (((CheckBox)row.FindControl("ChkApproval")).Checked)
                         {
                             ...
                             {
                                 Hdfeid.Value = dsch.Tables[0].Rows[0]["intCFEEnterpid"].ToString();
                                 int s = Gen.InsertPaymentDisbusmentSBI( dsch.Tables[0].Rows[0]["intCFEEnterpid"].ToString(),sonlinetrnsNo, HdfDeptid,HdfAmount,"Y"  , Session["uid"].ToString());
                     ...
                     if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                     {
                         ...
                         foreach (GridViewRow row in grdDetails.Rows)
                         {
                             if (((CheckBox)row.FindControl("ChkApproval")).Checked)
                             {
                                 ...
                                 {
                                     Hdfeid.Value = dsch.Tables[0].Rows[0]["intCFEEnterpid"].ToString();
                                     int s = Gen.InsertPaymentDisbusment(dsch.Tables[0].Rows[0]["intCFEEnterpid"].ToString(), Session["IcicitransId"].ToString().Trim(), HdfDeptid, HdfAmount, "Y", Session["uid"].ToString());
            ...
            if ((FileUpload1.PostedFile != null) && (FileUpload1.PostedFile.ContentLength > 0))
            {
                ...
                try
                {
                    ...
                    if (fileType[i - 1].ToUpper().Trim() == "PDF" || fileType[i - 1].ToUpper().Trim() == "DOC" || fileType[i - 1].ToUpper().Trim() == "JPG" || fileType[i - 1].ToUpper().Trim() == "XLS" || fileType[i - 1].ToUpper().Trim() == "XLSX" || fileType[i - 1].ToUpper().Trim() == "DOCX" || fileType[i - 1].ToUpper().Trim() == "ZIP" || fileType[i - 1].ToUpper().Trim() == "RAR" || fileType[i - 1].ToUpper().Trim() == "DWG")
                    {
                        ...
                        newPath = System.IO.Path.Combine(sFileDir, Request.QueryString[0].ToString() + "\\DD Upload");
                        ...
                        FileUpload1.PostedFile.SaveAs(newPath + "\\" + sFileName);
                        int result = 0;
                        result = t1.InsertImagedata(Session["Applid"].ToString(), Request.QueryString[0].ToString(), fileType[i - 1].ToUpper(), newPath, sFileName, "DD Upload", "", Session["uid"].ToString(), DateTime.Now.ToString(), "1000", DateTime.Now.ToString());
