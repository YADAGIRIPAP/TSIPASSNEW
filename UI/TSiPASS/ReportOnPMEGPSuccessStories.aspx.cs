...
public partial class UI_TSiPASS_ReportOnPMEGPSuccessStories : System.Web.UI.Page
{
    ...
    {
        Response.Redirect("~/UI/TSIPASS/ReportOnPMEGPSuccessStories.aspx");
    }
    protected void binddata(string fromdate, string todate, string DistrictID)
    {
        try
        {
            ...
            con.OpenConnection();
            SqlDataAdapter da = new SqlDataAdapter("USP_GetPMEGPSuccessStoriesCount", con.GetConnection);
    ...
    protected void grdsupport_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        {
            HyperLink hcount = (HyperLink)e.Row.Cells[2].Controls[0];
            hcount.NavigateUrl = "PMEGPSuccessStoriesDetails.aspx?DistrictId=" + Convert.ToString(DataBinder.Eval(e.Row.DataItem, "Districtid")).Trim()+"& Form="+txtfrmdate.Text+"& To="+txttodate.Text;
        }
    }
