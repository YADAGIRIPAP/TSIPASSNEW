...
public partial class UI_TSiPASS_frmSteamPipelineInspectUpload : System.Web.UI.Page
{
    int delete = 0;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            Response.Redirect("frmSteamPipelineInspectUpload.aspx");
        }
        ...
        if (!IsPostBack)
        {
            ...
            dsnew = Gen.getdataofidentityCFONewApproval(Session["ApplidA"].ToString(), "68");
            ...
            else
            {
                if (Request.QueryString[1].ToString() == "N")
                {
                    ...
                    Response.Redirect("frmFiredetailsCFO.aspx?intApplicationId=" + Session["uid"].ToString() + "&next=" + "N");

                }
                else
                {
                    Response.Redirect("frmCAFSteamPipeline.aspx?intApplicationId=" + Session["uid"].ToString() + "&Previous=" + "P");
            ...
            if (dsnew.Tables[0].Rows.Count > 0)
            {
                ...
                while (i < c)
                {
                    ...
                    if (sen1.Contains("CFOAttachments"))
                    {
                        ...
                        if (sen.Contains("SteamPipelineErectionPermissionAttachment"))
                        {
    ...
    protected void BtnClear_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            ds = Gen.GetBoilerUploadDetailsCFO(Request.QueryString[0].ToString(), "68");
        ...
        try
        {
            ...
            if (payment != "Y")
            {
                ...
                {
                    int quedata = Gen.updatenonPaymentinCFOQueNew(dslat.Tables[0].Rows[0]["intQuessionaireCFOid"].ToString(), Session["uid"].ToString(), "68", "0.00");
                }
                result = 1;
                    ...
                    if (intApprovalid == "68")
                    {
                        ...
                        if (dsdept != null && dsdept.Tables.Count > 0 && dsdept.Tables[0].Rows.Count > 0)
                        {
                            ...
                            if (boilerout == "SUCCESS")
                            {
                                ...
                                try
                                {
                                    int k = Gen.InsertDeptDateTracingCFO(intDeptid, intQuessionaireid, Label447.Text.Trim(), null, System.DateTime.Now.ToString("MM/dd/yyyy"), null, null, null, "CFO", intApprovalid);
                                }
                                catch (Exception ex)
                                {
        ...
        if ((FileUpload1.PostedFile != null) && (FileUpload1.PostedFile.ContentLength > 0))
        {
            ...
            try
            {
                ...
                if (fileType[i - 1].ToUpper().Trim() == "PDF" || fileType[i - 1].ToUpper().Trim() == "DOC" || fileType[i - 1].ToUpper().Trim() == "JPG" || fileType[i - 1].ToUpper().Trim() == "XLS" || fileType[i - 1].ToUpper().Trim() == "XLSX" || fileType[i - 1].ToUpper().Trim() == "DOCX" || fileType[i - 1].ToUpper().Trim() == "ZIP" || fileType[i - 1].ToUpper().Trim() == "RAR" || fileType[i - 1].ToUpper().Trim() == "DWG")
                {
                    ...
                    newPath = System.IO.Path.Combine(sFileDir, Session["uid"].ToString() + "\\SteamPipelineErectionPermissionAttachment\\" + hdfFlagID1.Value);
                    ...
                    result = t1.InsertCFOAttachement(Session["Applid"].ToString(), Session["uid"].ToString(), "1", "1", fileType[i - 1].ToUpper(), sFileName, newPath, "A", Session["uid"].ToString(), "SteamPipelineErectionPermissionAttachment");
    ...
    protected void btnnext_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (Label453.Text == "")
            {
                Label453.Text = Label453.Text.ToString();
                lblmsg0.Text = "<font color='red'>Please Upload Errection Attachment..!</font>";
                success.Visible = false;
                Failure.Visible = true;
                return;
    ...
    {
        Response.Redirect("frmSteamPipelineInspectUpload.aspx" + Session["uid"].ToString());
    }
}