...
public partial class TSTBDCReg1 : System.Web.UI.Page
{
    ...
    void FillDetails()
    {
        ...
        try
        {
            ...
            ds = Gen.getApprovalDocumentDetailsCFO(txtUID.Text, ddlApproval.SelectedValue);
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                lblApproval.Text = ds.Tables[0].Rows[0]["ApprovalName"].ToString().Trim();
                ...
                hdfFlagID0.Value = ds.Tables[0].Rows[0]["intCFOEnterpid"].ToString();
            ...
            if (FileUpload1.HasFile)
            {
                if ((FileUpload1.PostedFile != null) && (FileUpload1.PostedFile.ContentLength > 0))
                {
                    ...
                    try
                    {
                        ...
                        if (fileType[i - 1].ToUpper().Trim() == "PDF" || fileType[i - 1].ToUpper().Trim() == "DOC" || fileType[i - 1].ToUpper().Trim() == "JPG" || fileType[i - 1].ToUpper().Trim() == "JPEG" || fileType[i - 1].ToUpper().Trim() == "DOCX" || fileType[i - 1].ToUpper().Trim() == "RAR")
                        {
                            //Create a new subfolder under the current active folder
                            newPath = System.IO.Path.Combine(sFileDir, hdfFlagID0.Value + "\\ApprovalDocument\\" + hdfFlagID2.Value + System.DateTime.Now.ToString("ddMMyyyyHHmmtt"));
                            ...
                            int result = 0;
                            result = t1.InsertCFOAttachementApproval(hdfFlagID3.Value, hdfFlagID0.Value, "1", "1", fileType[i - 1].ToUpper(), sFileName, newPath, "A", Session["uid"].ToString(), "ApprovalDocument", hdfFlagID2.Value, hdfFlagID1.Value);
    ...
    private void getApprovalbyUIDCFO()
    {
        try
        {
            DataSet dsnew = new DataSet();

            dsnew = Gen.getApprovalbyUIDCFO(txtUID.Text);
    ...
    protected void BtnSubmit_Click(object sender, EventArgs e)
    {
        try
        {
            if (txtUID.Text != "")
            {
                ...
                {
                    Label2.Text = "Please Select Approval";
                }

            }
            else
            {
                Label2.Text = "Please Enter UID";
            }
        }
        catch (Exception ex)
        {
