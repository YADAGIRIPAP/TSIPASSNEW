...
public partial class UI_TSIPASS_ApplicantIncentiveDtlsDraftView : System.Web.UI.Page
{
    DB.DB con = new DB.DB();
            ...
            if (Session["uid"] != null)
            {
                ...
                try
                {
                    ...
                    if (!IsPostBack)
                    {
                        ...
                        //   obcmf.FillGrid(objFetch.FetchIncetiveUploadsViewNewINCType(Convert.ToInt32(Request.QueryString["EntrpId"].ToString()), 0), gvAttachments, false);

                        string usercode = "";
                        if (Role_Code == "COI-AD" || Role_Code == "COI-DD" || Role_Code == "COI-SUPDT" || Role_Code == "COI-CLERK" || Session["uid"].ToString() == "34668"
                            || Session["uid"].ToString() == "324136" || Role_Code == "SVCDEPARTMENTS"|| Session["uid"].ToString() == "1213")
                        {
                            Userid = "3377";
                            if (Session["uid"].ToString() == "324136" || Session["uid"].ToString() == "1213")
                            {
                                Role_Code = "JD";
                            }
                        ...
                        try
                        {
                            ...
                            if (Role_Code == "GM" || Session["uid"].ToString() == "34670" || Session["uid"].ToString() == "34668" || Session["uid"].ToString() == "SVCDEPARTMENTS"
                                || Session["uid"].ToString() == "21345" )
                            {
                            ...
                            if (Role_Code == "JD" || Role_Code == "COMM" || Role_Code == "COI-AD" || Role_Code == "COI-DD" || Role_Code == "COI-SUPDT" || Role_Code == "COI-CLERK" || Session["uid"].ToString() == "34668" || Role_Code == "SVCDEPARTMENTS" || Session["uid"].ToString() == "21345" || Session["uid"].ToString() == "1213" )
                            {
                            ...
                            if (Role_Code == "ADDL"|| Role_Code == "COMM")
                            {
                                if (ds != null && ds.Tables.Count > 1 && ds.Tables[1].Rows.Count > 0)
                                {
                                    BindingGmRecommendedIncentives(ds.Tables[1]);
            ...
            if (ds != null && ds.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < ds.Rows.Count; i++)
                {
                    ...
                    if (ds.Rows[i]["GMApprovalSentTo"].ToString() != "" && ds.Rows[i]["GMStatusid"].ToString() == "2")
                    {
                        ...
                        if (lblMstIncentiveId.Text == "18") // Coal
                        {
                            (gvinspectionOfficer.Rows[i].FindControl("anchortagGMCertificate") as HyperLink).NavigateUrl = "ProformaCoal.aspx?incentiveid=" + enterid.Text.Trim();
                        ...
                        if (lblMstIncentiveId.Text == "19") // Wood
                        {
                            (gvinspectionOfficer.Rows[i].FindControl("anchortagGMCertificate") as HyperLink).NavigateUrl = "ProformaWood.aspx?incentiveid=" + enterid.Text.Trim();
                        ...
                        if (lblMstIncentiveId.Text == "20") // TRANSPORT DUTY
                        {
                            (gvinspectionOfficer.Rows[i].FindControl("anchortagGMCertificate") as HyperLink).NavigateUrl = "ProformaTransportDuty.aspx?incentiveid=" + enterid.Text.Trim();
                        }
    ...
    protected void Button3_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                //valid = Gen.InsertincentiveOfficerCommentsjD(lstremarks, lstincentives, getclientIP());
                valid = Gen.InsertincentiveOfficerCommentsjD(lstremarks, lstincentives, getclientIP(), "", "", "", "");
                // added on 18.05.2018 
        ...
        try
        {
            ...
            foreach (GridViewRow gvrow in gvdicinspection.Rows)
            {
                ...
                try
                {
                    ...
                    //  cmf.SendSingleSMS(txtunitmobileno.Text, "Dear Sir, Incentive Inspection is scheduled on " + date1.ToString("dd-MMMM-yyyy") + ", and your assigned Inspecting Officer is " + deptIPOName + " and contact number is " + deptIPOMoileNO + ". Please make available mandatory documents at the Unit Location.a detail mail is sent to " + nameuid + ". Thank You, GM, DIC " + ddldistrictunit.Text + " Govt. of Telangana.");
                }
                catch (Exception ex)
                {
                    LogErrorFile.LogerrorDB(ex, HttpContext.Current.Request.Url.AbsoluteUri, Session["uid"].ToString());
    ...
    protected void Button6_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                string message = "alert('Kindly Check whether the incentive full shap or not')";
                ScriptManager.RegisterClientScriptBlock((sender as Control), this.GetType(), "alert", message, true);
    ...
    public void SendSmsEmail(string querydesc, string incentivename)
    {
        try
        {
            ...
            try
            {
                cmf.SendMailIncentive(UnitEmail, "Dear " + UnitName + ", Query is raised for Your incentives application on " + QueryRaisedDate + ", and Query raised incentives : " + incentivename + ", and query description is  " + querydesc + ", <br /> Thank You, GM, DIC -" + DistrictName + ",<br />  Govt. of Telangana.");
            ...
            try
            {
                cmf.SendSingleSMS(UnitMobileNo, "Dear " + UnitName + ", Query is raised for Your incentives application on " + QueryRaisedDate + ", and Query raised incentives : " + incentivename + ", and a detail mail is sent to " + nameuid + "," + '\n' + "Thank You," + '\n' + "GM, DIC -" + DistrictName + "," + '\n' + "Govt. of Telangana.");
    ...
    protected void gvquerygm_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (gmquerystatus == "GMQUERYRAISED")
                {
                    btncoigmraisequery.Visible = false;
    ...
    public void SendRejectSmsEmail(string querydesc, string incentivename, string Role)
    {
        try
        {
            ...
            try
            {
                if (Role == "GM")
                {
                    cmf.SendSingleSMS(UnitMobileNo, "Dear " + UnitName + ", Your incentive  application rejected on " + QueryRaisedDate + ", and Rejected incentive : " + incentivename + ", and a detail mail is sent to " + nameuid + "," + '\n' + "Thank You," + '\n' + "GM, DIC -" + DistrictName + "," + '\n' + "Govt. of Telangana.");
                }
                else if (Role == "JD")
                {
                    cmf.SendSingleSMS(UnitMobileNo, "Dear " + UnitName + ", Your incentive  application rejected on " + QueryRaisedDate + ", and Rejected incentive : " + incentivename + ", and a detail mail is sent to " + nameuid + "," + '\n' + "Thank You," + '\n' + "JD-INCENTIVES, COI," + '\n' + "Govt. of Telangana.");
    ...
    public void OTPMobile()
    {
        try
        {
            ...
            if (dsdept != null && dsdept.Tables.Count > 0 && dsdept.Tables[0].Rows.Count > 0)
            {
                ...
                SendSingleSMSnew(dsdept.Tables[0].Rows[0]["MobileNumber"].ToString(), bodyMobile);
        ...
        try
        {
            if (ds != null && ds.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < ds.Rows.Count; i++)
                {
                    ...
                    {
                        if (enterid.Text == "66420" || enterid.Text == "71515" || enterid.Text == "70553" || enterid.Text == "56155" || enterid.Text == "77623" || enterid.Text == "60488" || enterid.Text == "77946")
                        {
                            string msttid;
