...
public partial class UI_TSiPASS_frmPaymentMultiplex : System.Web.UI.Page
{
    Cls_Multiplex obj_adv = new Cls_Multiplex();
    int delete = 0;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            DataSet ds = obj_adv.GetMultiplexAppIDbyuserid(CreatedBy, Request.QueryString[0].ToString());
            if (ds.Tables.Count > 0)
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (App_Status == "2")
                    {
                        ...
                        dsv = null;
                        dsv = obj_adv.GetShowApprovalandFees_Multiplex(Session["Applid"].ToString(), Request.QueryString[0].ToString());
                    ...
                    else
                    {
                        ...
                        {
                            DataSet ds_ap = obj_adv.GetMultiplexPayDetailsAddtionalPayment(Session["Applid"].ToString().Trim(), Request.QueryString[0].ToString());
    ...
    protected void BtnSave_Click(object sender, EventArgs e)
    {
        ...
        int UIDUpdate = obj_adv.UpdateUIDMultiplex("", hdnTransactionNumber.Value);
        ...
        //int UIDUpdate = obj_adv.UpdateUIDMultiplex(Session["UID_no"].ToString(), hdnTransactionNumber.Value);
        ...
        foreach (GridViewRow row in grdDetails.Rows)
        {
            ...
            {
                s = obj_adv.insertDepartmentAprroval_Multiplex(hdnTransactionNumber.Value, HdfQueid, HdfApprovalid, HdfAmount, "N", Session["uid"].ToString().Trim(), "N");
            }

            DataSet dsch = new DataSet();
            dsch = obj_adv.getMultiplexdatabyQues(hdnTransactionNumber.Value);
            if (dsch.Tables[0].Rows.Count > 0)
            {
                string Hdfeid = dsch.Tables[0].Rows[0]["CreatedBy"].ToString();

                int A = obj_adv.InsertPaymentBillDesk_Multiplex(Session["UID_no"].ToString(), Convert.ToString(Session["Applid"]), sonlinetrnsNo, HdfQueid, HdfAmount, Session["uid"].ToString(), HdfApprovalid, "MS", Session["AdditionalPayment"].ToString());
        ...
        if (Result.ToUpper().ToString() == "SUCCESS")
        {
            try
            {
                Response.Redirect("KotakMultiplex.aspx");
            }
            catch (Exception ex)
            {
    ...
    public DataSet GetUidnumber(string intQuessionaireid)
    {
        ...
        try
        {
            da = new SqlDataAdapter("getUIDNumberdata_Multiplex", con.GetConnection);
