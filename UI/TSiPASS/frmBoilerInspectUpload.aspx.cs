...
public partial class UI_TSiPASS_frmBoilerInspectUpload : System.Web.UI.Page
{
    int delete = 0;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            Response.Redirect("frmBoilerInspectUpload.aspx");
        }
        ...
        if (!IsPostBack)
        {
            ...
            dsnew = Gen.getdataofidentityCFONewApproval(Session["ApplidA"].ToString(), "63");
            ...
            else
            {
                if (Request.QueryString[1].ToString() == "N")
                {
                    ...
                    Response.Redirect("frmBoilerSteamTest.aspx?intApplicationId=" + Session["uid"].ToString() + "&next=" + "N");
    ...
    void FillDetails()
    {
        ...
        try
        {
            ...
            ds = Gen.GetBoilerUploadDetailsCFO(cfeid, "27");
            ...
            if (dsnew.Tables[0].Rows.Count > 0)
            {
                ...
                while (i < c)
                {
                    ...
                    if (sen1.Contains("CFOAttachments"))
                    {
                        ...
                        if (sen.Contains("ErectionPermissionAttachment"))
                        {
    ...
    protected void BtnClear_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            ds = Gen.GetBoilerUploadDetailsCFO(Request.QueryString[0].ToString(), "63");
        ...
        try
        {
            ...
            if (Label453.Text == "")
            {
                lblmsg0.Text = "<font color='red'>Please Upload Errection Attachment..!</font>";
            ...
            if (payment != "Y")
            {
                ...
                {
                    int quedata = Gen.updatenonPaymentinCFOQueNew(dslat.Tables[0].Rows[0]["intQuessionaireCFOid"].ToString(), Session["uid"].ToString(), "63", "0.00");
                }

                result = 1;
                //result = Gen.InsertErectionPermissionUploadCFO(intEnterpreniourApprovalid, intQuessionaireid, intCFEEnterpid, intDeptid, intApprovalid, QueryRaiseDate, QueryDescription, QueryStatus, ViewState["AttachmentName"].ToString(), ViewState["pathAttachment"].ToString(), "", queryresopnedesc, Created_by, "", "", "", getclientIP());
                ...
                if (result > 0)
                {
                    ...
                    //dsdept = Gen.GetBoilerErectionCFOResponse(intQuessionaireid, intApprovalid);
                    ...
                    if (dsMail.Tables[0].Rows.Count > 0)
                    {
                        ...
                        cmf.SendMailTSiPASSCFO(dsMail.Tables[0].Rows[0]["Email"].ToString().Trim(), "Dear " + Label448.Text + " - (" + Label447.Text + ") :<br/><br/> <b> Errection Report has been submitted successfully.Further details will be communicated. Thank You.");
                    }
                    if (dsMail.Tables[0].Rows[0]["MobileNo"].ToString().Trim() != "")
                    {
                        ...
                        cmf.SendSingleSMS(dsMail.Tables[0].Rows[0]["MobileNo"].ToString().Trim(), "Dear " + Label448.Text + " - (" + Label447.Text + ") : Errection Report has been submitted successfully.Further details will be communicated. Thank You.");
                    }



                    //Response.Redirect("frmLINEOFMANUFACTURE.aspx?intApplicationId=" + hdfID.Value);
                    lblmsg.Text = "<font color='green'>Errection Report Added Successfully..!</font>";
                ...
                {
                    lblmsg0.Text = "<font color='red'>Errection Report Adding Failed..!</font>";
                    success.Visible = false;
                    Failure.Visible = true;
        ...
        if ((FileUpload1.PostedFile != null) && (FileUpload1.PostedFile.ContentLength > 0))
        {
            ...
            try
            {
                ...
                if (fileType[i - 1].ToUpper().Trim() == "PDF" || fileType[i - 1].ToUpper().Trim() == "DOC" || fileType[i - 1].ToUpper().Trim() == "JPG" || fileType[i - 1].ToUpper().Trim() == "XLS" || fileType[i - 1].ToUpper().Trim() == "XLSX" || fileType[i - 1].ToUpper().Trim() == "DOCX" || fileType[i - 1].ToUpper().Trim() == "ZIP" || fileType[i - 1].ToUpper().Trim() == "RAR" || fileType[i - 1].ToUpper().Trim() == "DWG")
                {
                    ...
                    newPath = System.IO.Path.Combine(sFileDir, Session["uid"].ToString() + "\\ErectionPermissionAttachment\\" + hdfFlagID1.Value);
                    ...
                    result = t1.InsertCFOAttachement(Session["ApplidA"].ToString(), Session["uid"].ToString(), "1", "1", fileType[i - 1].ToUpper(), sFileName, newPath, "A", Session["uid"].ToString(), "ErectionPermissionAttachment");



                    //result = t1.InsertImagedata(Session["ApplidA"].ToString(), Request.QueryString[0].ToString(), fileType[i - 1].ToUpper(), newPath, sFileName, Session["uid"].ToString(), DateTime.Now.ToString(), "1000", DateTime.Now.ToString());
                    if (result > 0)
                    {
                        //ResetFormControl(this);
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (payment != "Y")
                    {
                        ...
                        {
                            lblmsg0.Text = "<font color='red'>Errection Report dding Failed..!</font>";
                            success.Visible = false;
                            Failure.Visible = true;
                            // Response.Write("<script>alert('Attachment Added Failed ')</script> ");
                        }
